...
require('../declarationVariables.php');
    ...
    if ($operation=="image") {
      ...
      $sql13="SELECT * from  `".$categorieTypeCateg."` where id='".$idCategorie."'";
      //var_dump($sql13);
      $res13 = mysql_query($sql13) or die ("persoonel requête 3".mysql_error());
      $design = mysql_fetch_assoc($res13);
  
      $result=$design['id'].'<>'.$design['nomCategorie'].'<>'.$design['image'];
      exit($result);
    ...
    if($operation == 'EditCategorie_B'){
          ...
          if(mysql_num_rows($res1)){
                $design=mysql_fetch_assoc($res1);
                $nomCategorie_old=$design['nomCategorie'];
            }
        $nomCategorie=@htmlspecialchars($_POST["categorieD"]);
        $categorieParent=@htmlspecialchars($_POST["categorieP"]);
        $result="1";
    ...
    if($operation == 'SearchCategorie_B'){
      $idCategorie=@$_POST["idCategorie"];
      $result='';
      $sql1="SELECT * from  `".$categorieTypeCateg."` WHERE id ='".$idCategorie."' ";
      $res1=mysql_query($sql1);
      if(mysql_num_rows($res1)){
            $design=mysql_fetch_assoc($res1);
            $result=$idCategorie.'<>'.$design['nomCategorie'].'<>'.$design['categorieParent'];
        }
     exit($result);
  }
  if($operation == 'DeleteCategorie_B'){
    $idCategorie=@$_POST["idCategorie"];
    $nomCategorie=@$_POST["nomCategorie"];
    $categorieRemplacant=@$_POST["categorieRempl"];
  ...
  if(mysql_num_rows($res1)){
    ...
    if ($res0=mysql_query($sql0)) {
      ...
      $res1=mysql_query($sql1);
        while($catalogue =mysql_fetch_array($res1)){
          $sql2="update `".$catalogueTypeCateg."` set
          categorie='".mysql_real_escape_string($categorieRemplacant)."',
          categorieParent='".mysql_real_escape_string($categorieParent)."'
          where id=".$catalogue['id'];
          var_dump($sql2);
          $res2=@mysql_query($sql2)or die ("modification impossible1 ".mysql_error());
  ...
  ///////////////////
   exit($result);
    ...
    if ($_POST['action']=='listeSelectCategorie') {
      $rows[]='Sans categorie';
