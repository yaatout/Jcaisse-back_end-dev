...
if ($operation=="1" || $operation=="3" || $operation=="4") {  
  ...
  $nbEntreePhF2 = @$_POST["nbEntreePhF2"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= $nbEntreePhF2; //item to display per page
    ...
    JOIN (SELECT *, COUNT( * ) AS qty FROM `".$catalogueTypeCateg."`
       GROUP BY codeBarreDesignation HAVING COUNT( * ) >=1 )t ON s.codeBarreDesignation = t.codeBarreDesignation AND
        ...
        OR s.`prixSession` <> t.`prixSession` OR s.`prixPublic` <> t.`prixPublic`
         OR s.`designation` <> t.`designation` ) and (s.codeBarreDesignation<>'' or s.codeBarreDesignation<>null) 
                ";
    $res=mysql_query($sql);
  } else {
    //get total number of records from database
