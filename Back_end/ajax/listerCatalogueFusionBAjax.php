...
if ($operation=="1" || $operation=="3" || $operation=="4") {  
  ...
  $nbEntreeBF = @$_POST["nbEntreeBF"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= $nbEntreeBF; //item to display per page
  ...
  if ($query =="") {
    ...
    JOIN (SELECT *, COUNT( * ) AS qty FROM `".$catalogueTypeCateg."`
       GROUP BY designation HAVING COUNT( * ) >=1 )t ON s.designation = t.designation AND
        (s.categorie <> t.categorie OR s.`uniteStock`<>t.`uniteStock` OR s.`nbreArticleUniteStock` <> t.`nbreArticleUniteStock` 
        OR s.`prixuniteStock` <> t.`prixuniteStock` OR s.`prix` <> t.`prix`
  ...
  while($design=mysql_fetch_array($res)){ //fetch values
    ...
    echo '<td><a><button type="button"  name="button" id="btnFusB-'.$design['id'].'" onclick="fus_Designation_BO('.$design["id"].','.$id.','.$i.')">Fusion</button></a></td>'; 
    echo '</tr>';
    $i++;
    $cpt++;
  }
