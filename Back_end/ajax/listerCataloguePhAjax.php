...
if ($operation=="1" || $operation=="3" || $operation=="4") {  
  ...
  while($design=mysql_fetch_array($res)){ //fetch values
    ...
    $selForme='<td><span style="" >
            <select class="form-control" id="formeD-'.$design['id'].'" >
    ...
    $selTableau='<td><span style="" >
            <select class="form-control" id="tableauD-'.$design['id'].'" >
    ...
    //if ($i==1) {
      echo  '<td><span style="color:blue;"><form class="form form-inline" role="form"  method="post" >
							<input type="text" size="40" class="form-control" id="designationD-'.$design['id'].'" min=1 value="'.$design['designation'].'" required=""/></span></td>';
      echo $selCategorie;
      echo $selForme;
      echo $selTableau;
      echo  '<td><span style="color:blue;"><input type="number" size="6" class="form-control" id="prixSessionD-'.$design['id'].'" min=0 value="'.strtoupper($design['prixSession']).'" required=""/></span></td>';
      echo  '<td><span ><input type="number" size="6" class="form-control" id="prixPublicD-'.$design['id'].'" min=0 value="'.strtoupper($design['prixPublic']).'" required=""/>
      </span></td>';
      echo  '<td><span ><input type="number" size="6" class="form-control" id="codebarreD-'.$design['id'].'" min=0 value="'.strtoupper($design['codeBarreDesignation']).'" >
      </form></span></td>';
   /* } else {
      echo  '<td>'.$i.'</td>';
      ...
      echo  '<td><input type="number" size="6" class="form-control" id="prixPublicD-'.$design['id'].'" min=0 value="'.strtoupper($design['prixPublic']).'" required=""/>
        ...
        if ($design["image"]) {
          ...
          echo '<td><a onclick="mdf_Designation_PH('.$design["id"].','.$id.','.$i.')" id="pencilmoD-'.$design['id'].'"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
          <a onclick="spm_Designation_PH('.$design["id"].','.$id.','.$i.')" /></a>
          <a><img src="images/iconfinder11.png" align="middle" alt="apperçu"  onclick="img_Designation_PH('.$design["id"].','.$id.','.$i.')" /></a></td>';
        }
          else{
            echo '<td><a onclick="mdf_Designation_PH('.$design["id"].','.$id.','.$i.')" id="pencilmoD-'.$design['id'].'"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
          <a onclick="spm_Designation_PH('.$design["id"].','.$id.','.$i.')" ><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>
           <a><img src="images/iconfinder11.png" align="middle" alt="apperçu"  onclick="img_Designation_PH('.$design["id"].','.$id.','.$i.')" /></a></td>';
        }
      }else{
        if($design["codeBarreDesignation"]!=null){
          echo '<td><a onclick="mdf_Designation_PH('.$design["id"].','.$id.','.$i.')" id="pencilmoD-'.$design['id'].'"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
            <a onclick="spm_Designation_PH('.$design["id"].','.$id.','.$i.')" ><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>
            <a><img src="images/iconfinder9.png" align="middle" alt="apperçu"  onclick="img_Designation_PH('.$design["id"].','.$id.','.$i.')" /></a></td>';
        }
        else{
          echo '<td><a onclick="mdf_Designation_PH('.$design["id"].','.$id.','.$i.')" id="pencilmoD-'.$design['id'].'"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
            <a onclick="spm_Designation_PH('.$design["id"].','.$id.','.$i.')" ><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>
...
if ($operation=="2") {
  ...
  if ($tri==0) {
    if ($query =="") {
      ...
      $sql="select * from `".$catalogueTypeCateg."` where designation LIKE '%".$query."%' or codeBarreDesignation LIKE '%".$query."%' order by designation ASC LIMIT $page_position, $item_per_page";
      $res=mysql_query($sql);
      $sql="select * from `".$catalogueTypeCateg."` where designation LIKE '%".$query."%' or codeBarreDesignation LIKE '%".$query."%' order by id desc LIMIT 1";
      $res1=mysql_query($sql);  
    }
  } else {
    if ($query =="") {
      ...
      $res=mysql_query($sql);
      $sql="select * from `".$catalogueTypeCateg."` where designation LIKE '%".$query."%' or codeBarreDesignation LIKE '%".$query."%' order by id desc LIMIT $page_position, $item_per_page";
      $res1=mysql_query($sql);
    }
  }
  ...
  while($design=mysql_fetch_array($res)){ //fetch values
    ...
    if ($design['idDesignation']==$maxId[0]) {
      ...
      <option selected value= "'.$design['forme'].'"
       >'.$design['forme'].'</option></select></span></td>';
      echo  '<td><span style="color:blue;"><input type="text" size="4" class="form-control" id="tableauD-'.$design['id'].'" value="'.strtoupper($design['tableau']).'" required=""/></span></td>';
      echo  '<td><span style="color:blue;"><input type="number" size="6" class="form-control" id="prixSessionD-'.$design['id'].'" min=0 value="'.strtoupper($design['prixSession']).'" required=""/></span></td>';
      echo  '<td><span style="color:blue;"><input type="number" size="6" class="form-control" id="prixPublicD-'.$design['id'].'" min=0 value="'.strtoupper($design['prixPublic']).'" required=""/>
      </form></span></td>';
    } else {
      echo  '<td>'.$i.'</td>';
      echo  '<td><form class="form form-inline" role="form"  method="post" >
      <input type="text" size="40" class="form-control" id="designationD-'.$design['id'].'" min=1 value="'.$design['designation'].'" required=""/></td>';
      echo  '<td><input size="4" type="text" class="form-control"  value="'.strtoupper($design['idFusion']).'" disabled/></td>';
      echo  '<td><span onclick="selectCategorie_PH('.$design["id"].','.$id.')"><select  size="5" class="form-control" id="categorieD-'.$design['id'].'"  >
      <option selected value= "'.$design['categorie'].'"
       >'.$design['categorie'].'</option></select></span></td>';
      echo  '<td><span onclick="selectForme('.$design["id"].','.$id.')"><select  size="5" class="form-control" id="formeD-'.$design['id'].'"  >
      <option selected value= "'.$design['forme'].'"
...
if ($operation=="13") {
  ...
  if ($query =="") {
    ...
    //get total number of records from database
    $sql="select COUNT(*) as total_row from `".$catalogueTypeCateg."` where designation LIKE '%".$query."%'";
    $res=mysql_query($sql);
  }
  $total_rows = mysql_fetch_array($res);
  $total_pages = ceil($total_rows[0]/$item_per_page);
  ...
  $page_position = (($page_number-1) * $item_per_page);
  if ($query =="") {
    $sql="select * from `".$catalogueTypeCateg."` order by idDesignation desc LIMIT $page_position, $item_per_page";
    $res=mysql_query($sql);
  } else {
    //Limit our results within a specified range. 
    $sql="select * from `".$catalogueTypeCateg."` where designation LIKE '%".$query."%' order by idDesignation desc LIMIT $page_position, $item_per_page";
    $res=mysql_query($sql);
  }
  //Display records fetched from database.
