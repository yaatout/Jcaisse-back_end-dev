...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbCompteBancaire = @$_POST["nbCompteBancaire"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbCompteBancaire!=0) ? $nbCompteBancaire : 10; //item to display per page
  ...
  $stmt0 = $bdd->prepare("SELECT * FROM `aaa-comptetype` WHERE nomType like :typeCompte");
  $stmt0->execute(array(  'typeCompte' => "%$typeCompte%" ));
  $type = $stmt0->fetch(PDO::FETCH_ASSOC);

  $stmt1 = $bdd->prepare("SELECT * FROM `aaa-compte` WHERE typeCompte = :typeId  ");
  $stmt1->execute(array( 'typeId' => $type['idType'] ));
  $compte = $stmt1->fetch(PDO::FETCH_ASSOC);

  $sql = "SELECT * FROM `aaa-compte` c 
        INNER JOIN `aaa-comptetype` t ON c.typeCompte = t.idType 
        INNER JOIN `aaa-comptemouvement` m ON c.idCompte = m.idCompte 
        WHERE idType = :idType 
...
$data=array();
$i=1;
$mouvs=$stmt->fetchAll(PDO::FETCH_ASSOC);
foreach($mouvs as $mouv){
  //var_dump($mouv);
        ...
        $style=($i == 1) ? 'color:blue;' : '';
          ...
          $rows[] = '<span style="'.$style.'">'.strtoupper($mouv['nomCompte']).'</span>';  
