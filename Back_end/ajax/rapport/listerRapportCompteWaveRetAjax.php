...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbCompteWaveRet = @$_POST["nbCompteWaveRet"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbCompteWaveRet!=0) ? $nbCompteWaveRet : 10; //item to display per page
  ...
  foreach ($results as $mouv) {
    ...
    // Gestion du texte d'opération avec vérification d'annulation
    $text = strtoupper($mouv['operation']);
    if ($mouv['operation'] == "retrait" && isset($mouv['annuler']) && $mouv['annuler'] == 1) {
        $text .= ' Mais Annulé';
    }
    
    $rows[] = $i;  
    $rows[] = '<span style="'.$style.'">'.$text.'</span>';  
    $rows[] = '<span style="'.$style.'">'.number_format($mouv['montant'], 0, ',', ' ').'</span>';
    $rows[] = '<span style="'.$style.'">'.htmlspecialchars($mouv['description']).'</span>';  
