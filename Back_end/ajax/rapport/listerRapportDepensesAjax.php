...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbDepenses = @$_POST["nbDepenses"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbDepenses!=0) ? $nbDepenses : 10; //item to display per page
  ...
  $sql = "SELECT * FROM `aaa-payement-salaire`  
    WHERE  datePS BETWEEN :debut AND :fin";
  ...
  if (!empty($query)) {
      $sql .= " AND idBoutique LIKE :query";
      $params['query'] = "%$query%";
  }

  $sql .= " ORDER BY datePaiement DESC ";
  ...
  foreach($payements as $pay){
    ...
    $sqlN="select * from `aaa-boutique` where idBoutique=:idBoutique"; 
    $stmtN = $bdd->prepare($sqlN);
    $stmtN->execute(array('idBoutique' => $pay['idBoutique'])) or die(print_r($stmtN->errorInfo(), true));
    $boutique = $stmtN->fetch(PDO::FETCH_ASSOC);
        $rows = array();
        $style = ($i == 1) ? 'color:blue;' : '';

        $rows[] = '<span style="'.$style.'">'.strtoupper($boutique['nomBoutique']).'</span>';  
        $rows[] = '<span style="'.$style.'">'.strtoupper($pay['datePaiement']).'</span>';
        $rows[] = '<span style="'.$style.'">'.number_format($pay['montantFixePayement'], 0, ',', ' ').'</span>';
        $rows[] = '<span style="'.$style.'">'.strtoupper($pay['refTransfert']).'</span>';  
