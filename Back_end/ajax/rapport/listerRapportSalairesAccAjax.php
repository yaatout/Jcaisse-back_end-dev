...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbSalaireAcc = @$_POST["nbSalaireAcc"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbSalaireAcc!=0) ? $nbSalaireAcc : 10; //item to display per page
  ...
  $sql = "SELECT ps.*, u.prenom, u.nom, b.nomBoutique, b.labelB 
  FROM `aaa-payement-salaire` ps
  LEFT JOIN `aaa-utilisateur` u ON ps.accompagnateur = u.matricule
  LEFT JOIN `aaa-boutique` b ON ps.idBoutique = b.idBoutique
  WHERE ps.aSalaireAccompagnateur = 1 
  AND ps.datePS BETWEEN :debut AND :fin";
  ...
  if (!empty($query)) {
  $sql .= " AND (b.idBoutique LIKE :query OR u.nom LIKE :queryNom OR u.prenom LIKE :queryPrenom)";
  $params['query'] = "%$query%";
  ...
  while($pay = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    $rows[] = '<span style="'.$style.'">'.strtoupper($pay['accompagnateur']).'</span>';   
    $rows[] = '<span style="'.$style.'">'.strtoupper($pay['etapeAccompagnement']).'</span>';
    $rows[] = '<span style="'.$style.'">'.number_format($pay['montantFixePayement'], 0, ',', ' ').'</span>';
    $rows[] = '<span style="'.$style.'">'.strtoupper($pay['labelB']).'</span>';  
