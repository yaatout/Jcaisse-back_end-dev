...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbSalairesEdi= @$_POST["nbSalairesEdi"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbSalairesEdi!=0) ? $nbSalairesEdi : 10; //item to display per page
  ...
  // Construction de la requête principale avec JOIN
  $sql = "SELECT sp.*, u.prenom, u.nom, u.profil, c.nomCompte 
  FROM `aaa-salaire-personnel` sp
  LEFT JOIN `aaa-utilisateur` u ON sp.idUtilisateur = u.idutilisateur
  LEFT JOIN `aaa-compte` c ON sp.comptePaiement = c.idCompte
  WHERE sp.profil = 'Editeur catalogue' 
  AND sp.datePaiement BETWEEN :debut AND :fin";
  ...
  if (!empty($query)) {
    $sql .= " AND (u.profil LIKE :query OR u.nom LIKE :queryNom OR u.prenom LIKE :queryPrenom)";
    $params['query'] = "%$query%";
    $params['queryNom'] = "%$query%";
    $params['queryPrenom'] = "%$query%";
  }

  // Tri
  $sql .= " ORDER BY sp.idSP DESC";
  ...
  $data = array();
  $i = 1;
  while($pay = $stmt->fetch(PDO::FETCH_ASSOC)){
    $rows = array();
    $style = ($i == 1) ? 'color:blue;' : '';

    $rows[] = '<span style="'.$style.'">'.$i.'</span>';  
    $rows[] = '<span style="'.$style.'">'.strtoupper($pay['prenom']).'</span>';  
    $rows[] = '<span style="'.$style.'">'.strtoupper($pay['nom']).'</span>'; 
    $rows[] = '<span style="'.$style.'">'.strtoupper($pay['profil']).'</span>';
    $rows[] = '<span style="'.$style.'">'.number_format($pay['montant'], 0, ',', ' ').'</span>';
    $rows[] = '<span style="'.$style.'">'.strtoupper($pay['nomCompte']).'</span>';  
