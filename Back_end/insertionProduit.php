      ...
      if($classe==0){
        ...
        if(!mysql_num_rows($res11)){
          if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
            $sql="insert into `".$nomtableDesignation."`     (designation,classe,uniteStock,prixuniteStock,nbreArticleUniteStock,uniteDetails,prixuniteDetails,seuil,categorie)values ('".mysql_real_escape_string($designation)."',0,'".mysql_real_escape_string($uniteStock)."','".$prixuniteStock."','".$nbArticleUniteStock."','".mysql_real_escape_string($uniteDetails)."','".$prixuniteDetails."','10','".mysql_real_escape_string($categorie2)."')";
            ...
            if ($uniteStock=="article" || $uniteStock=="Article"){
              $sql="insert into `".$nomtableDesignation."` (designation,classe,prixachat,prix,uniteStock,prixuniteStock,nbreArticleUniteStock,seuil,categorie)
              values ('".mysql_real_escape_string($designation)."',0,".$prixAchat.",".$prixUnitaire.",'Article','".$prixUnitaire."','1','10','".mysql_real_escape_string($categorie2)."')";
              $res=@mysql_query($sql) or die ("insertion impossible Produit en Article".mysql_error());
            }
            else {
                  $sql="insert into `".$nomtableDesignation."` (designation,classe,prixachat,prix,uniteStock,prixuniteStock,nbreArticleUniteStock,seuil,categorie)
                  values ('".mysql_real_escape_string($designation)."',0,".$prixAchat.",".$prixUnitaire.",'".mysql_real_escape_string($uniteStock)."','".$prixuniteStock."','".$nbArticleUniteStock."','10','".mysql_real_escape_string($categorie2)."')";
                  $res=@mysql_query($sql) or die ("insertion impossible Produit en uniteStock".mysql_error());
            }
          }
...
if (isset($_POST['subImport1'])) {
  ...
  if ($_FILES["fileImport"]["size"] > 0) {
    ...
    while (($data=fgetcsv($handle,1000,";")) !=FALSE) {
		...
		$res11=mysql_query($sql11);
		if(!mysql_num_rows($res11)){
		   $sql3="insert into `".$nomtableDesignation."`(designation,uniteStock,nbreArticleUniteStock,prix,prixuniteStock,categorie,classe)
...
if(isset($_POST["Export"])){
      ...
      $fields=array('REFERENCE','CATEGORIE', 'UNITE-STOCK','NBRES-ARTICLES-UNITE-STOCK',
                    'PRIX-UNITAIRE', 'PRIX-UNITE-STOCK');
...
<script>
$('#uniteDetail').on('change', function() {
  var value = $(this).val();
  if(value =='Article'){
    document.getElementById('nbArticleUniteDetail').value = 1;
    document.getElementById("nbArticleUniteDetail").disabled = true;
    $("#div-nbArticleUniteDetail").hide();
    //document.getElementById('prixuniteStock').value = $("#prixuniteStock").value();
    $("#div-prixuniteDetails").hide();
  }
  else {
    $('#nbArticleUniteDetail').val(null);
    document.getElementById("nbArticleUniteDetail").disabled = false;
    $("#div-nbArticleUniteDetail").show();
    //document.getElementById('prixuniteStock').value = "";
    $("#div-prixuniteDetails").show();
  }
...
<div class="form-group" id="div-prixuniteDetails">
  <label for="prixuniteDetails">PRIX UNITE DETAILS</label>
  <input type="number" class="form-control" placeholder="Prix Unite Details" id="prixuniteDetails" name="prixuniteDetails" value="" />
</div>
    ...
    <div class="modal-content">
		...
		<div class="bs-example bs-example-tabs">
			<ul id="myTab" class="nav nav-tabs">
				...
				<li class=""><a href="#transaction" data-toggle="tab">Transfert d'argent</a></li>
        <li class=""><a href="#facture" data-toggle="tab">Paiement facture</a></li>
        <li class=""><a href="#credit" data-toggle="tab">Credits</a></li>
			</ul>
		</div>
      <div class="modal-body">
        <div id="myTabContent" class="tab-content">
            ...
            <div class="row">
              <?php 
                  ...
                  while ($transaction = mysql_fetch_array($resT)){
                    if(($transaction['idTransaction'] % 2) == 1){
                      echo '
                        <div class="col-xs-6">
                          <div class="well well-sm">';
                            ...
                            if(mysql_num_rows($res0)){
                              ...
                              if($trans['seuil']==1){
                                echo '
                                  <form  method="POST" >
                                    <img src="images/'.$transaction['aliasTransaction'].'.png" width="50" height="50" border="0" class="img-circle"></a>
                                    <input type="hidden" name="idDesignation" value='.$trans['idDesignation'].' >
                                    <button type="submit"  name="desactiverT" class="btn btn-danger pull-right">Desactiver</button>
                                    <h6 class="media-heading">'.$transaction['nomTransaction'].'</h6>
                                  </form>
                                ';
                              }
                              else{
                              ...
                              echo '
                                  <form  method="POST" >
                                    ...
                                    <button type="submit"  name="ajouterT" class="btn btn-success pull-right">Ajouter</button>
                                    <h6 class="media-heading">'.$transaction['nomTransaction'].'</h6>
                                  </form>
                                ';
                            }
          ...
          <div class="tab-pane fade" id="facture">
            <div class="row">
              <?php 
                  $reqT="SELECT * from `aaa-transaction` where typeTransaction='Facture' ";
                  $resT=mysql_query($reqT) or die ("select stock impossible =>".mysql_error());
                  while ($transaction = mysql_fetch_array($resT)){
                    if(($transaction['idTransaction'] % 2) == 1){
                      echo '
          ...
          <div class="tab-pane fade" id="credit">
            <div class="row">
              <?php 
                  $reqT="SELECT * from `aaa-transaction` where typeTransaction='Credit' ";
                  $resT=mysql_query($reqT) or die ("select stock impossible =>".mysql_error());
                  while ($transaction = mysql_fetch_array($resT)){
                    if(($transaction['idTransaction'] % 2) == 1){
                      echo '
