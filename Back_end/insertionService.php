...
if(!$modifier and !$supprimer){
  if($nomTransaction) {
      ...
      if(empty($nomTransaction)) {
          ...
          echo'<html>';
          echo'<head>';
          echo'<script>alert("Le nom du service ne peut pas être vide.");</script>';
      ...
      if(strlen($nomTransaction) < 2) {
          ...
          echo'<html>';
          echo'<head>';
          echo'<script>alert("Le nom du service doit contenir au moins 2 caractères.");</script>';
          echo'<script language="JavaScript">document.location="insertionService.php"</script>';
          echo'</head>';
          echo'</html>';
          exit;
      ...
      if(strlen($nomTransaction) > 100) {
          ...
          echo'<html>';
          echo'<head>';
          echo'<script>alert("Le nom du service ne peut pas dépasser 100 caractères.");</script>';
          echo'<script language="JavaScript">document.location="insertionService.php"</script>';
          echo'</head>';
          echo'</html>';
          exit;
      ...
      // Vérification si la transaction existe déjà
		...
		echo'<html>';
		echo'<head>';
		echo'<script>alert("Ce service existe déjà.");</script>';
		echo'<script language="JavaScript">document.location="insertionService.php"</script>';
		echo'</head>';
		echo'</html>';
		}
...
elseif($modifier){
      ...
      if($req11->rowCount() > 0){
       ...
       try {
           ...
           die("Erreur lors de la modification du service : " . $e->getMessage());
       }
}
else if ($supprimer) {
    try {
...
<div class="container">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                       <form role="form" class=" formulaire2" name="formulaire2" method="post" action="insertionService.php" onsubmit="return validerService();">
    ...
    function validerService() {
        var nomService = document.getElementById('nomTransaction').value.trim();
        var aliasService = document.getElementById('aliasTransaction').value.trim();
        ...
        if(nomService === '') {
            erreurNom.innerHTML = 'Le nom du service ne peut pas être vide.';
            erreurNom.style.display = 'block';
            document.getElementById('nomTransaction').focus();
            return false;
        }
        
        if(nomService.length < 2) {
            erreurNom.innerHTML = 'Le nom du service doit contenir au moins 2 caractères.';
            erreurNom.style.display = 'block';
            document.getElementById('nomTransaction').focus();
            return false;
        }
        ...
        if(nomService.length > 100) {
            erreurNom.innerHTML = 'Le nom du service ne peut pas dépasser 100 caractères.';
            erreurNom.style.display = 'block';
            document.getElementById('nomTransaction').focus();
            return false;
        }
