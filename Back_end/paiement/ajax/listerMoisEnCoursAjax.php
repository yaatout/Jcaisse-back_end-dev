...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreEnCours = @$_POST["nbEntreEnCours"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntreEnCours!=0) ? $nbEntreEnCours : 10; //item to display per page
...
//get total number of records from database
    ...
    and (`datePS` LIKE '%".$annee."-".$mois."%')");     
    $sqlb->execute(array('p' =>1)); 
    $total_b = $sqlb->fetch();   */                                  
    
    //var_dump(intval($total_a[0]));
    //var_dump(intval($total_b[0]));
  ...
  $total_pages = ceil($total_rows/$item_per_page);
      ...
      if ($query =="") {
        $req1 = $bdd->prepare("SELECT * FROM `aaa-payement-salaire` p LEFT JOIN `aaa-boutique` b ON p.idBoutique = b.idBoutique WHERE 
            (`datePS` LIKE '%".$annee."-".$mois."%') GROUP BY b.idBoutique ORDER BY `aPayementBoutique` ASC,pPlusieursMois  LIMIT $page_position ,$item_per_page"); 
        ...
        //Limit our results within a specified range.         
            $req1 = $bdd->prepare("SELECT * FROM `aaa-payement-salaire` p LEFT JOIN `aaa-boutique` b ON p.idBoutique = b.idBoutique WHERE 
                 labelB LIKE '%".$query."%' and (`datePS` LIKE '%".$annee."-".$mois."%' ) GROUP BY b.idBoutique ORDER BY `aPayementBoutique` ASC,pPlusieursMois LIMIT $page_position ,$item_per_page"); 
            $req1->execute()  or die(print_r($req1->errorInfo())); 
                                           ...
                                           <td>  
                                            ...
                                            <?php
                                                 if ($payement['aPayementBoutique']==0) { ?>
                                                  ...
                                                  <td>
                                                    <?php   if ($payement['pPlusieursMois']!=0) { ?>
                                                          Payé à l'avance
                                                    <?php   } else { ?>
                                                        Déja payé
                                                    <?php   } ?>
