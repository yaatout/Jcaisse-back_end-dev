...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntrePrec = @$_POST["nbEntrePrec"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntrePrec!=0) ? $nbEntrePrec : 10; //item to display per page
  ...
  $total_pages = ceil($total_rows/$item_per_page);
      ...
      if ($query =="") {
        $req1 = $bdd->prepare("SELECT * FROM `aaa-payement-salaire` p LEFT JOIN `aaa-boutique` b ON p.idBoutique = b.idBoutique WHERE 
            (`datePS` LIKE '%".$anneeM."-".$moiM."%') GROUP BY b.idBoutique ORDER BY `aPayementBoutique` ASC,pPlusieursMois  LIMIT $page_position ,$item_per_page"); 
        ...
        //Limit our results within a specified range.         
            $req1 = $bdd->prepare("SELECT * FROM `aaa-payement-salaire` p LEFT JOIN `aaa-boutique` b ON p.idBoutique = b.idBoutique WHERE
             labelB LIKE '%".$query."%' and (`datePS` LIKE '%".$anneeM."-".$moiM."%' ) GROUP BY b.idBoutique ORDER BY `aPayementBoutique` ASC,pPlusieursMois LIMIT $page_position ,$item_per_page"); 
            $req1->execute()  or die(print_r($req1->errorInfo())); 
