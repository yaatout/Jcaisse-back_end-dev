...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreRetard = @$_POST["nbEntreRetard"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntreRetard!=0) ? $nbEntreRetard : 10; //item to display per page
  ...
  $total_pages = ceil($total_rows/$item_per_page);
      ...
      if ($query =="") {
        $req1 = $bdd->prepare("SELECT * FROM `aaa-payement-salaire` p LEFT JOIN `aaa-boutique` b ON p.idBoutique = b.idBoutique WHERE 
            aPayementBoutique=0 ORDER BY `aPayementBoutique` ASC,pPlusieursMois  LIMIT $page_position ,$item_per_page"); 
        ...
        //Limit our results within a specified range.         
            $req1 = $bdd->prepare("SELECT * FROM `aaa-payement-salaire` p LEFT JOIN `aaa-boutique` b ON p.idBoutique = b.idBoutique WHERE
             labelB LIKE '%".$query."%' and aPayementBoutique=0 ORDER BY `aPayementBoutique` ASC,pPlusieursMois LIMIT $page_position ,$item_per_page"); 
            $req1->execute()  or die(print_r($req1->errorInfo())); 
                                           ...
                                           <td>  
                                            ...
                                            <?php
                                                 if ($payement['aPayementBoutique']==0) { ?>
                                                  ...
                                                  <td>
                                                    <?php   if ($payement['pPlusieursMois']==1) { ?>
                                                          Payé à l'avance
                                                    <?php   } else { ?>
                                                        Déja payé
                                                    <?php   } ?>
