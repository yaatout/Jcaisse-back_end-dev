...
if (isset($_POST['btnActiver'])) {
	...
	$activer=1;
	$sql3="UPDATE `aaa-payement-salaire` set  aPayementBoutique='".$activer."' and datePaiement='".$dateString."' and heurePaiement='".$heureString."' where idBoutique=".$idBoutique;
...
<body>
    ...
    ?>
   	...
   	<?php
       /* $somme=0;
        //$sql1="SELECT idBoutique FROM `aaa-payement-salaire` WHERE `datePS` LIKE '%".$annee."-".$mois."%' and aPayementBoutique=1" ;
		$sql1="SELECT idBoutique FROM `aaa-payement-salaire` WHERE aPayementBoutique=1" ;
            ...
            <div class="tab-pane fade in active" id="LISTEPERSONNEL">
				    <table id="exemple" class="display" border="1" class="table table-bordered table-striped" id="userTable">
						...
						<tbody>
                            ...
                            $somme=0;
							//$sql1="SELECT idBoutique FROM `aaa-payement-salaire` WHERE `datePS` LIKE '%".$annee."-".$mois."%' and aPayementBoutique=1" ;
							$sql1="SELECT * FROM `aaa-payement-salaire` WHERE aPayementBoutique=1  ORDER BY datePS DESC" ;
							//echo $sql1;
                            $res1 = mysql_query($sql1) or die ("etape requête 4".mysql_error());
							...
							?>
									<tr>
										...
										<td> <?php echo  $payement['datePaiement']; ?>  </td>
										...
										<?php 
											 ...
											 if ($payement['aPayementBoutique']==0) { ?>
												...
												<td><!--button type="button" class="btn btn-danger" data-toggle="modal" <?php echo  "data-target=#Desactiver".$payement['idBoutique'] ; ?> >
												Annuler</button-->&nbsp;&nbsp;<button type="button" class="btn btn-success" data-toggle="modal" <?php echo  "data-target=#Facture".$payement['idPS'] ; ?> >
												Facture</button></td>
											<?php }
											 ?>
