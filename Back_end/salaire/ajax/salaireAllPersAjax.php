    ...
    $total_pages = ceil($total_rows/$item_per_page);
      ...
      if ($query =="") {
        //$salaireTotal =recupererPersonnel($bdd, $date, 7,$page_position ,$item_per_page);
          ...
          $stmt = $bdd->prepare("SELECT  c.*,  p.*,  s.* FROM `aaa-contrat` c
            INNER JOIN `aaa-personnel` p ON c.idPersonnel = p.idPersonnel 
            INNER JOIN `aaa-salaire-personnel` s ON c.idContrat = s.idContrat
          ...
          //$salaireTotal =recupererPersonnel($bdd, $date,  7,$page_position ,$item_per_page,$query);        
          $stmt = $bdd->prepare(" SELECT c.*, p.*,  s.* FROM `aaa-contrat` c
          INNER JOIN `aaa-personnel` p ON c.idPersonnel = p.idPersonnel 
          INNER JOIN `aaa-salaire-personnel` s ON c.idContrat = s.idContrat
          WHERE p.profilPersonnel = :profil  AND c.dateDebut <= :date AND c.dateFin >= :date
      ...
      $cpt = 0; 
        ...
        foreach ($salaireTotal as $salaire) {  ?>
            <tr>
                                                        ...
                                                        echo $salaire['matriculePersonnel']; ?>  </td>
                                                        ...
                                                        <td> <?php echo  $salaire['dateDebut'];  ?>  </td>
