...
elseif (isset($_POST['btnEnregistrerPersonnel'])) {
		...
		if ($nomE) {
			 $sql1="insert into `aaa-etapesaccompagnement` (nometape,nbremois,pourcentage,ordre,description,activer) values('".$nomE."',".$duree.",".$pourcentage.",".$ordre.",'".mysql_real_escape_string($description)."',0)";
			 //echo $sql1;
...
if(($nomE==$nomEInitial)&&($duree==$dureeInitial)&&($pourcentage==$pourcentageInitial)&&($ordre==$ordreInitial)&&($description==$descriptionInitial)){
	...
	$sql3="UPDATE `aaa-etapesaccompagnement` set  `nometape`='".$nomE."',nbremois=".$duree.",pourcentage=".$pourcentage.",description='".mysql_real_escape_string($description)."',ordre=".$ordre." where idetape=".$idetape;
	...
	$sql="DELETE FROM `aaa-etapesaccompagnement` WHERE idetape=".$idetape;
...
if (isset($_POST['btnActiver'])) {
	$idetape=$_POST['idetape'];
	$activer=1;
	$sql3="UPDATE `aaa-etapesaccompagnement` set  activer='".$activer."' where idetape=".$idetape;
            ...
            <div class="tab-pane fade in active" id="LISTEPERSONNEL">
				    <table id="exemple" class="display" border="1" class="table table-bordered table-striped" id="userTable">
						...
						<tbody>
							...
							$sql4="SELECT * FROM `aaa-etapesaccompagnement`" ;
