...
$sql="SELECT * from  `aaa-payement-salaire` where idBoutique =".$_SESSION['idBoutique']." order by datePS desc";
//echo $sql;
$res=mysql_query($sql);
	...
	while($tab=mysql_fetch_array($res)){
      ...
      if ($tab['aPayementBoutique']==0){
		  ...
		  if ($dateStringPS == $dateStringMA){
			  ...
			  $rows[] = '<button type="button" onclick="effectue_paiement('.$tab["idPS"].')" id="btn_paiement-'.$tab['idPS'].'" class="btn btn-warning" >
					<img src="images/Orange.png" width="25" height="25"> OMoney
					</button>
                    <button type="button" onclick="effectue_paiementWave('.$tab["idPS"].')"  id="btn_paiementWave-'.$tab['idPS'].'" class="btn btn-success" >
