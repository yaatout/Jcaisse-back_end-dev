...
if(mysql_num_rows($res)){
    ...
    $res1=mysql_query($sql1);
      ...
      if($design=mysql_fetch_array($res1)){
        $rows[] = '<span style="color:#C3C3C3;">'.strtoupper($design['categorie']).'</span>';
      }
      $rows[] = '<span style="color:#C3C3C3;">'.$tab['quantiteStockCourant'].'</span>';
      if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
        $rows[] = '<button type="button" onclick="rtr_Stock_P('.$tab["idStock"].')" id="btn_RetirerStock_P-'.$tab['idStock'].'" class="btn btn-success" >
        <i class="glyphicon glyphicon-transfer"></i> RETIRER
        </button>';
      }else{                                                   
        $rows[] = '<button disabled="true" type="button" onclick="rtr_Stock_P('.$tab["idStock"].')" id="btn_RetirerStock_P-'.$tab['idStock'].'" class="btn btn-success" >
        <i class="glyphicon glyphicon-transfer"></i> RETIRER
        </button>';
      }
...
if(mysql_num_rows($res)){
    ...
    $res1=mysql_query($sql1);
      ...
      if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
        ...
        $rows[] = '<button type="button" disabled="true" onclick="rtr_Stock_P('.$tab["idStock"].')" id="btn_RetirerStock_P-'.$tab['idStock'].'" class="btn btn-success" >
        <i class="glyphicon glyphicon-transfer"></i> RETIRER
        </button>';
      }
      $data[] = $rows;
...
$date2= date('Y-m-d', strtotime('+11 days'));
...
if(mysql_num_rows($res)){
  while($tab=mysql_fetch_array($res)){
    ...
    if($tab["quantiteStockCourant"]!=0){
      ...
      $rows[] = '<span style="color:#ffcc00;">ALERTE</span>';
      $data[] = $rows;
    }
  }
}
