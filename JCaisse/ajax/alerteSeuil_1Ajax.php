...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  if ($query =="") {
    ...
    # code...
      ...
      HAVING (SUM(quantiteStockCourant)/nbreArticleUniteStock)>0 and (SUM(quantiteStockCourant)/nbreArticleUniteStock)<=150");
      $sqlGetStock->execute(array()) or
      die(print_r($sqlGetStock->errorInfo()));
      ...
      GROUP BY idDesignation 
      HAVING (SUM(quantiteStockCourant)>0 and SUM(quantiteStockCourant)<=20)");
      $sqlGetStock->execute(array()) or
      die(print_r($sqlGetStock->errorInfo()));
    }
  } else {
    ...
    //   $res=mysql_query($sql);
      ...
      GROUP BY idDesignation 
      HAVING ((SUM(quantiteStockCourant)/nbreArticleUniteStock)>0 and (SUM(quantiteStockCourant)/nbreArticleUniteStock)<=150) and (designation LIKE '%".$query."%')");
      $sqlGetStock->execute(array()) or
      die(print_r($sqlGetStock->errorInfo()));
      ...
      GROUP BY idDesignation 
      HAVING (SUM(quantiteStockCourant)>0 and SUM(quantiteStockCourant)<=20) and (designation LIKE '%".$query."%')");
      $sqlGetStock->execute(array()) or
      die(print_r($sqlGetStock->errorInfo()));
      ...
      echo  '<tr>';    
        ...
        // if($S_produit==0){
          ...
          if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
            ...
            echo  '<td><span style="color:#501003;">'.$T_produit.'</span></td>';
          }
          else{
            echo  '<td><span style="color:#501003;">'.$S_produit.'</span></td>';
          }
            // echo  '<td><span style="color:orange;">'.$S_produit.'</span></td>';
            echo  '<td><span style="color:#501003;">'.strtoupper($design['designation']).'</span></td>';
            echo  '<td><span style="color:#501003;">'.strtoupper($design['categorie']).'</span></td>';
            echo  '<td><span style="color:#905D53;">REAPPROVISIONNEMENT</span>';
