...
while($design=mysql_fetch_array($res)){
  ...
  $sql2="SELECT * from `".$nomtableEntrepot."` order by idEntrepot desc";
  $res2=mysql_query($sql2);
  $options='<option selected>----------</option>';
  while($entrepot=mysql_fetch_array($res2)){
    $options=$options.'<option value= "'.$entrepot["idEntrepot"].'">'.$entrepot["nomEntrepot"].'</option>';
  }

  $rows = array();			
  $rows[] = strtoupper($design['designation']);	
  $rows[] = strtoupper($design['categorie']);
    $rows[] = '<select class="form-control"  id="entrepot-'.$design['idDesignation'].'">'.$options.'</select>';
  ...
  $rows[] = '<button type="button" onclick="ajt_Transfert('.$design["idDesignation"].')" id="btn_AjtTransfert-'.$design['idDesignation'].'" class="btn btn-success">
