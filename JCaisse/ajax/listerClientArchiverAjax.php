...
if ($operation=="1") {
  $sqlA="SELECT * from `".$nomtableClient."` where archiver=1 order by idClient desc";
  $resA=mysql_query($sqlA);

  while($client=mysql_fetch_array($resA)){
    /*
  ...
  */
  if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
    ...
    $T_solde=$TotalB[0] + $TotalB[0] - $TotalV[0];
  }
  else{
    $sql12="SELECT SUM(apayerPagnet) FROM `".$nomtablePagnet."` where idClient='".$client['idClient']."' AND verrouiller=1 AND (type=0 || type=30 || type=11) ";
    $res12 = mysql_query($sql12) or die ("persoonel requête 2".mysql_error());
...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeArch = @$_POST["nbEntreeArch"];
  $query = @$_POST["query"];
  $item_per_page = ($nbEntreeArch!=0) ? $nbEntreeArch : 10; //item to display per page
  ...
  if ($query =="") {
    ...
    $sql="SELECT * from `".$nomtableClient."` where archiver=1 order by idClient desc";
    $res=mysql_query($sql);
    ...
    //get total number of records from database
      ...
      $sql="SELECT * from `".$nomtableClient."` where archiver=1 and (nom LIKE '%".$query."%' or prenom LIKE '%".$query."%') order by idClient desc";
      $res=mysql_query($sql);
      # code...
  }
    ...
    echo '<br/><div class="container row">
          <a class="col-lg-6 alert alert-info" href="#">LISTE DES CLIENTS ';
            ...
            if ($_SESSION['proprietaire']==1) { 
              echo  ' => Valeur Montant des archives =  '.number_format(($T_archive * $_SESSION['devise']), 2, ',', ' ').' '.$_SESSION['symbole'];
            }
