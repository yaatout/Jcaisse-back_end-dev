...
if ($operation=="1") {
  $sqlD="SELECT * from `".$nomtableClient."` where archiver=0 order by idClient desc";
  $resD=mysql_query($sqlD);
  

  while($client=mysql_fetch_array($resD)){
    /*
  ...
  */
  if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
    ...
    if ($T_solde < 0) {
      ...
      $T_depots=$T_depots+$T_solde;

    }else if ($T_solde > 0) {

      $T_dettes=$T_dettes+$T_solde;
...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeDette = @$_POST["nbEntreeDette"];
  $query = @$_POST["query"];
  $item_per_page = ($nbEntreeDette!=0) ? $nbEntreeDette : 10; //item to display per page
  ...
  echo '
    ...
    <button onclick="clientDette_Excel()">
      <span class="glyphicon glyphicon-download"></span>
      EXCEL
    </button> 
    <button onclick="clientDette_PDF()">
      <span class="glyphicon glyphicon-download"></span>
      PDF
    </button> 
    </center> 
    ...
    echo '<div class="container row">
          <a class="col-lg-6 alert alert-info" href="#">LISTE DES CLIENTS ';
            ...
            if ($_SESSION['proprietaire']==1) { 
              echo  ' => Valeur Montant dettes =  '.number_format(($T_dettes * $_SESSION['devise']), 2, ',', ' ').' '.$_SESSION['symbole'];
            }
    ...
    echo '<tr>';
        ...
        if ($cli['archiver']==0) {
            ...
            if ($cli['activer']==0){
                ...
                echo '<td><button disabled="true" type="button" onclick="activer_Client('.$cli["idClient"].')" id="btn_Client-'.$cli['idClient'].'" class="btn btn-success btn-xs" > Activer</button>&nbsp
    
                <a hidden><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_Client('.$cli["idClient"].','.$i.')" data-toggle="modal"    /></a>&nbsp
    
            <a hidden><img src="images/drop.png" align="middle" alt="supprimer" onclick="spm_Client('.$cli["idClient"].','.$i.')" data-toggle="modal"    /></a>&nbsp</td>';
    
            }
    
            else{
    
                echo '<td><a hidden><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_Client('.$cli["idClient"].','.$i.')" data-toggle="modal"    /></a>&nbsp
    
                <button disabled="true" type="button" onclick="archiver_Client('.$cli["idClient"].')" id="btn_archive-'.$cli['idClient'].'" class="btn btn-warning btn-xs" >Archiver </button>&nbsp
    
                <button disabled="true" type="button" onclick="desactiver_Client('.$cli["idClient"].')" id="btn_Client-'.$cli['idClient'].'" class="btn btn-danger btn-xs" >Desactiver </button>&nbsp
            ...
            if ($cli['activer']==0){
                ...
                <a hidden><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_Client('.$cli["idClient"].','.$i.')" data-toggle="modal"    /></a>&nbsp</td>';
    
            }
    
            else{
    
                echo '<td><button disabled="true" type="button" onclick="archiver_Client('.$cli["idClient"].')" id="btn_archive-'.$cli['idClient'].'" class="btn btn-warning btn-xs" >Archiver </button>&nbsp
      ...
      $(document).ready(function() {    
          ...
          $(function(){
            clientDette_Excel= function ()  {
                ...
                link.setAttribute("download", "dettesClients.csv");
                    ...
                    columns: [
                      {
                        stack:[
                          ...
                          {
                            text: "Liste des dettes du "+dateDebut,
