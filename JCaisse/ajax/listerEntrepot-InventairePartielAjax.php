...
while($stock=mysql_fetch_array($res)){
  ...
  $rows[] = '<input type="number" id="quantite-'.$stock["idDesignation"].'" data-id="'.$idEntrepot.'_'.$stock["idDesignation"].'" class="form-control" width="20%"  min=1 value="" />'; 

  $rows[] = '<button type="button" class="btn btn-success btn_ajtStock" id="btn_InvPStock-'.$stock['idDesignation'].'" onclick="inventairePartielDepot_ET_I('.$idEntrepot.','.$stock["idDesignation"].')"><i class="glyphicon glyphicon-plus">
    </i>CORRIGER
  </button>&nbsp;';
