...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  // if ($query =="") {
    # code...
  /*
  $sql="SELECT * from `".$nomtableInventaire."` WHERE type=10 order by idInventaire desc";
  $res=mysql_query($sql);
  */
...
while($produit=mysql_fetch_array($res)){
  ...
  if ($query =="") { 
    ...
    if ($cb==1) {
      ...
      $sql1="SELECT * FROM `". $nomtableDesignation."` where codeBarreDesignation='".$query."' AND idDesignation='".$produit['idDesignation']."'";
      $res1=mysql_query($sql1);
      $designation=mysql_fetch_array($res1);
      ...
      $sql1="SELECT * FROM `". $nomtableDesignation."` where (designation LIKE '%".$query."%' or codeBarreDesignation LIKE '%".$query."%') AND idDesignation='".$produit['idDesignation']."'";
      $res1=mysql_query($sql1);
      $designation=mysql_fetch_array($res1);
  ...
  $sql2="SELECT * FROM `".$nomtableInventaire."` where idDesignation='".$produit['idDesignation']."' AND type=10 ORDER by idInventaire DESC";
  ...
  if ($designation) {
    # code...
    $rows = array();
    $rows[] = '<span>'.$designation['designation'].'</span>';
    $rows[] = '<span>'.$designation['categorie'].'</span>';
    if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
      $rows[] = '<span>'.$designation['prixSession'].'</span>';
      $rows[] = '<span>'.$designation['prixPublic'].'</span>';
    }
    else if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
    }
    else{
      $rows[] = '<span>'.$designation['prixachat'].'</span>';
      $rows[] = '<span>'.$designation['prix'].'</span>';
    }
    $rows[] = '<span>'.$inventaire['quantiteStockCourant'].'</span>';
    $rows[] = '<span>'.$inventaire['quantite'].'</span>';
    $rows[] = '<span>'.$inventaire['dateInventaire'].'</span>';
