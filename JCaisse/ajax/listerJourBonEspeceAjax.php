...
WHERE l.classe=6 &&  p.datepagej ='".$dateJour."' && (p.type=0 || p.type=30)  ORDER BY c.idClient DESC";
...
while($client=mysql_fetch_array($res)){
  ...
  $sqlS="SELECT DISTINCT p.idPagnet
  ...
  $rows[] = number_format(($T_Bonp * $_SESSION['devise']), 0, ',', ' ');
