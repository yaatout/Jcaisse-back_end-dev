...
if ($_SESSION['tampon'] == 1 && $_SESSION['page'] == 'vente') {
  require('listerJourPersonnelAjax-tampon.php');
} else {
  $date = new DateTime();
...
*/
  ...
  while ($personnel = mysql_fetch_array($res)) {
    ...
    $sqlP = mysql_query("SELECT EXISTS (SELECT * FROM `" . $nomtablePagnet . "` WHERE idUser=" . $personnel["idutilisateur"] . " && datepagej='" . $dateJour . "') AS panier_exist");
    ...
    if (($reqP['panier_exist'] == true) || ($reqV['versement_exist'] == true)) {
      ...
      if (($_SESSION['mutuelle'] == 1) && ($_SESSION['type'] == "Pharmacie") && ($_SESSION['categorie'] == "Detaillant")) {
        ...
        if ($_SESSION['compte'] == 1) {
          ...
          WHERE l.classe=0  && p.idUser='" . $personnel["idutilisateur"] . "'  && p.idClient=0  && p.verrouiller=1 && p.idCompte<>0 && p.idCompte<>1 && p.idCompte<>2 && p.idCompte<>3 && p.idCompte<>1000 && p.datepagej ='" . $dateJour . "'  && p.type=0  ORDER BY p.idPagnet DESC";
        ...
        FROM  `" . $nomtablePagnet . "` p WHERE p.idClient=0  && p.verrouiller=1 && p.idUser='" . $personnel["idutilisateur"] . "' && p.idCompte=1000 && p.datepagej ='" . $dateJour . "' && p.type=0 ";
      ...
      if ($_SESSION['compte'] == 1) {
        ...
        WHERE l.classe=0  && p.idUser='" . $personnel["idutilisateur"] . "' && p.idCompte<>0 && p.idCompte<>1 && p.idCompte<>2 && p.idCompte<>3 && p.idCompte<>1000 && p.idClient=0  && p.verrouiller=1 && p.datepagej ='" . $dateJour . "'  && p.type=1  ORDER BY p.idPagnet DESC";
      ...
      if ($_SESSION['compte'] == 1) {
        ...
        $sqlP5Mobile = "SELECT SUM(montant) FROM `" . $nomtableVersement . "` where idClient!=0 && idUser='" . $personnel["idutilisateur"] . "' && (dateVersement  ='" . $dateJour . "' || dateVersement  ='" . $dateAnnee . "') && (idCompte<>0 && idCompte<>1 && idCompte<>2 && idCompte<>3 && idCompte<>1000) ORDER BY idVersement DESC";
