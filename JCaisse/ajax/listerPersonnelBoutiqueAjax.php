...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntree = @$_POST["nbEntreeUserBoutique"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  ...
  if ($query =="") {
    # code...
    $sql="SELECT a.idutilisateur,a.proprietaire,a.gerant,a.gestionnaire,a.caissier,a.vendeur,a.activer FROM `aaa-acces` a
    LEFT JOIN `aaa-utilisateur` u ON u.idutilisateur = a.idutilisateur 
    WHERE a.idBoutique =".$_SESSION['idBoutique']." AND visible=0  ORDER BY u.idutilisateur DESC ";
    ...
    $sql="SELECT a.idutilisateur,a.proprietaire,a.gerant,a.gestionnaire,a.caissier,a.vendeur,a.activer FROM `aaa-acces` a
    LEFT JOIN `aaa-utilisateur` u ON u.idutilisateur = a.idutilisateur 
    WHERE a.idBoutique =".$_SESSION['idBoutique']." AND visible=0 AND (u.nom LIKE '%".$query."%' or u.prenom LIKE '%".$query."%' or u.adresse LIKE '%".$query."%') ORDER BY u.idutilisateur DESC ";
    $res = mysql_query($sql) or die ("persoonel requête 4".mysql_error());
  }
  ...
  foreach ($personnels as $acces) {
    ...
    echo '<tr>';
      ...
      if($acces['vendeur']==1){
        echo  '<td><input disabled=""  class="form-check-input" type="checkbox" checked=""  name="vendeur"></td>';
      }
      else{
        echo  '<td><input disabled=""  class="form-check-input" type="checkbox" name="vendeur"></td>';
      }
      if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
      ...
      if ($acces['activer']==0){
        echo  '<td><a ><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_UserBoutique('.$utilisateur["idutilisateur"].')" data-toggle="modal"    /></a>&nbsp
        <button type="button" onclick="activer_UserBoutique('.$utilisateur["idutilisateur"].')" id="btn_Boutique-'.$utilisateur['idutilisateur'].'" class="btn btn-success btn-xs" > Activer</button>&nbsp
        <a ><img src="images/drop.png" align="middle" alt="modifier" onclick="spm_UserBoutique('.$utilisateur["idutilisateur"].')" data-toggle="modal"    /></a>
        </td>';
      }
      else{
        echo  '<td><a ><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_UserBoutique('.$utilisateur["idutilisateur"].')" data-toggle="modal"    /></a>&nbsp
        <button type="button" onclick="desactiver_UserBoutique('.$utilisateur["idutilisateur"].')" id="btn_Boutique-'.$utilisateur['idutilisateur'].'" class="btn btn-danger btn-xs" >Desactiver </button>&nbsp
        </td>';
      }
    echo '</tr>';
