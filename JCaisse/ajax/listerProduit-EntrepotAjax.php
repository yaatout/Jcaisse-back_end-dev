...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeEt = @$_POST["nbEntreeEt"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= $nbEntreeEt; //item to display per page
  ...
  while($design=mysql_fetch_array($res)){ //fetch values
    ...
    if ($i==1) {
      ...
      echo  '<td><span style="color:blue;">'.number_format($design['prixuniteStock'], 0, ',', ' ').'</span></td>';
      echo  '<td><span style="color:blue;">'.number_format($design['prixachat'], 0, ',', ' ').'</span></td>';
      ...
      echo  '<td>'.number_format($design['prixuniteStock'], 0, ',', ' ').'</td>';
      echo  '<td>'.number_format($design['prixachat'], 0, ',', ' ').'</td>';
    }		
    ...
    if($_SESSION['proprietaire']==1){
      if($design["codeBarreDesignation"]!=null){
        ...
        <a><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_Designation_ET('.$design["idDesignation"].','.$i.')" id="" data-toggle="modal" data-target="#imgmodifier'.$design["idDesignation"].'" /></a>&nbsp;
        <a><img src="images/drop.png" align="middle" alt="supprimer" onclick="spm_Designation_ET('.$design["idDesignation"].')"  data-toggle="modal" data-target="#imgsup'.$design["idDesignation"].'" /></a></td>';   
      }
      else{
      ...
      if($design["codeBarreDesignation"]!=null){
        echo  '<td><a onclick="mdf_CodeBarreDesign_ET('.$design["idDesignation"].','.$i.')" data-toggle="modal" data-target="#codeBP'.$design["idDesignation"].'"><span class="glyphicon glyphicon-barcode"></span></a>&nbsp;
        <a><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_Designation_ET('.$design["idDesignation"].','.$i.')" id="" data-toggle="modal" data-target="#imgmodifier'.$design["idDesignation"].'" /></a>&nbsp;</td>';  
      }
      else{
        echo  '<td><a onclick="mdf_CodeBarreDesign_ET('.$design["idDesignation"].','.$i.')" data-toggle="modal" data-target="#codeBP'.$design["idDesignation"].'"><span style="color:red" class="glyphicon glyphicon-barcode"></span></a>&nbsp;
        <a><img src="images/edit.png" align="middle" alt="modifier" onclick="mdf_Designation_ET('.$design["idDesignation"].','.$i.')" id="" data-toggle="modal" data-target="#imgmodifier'.$design["idDesignation"].'" /></a>&nbsp;</td>'; 
      }
    }
    echo '</tr>';
...
if ($operation=="2") {
  ...
  if ($tri==0) {
    if ($query =="") {
      ...
      // $results = $bddV->prepare("SELECT * from `".$nomtableDesignation."` WHERE (classe = 0 or classe = 1) and designation LIKE '%".$query."%' ORDER BY designation ASC LIMIT $page_position, $item_per_page");
      // $results->execute(); //Execute prepared Query
      $sql1="select * from `".$nomtableDesignation."` where classe=0 and (designation LIKE '%".$query."%' or codeBarreDesignation LIKE '%".$query."%') order by idDesignation DESC LIMIT 1";
      $res1=mysql_query($sql1);
    }
