...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  if ($query =="") {
    ...
    WHERE l.classe = 5 && p.idClient=0  && p.verrouiller=1 && p.type=0 && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) BETWEEN '".$debut."' AND '".$fin."' ORDER BY p.idPagnet DESC ";
  ...
  while($stock=mysql_fetch_array($res)){
    ...
    INNER JOIN `".$nomtablePagnet."` p ON p.idPagnet = l.idPagnet
    where  l.classe = 5 && l.numligne='".$stock['numligne']."' && p.idClient=0 &&  p.verrouiller=1 && p.type=0 &&  CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) BETWEEN '".$debut."' AND '".$fin."'  ";
