...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeBonParPanier = @$_POST["nbEntreeBonParPanier"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntreeBonParPanier!=0) ? $nbEntreeBonParPanier : 10; //item to display per page
  ...
  if ($query =="") {
    ...
    where p.idClient!=0 &&  p.verrouiller=1 && p.type=0 && l.classe=0  && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) 
    ...
    INNER JOIN `".$nomtablePagnet."` p ON p.idPagnet = l.idPagnet
    where p.idClient!=0 &&  p.verrouiller=1 && p.type=0 && l.classe=0  && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) 
    BETWEEN '".$debut."' AND '".$fin."' and (p.datepagej LIKE '%".$query."%' OR p.totalp LIKE '%".$query."%')";
    $res=mysql_query($sqlS) or die ("select stock impossible =>".mysql_error()); 
