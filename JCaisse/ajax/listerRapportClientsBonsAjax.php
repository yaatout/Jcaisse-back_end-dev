...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeClientBon = @$_POST["nbEntreeClientBon"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntreeClientBon!=0) ? $nbEntreeClientBon : 10; //item to display per page
  ...
  if ($query =="") {
    ...
    INNER JOIN `".$nomtableClient."` c ON c.idClient = p.idClient
    
    WHERE l.classe = 6 && p.idClient !=0  && p.verrouiller=1 && p.type=0 && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) BETWEEN '".$debut."' AND '".$fin."' ORDER BY p.idPagnet DESC ";
    ...
    WHERE l.classe = 6 && p.idClient !=0  && p.verrouiller=1 && p.type=0 && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2))
      BETWEEN '".$debut."' AND '".$fin."' and (c.nom LIKE '%".$query."%' or c.prenom LIKE '%".$query."%') ORDER BY p.idPagnet DESC ";
    ...
    // INNER JOIN `".$nomtableDesignation."` d ON d.idDesignation = l.idDesignation
    // INNER JOIN `".$nomtableClient."` c ON c.idClient = p.idClient
    // WHERE (p.designation LIKE '%".$query."%') and l.classe = 6  && p.idClient !=0  && p.verrouiller=1 && p.type=0 && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) BETWEEN '".$debut."' AND '".$fin."' ORDER BY p.idPagnet DESC ";
    ...
    // INNER JOIN `".$nomtablePagnet."` p ON p.idPagnet = l.idPagnet
    // INNER JOIN `".$nomtableDesignation."` d ON d.idDesignation = l.idDesignation
    // WHERE WHERE (d.designation LIKE '%".$query."%') and l.classe = 6 && p.idClient !=0  && p.verrouiller=1 && p.type=0 && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) BETWEEN '".$debut."' AND '".$fin."' ORDER BY p.idPagnet DESC ";
    // $res = mysql_query($sql) or die ("persoonel requête 2".mysql_error());
  ...
  while($stock=mysql_fetch_array($res)){
    ...
    $rows = array();
      ...
      $rows[] = '<span style="color:blue;">'.$stock['apayerPagnet'].'</span>';

      $rows[] = $stock['datepagej'].' <span style="color:blue;">'.$stock['heurePagnet'].' </span>';
