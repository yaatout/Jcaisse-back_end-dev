...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeVF = @$_POST["nbEntreeVF"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntreeVF!=0) ? $nbEntreeVF : 10; //item to display per page
  ...
  if ($query =="") {
    ...
    INNER JOIN `".$nomtableFournisseur."` f ON f.idFournisseur = v.idFournisseur
    WHERE v.idFournisseur!=0 && v.dateVersement BETWEEN '".$debut."' AND '".$fin."' ORDER BY v.idVersement DESC ";
    ...
    FROM  `".$nomtableVersement."`v 
    INNER JOIN `".$nomtableFournisseur."` f ON f.idFournisseur = v.idFournisseur
    WHERE (f.nomFournisseur LIKE '%".$query."%') and v.idFournisseur!=0 && v.dateVersement BETWEEN '".$debut."' AND '".$fin."' ORDER BY v.idVersement DESC ";
    $res = mysql_query($sql) or die ("persoonel requête 2".mysql_error());
  }
  ...
  while($stock=mysql_fetch_array($res)){
    ...
    if($i==1){
      ...
      $rows[] = '<span style="color:blue;">'.number_format($stock['montant'], 0, ',', ' ').'</span>';
      if($stock['paiement']!=null){
        $rows[] = '<span style="color:blue;">'.strtoupper($stock['paiement']).'</span>';
      }
      else{
        $rows[] = 'ESPECES';
      }
      $rows[] = '<span style="color:blue;">'.$stock['dateVersement'].' '.$stock['heureVersement'].' </span>';
    }
    else if($dateString2==$stock['dateVersement']){
      $rows[] = $i;
      $rows[] = '<span style="color:blue;">'.strtoupper($stock['nomFournisseur']);
      $rows[] = '<span style="color:#ffcc00;">'.number_format($stock['montant'], 0, ',', ' ').'</span>';
      if($stock['paiement']!=null){
      ...
      $rows[] = '<span style="color:#ffcc00;">'.$stock['dateVersement'].' '.$stock['heureVersement'].' </span>';
    }
    else{
      $rows[] = $i;
      $rows[] = '<span style="color:blue;">'.strtoupper($stock['nomFournisseur']);
      $rows[] = number_format($stock['montant'], 0, ',', ' ');
      if($stock['paiement']!=null){
        $rows[] = strtoupper($stock['paiement']);
      }
      else{
        $rows[] = 'ESPECES';
      }
      $rows[] = $stock['dateVersement'].' '.$stock['heureVersement'];
    }
    ...
    echo '<table class="table table-striped contents"  border="1">';
        echo '<thead>
                <tr>
                  <th>ORDRE</th>
                  <th style="width: 18%;">FOURNISSEURS</th>
                  <th>MONTANT</th>
                  <th>PAIEMENT</th>
