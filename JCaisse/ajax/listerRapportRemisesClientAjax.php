...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeRemiseClient = @$_POST["nbEntreeRemiseClient"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntreeRemiseClient!=0) ? $nbEntreeRemiseClient : 10; //item to display per page
  ...
  if ($query =="") {
    ...
    FROM  `".$nomtablePagnet."` p
    INNER JOIN `".$nomtableClient."` c ON c.idClient = p.idClient
    WHERE (c.nom LIKE '%".$query."%' or c.prenom LIKE '%".$query."%') and p.idClient!=0 && p.remise!=0 && p.verrouiller=1 && p.type=0 && CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) BETWEEN '".$debut."' AND '".$fin."' ORDER BY p.idPagnet DESC ";
    $res = mysql_query($sql) or die ("persoonel requête 2".mysql_error());
  }
  ...
  while($p=mysql_fetch_array($res)){
    ...
    if (empty($l)) {
      ...
      if($i==1){
        ...
        $rows[] = '<span style="color:blue;">'.strtoupper($p['prenom']).' '.strtoupper($p['nom']).'</span>';
      }
        ...
        $rows[] = '<span style="color:#ffcc00;">'.strtoupper($p['prenom']).' '.strtoupper($p['nom']).'</span>';
      }
      else{
        $rows[] = $i;
        ...
        $rows[] = $p['remise'];
        $rows[] = $p['apayerPagnet'];
        $rows[] = strtoupper($p['prenom']).' '.strtoupper($p['nom']);
      }
    // }
