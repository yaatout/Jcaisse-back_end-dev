...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeRetireProd = @$_POST["nbEntreeRetireProd"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page 		= ($nbEntreeRetireProd!=0) ? $nbEntreeRetireProd : 10; //item to display per page
  ...
  if ($query =="") {
    ...
    WHERE i.TYPE=1 AND i.dateInventaire BETWEEN '".$debut."' AND '".$fin."' order by i.idInventaire desc";
    ...
    if ($cb==1) {
      ...
      $sql="SELECT *from `".$nomtableInventaire."` i
      INNER JOIN `".$nomtableDesignation."` d ON d.idDesignation = i.idDesignation
      WHERE i.TYPE=1 AND i.dateInventaire BETWEEN '".$debut."' AND '".$fin."' and (d.codeBarreDesignation LIKE '%".$query."%') order by i.idInventaire desc";
      $res = mysql_query($sql) or die ("persoonel requête 2".mysql_error());
    }else {
      //Limit our results within a specified range. 
      $sql="SELECT *from `".$nomtableInventaire."` i
      INNER JOIN `".$nomtableDesignation."` d ON d.idDesignation = i.idDesignation
      WHERE i.TYPE=1 AND i.dateInventaire BETWEEN '".$debut."' AND '".$fin."' and (d.designation LIKE '%".$query."%' or d.codeBarreDesignation LIKE '%".$query."%') order by i.idInventaire desc";
      $res = mysql_query($sql) or die ("persoonel requête 2".mysql_error());
    }
