...
// $sql="SELECT * FROM `".$nomtableStock."` s
  ...
  //   $rows[] = '<button type="button" class="btn btn-success btn_ajtStock" id="btn_InvStock-'.$stock['idDesignation'].'" onclick="inventairePermanentDepot('.$stock["idDesignation"].')"><i class="glyphicon glyphicon-plus">
  //     </i>CORRIGER
  //   </button>&nbsp;';
...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  $nbEntreeInvJour = @$_POST["nbEntreeInvJour"];
  $idEntrepotInventaire = @$_POST["idEntrepotInventaire"];
  $query = @$_POST["query"];
  $cb = @$_POST["cb"];
  $item_per_page = ($nbEntreeInvJour!=0) ? $nbEntreeInvJour : 10; //item to display per page
  ...
  if ($idEntrepotInventaire != '') {
    ...
    if ($query =="") {
      ...
      if ($cb==1) {
        ...
        $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableEntrepotStock."` s, `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
        WHERE s.idDesignation = d.idDesignation and d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and s.idEntrepot='".$idEntrepotInventaire."'and p.datepagej='".$dateString."' and d.classe=0 and d.codeBarreDesignation='".$query."' ORDER BY d.designation";
        $res=mysql_query($sql);
      }else {
        //Limit our results within a specified range. 
        $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableEntrepotStock."` s, `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
        WHERE s.idDesignation = d.idDesignation and d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and s.idEntrepot='".$idEntrepotInventaire."' and p.datepagej='".$dateString."' and d.classe=0 and (d.designation LIKE '%".$query."%' or d.codeBarreDesignation LIKE '%".$query."%') ORDER BY d.designation";
        $res=mysql_query($sql);
      }
    }
  }else {
    ...
    if ($query =="") {
      ...
      if ($cb==1) {
        //Limit our results within a specified range. 
        $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
            WHERE d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 and d.codeBarreDesignation='".$query."' ORDER BY d.designation";
        $res=mysql_query($sql);
      }else {
        //Limit our results within a specified range. 
        $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
            WHERE d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 and (d.designation LIKE '%".$query."%' or d.codeBarreDesignation LIKE '%".$query."%') ORDER BY d.designation";
        $res=mysql_query($sql);
      }
    }
  }
  ...
  while($stock=mysql_fetch_array($res)){
    if (in_array($stock['idDesignation'], $tabIdDesigantion)) {
      ...
      if ($idEntrepotInventaire != '') {
        ...
        if (($S_stock[0]/$designation['nbreArticleUniteStock'] > 0) && ($S_stock[0]/$designation['nbreArticleUniteStock'] <= 20)) {
          # code...
  ...
  foreach ($produits as $stock) {
    ...
    if ($idEntrepotInventaire != '') {
      ...
      echo '<tr>';      
        ...
        echo '<td><input type="number" id="quantite-'.$stock["idDesignation"].'" class="form-control" width="20%"  min=1 value="" /></td>'; 

        echo '<td><button type="button" class="btn btn-success btn_ajtStock" id="btn_InvStock-'.$stock['idDesignation'].'" onclick="inventaireDepot_ET('.$idEntrepotInventaire.','.$stock["idDesignation"].')"><i class="glyphicon glyphicon-plus">
          </i>CORRIGER
        </button></td>&nbsp;';
      echo '</tr>';
      ...
      echo '<tr>';      
        ...
        echo '<td><button type="button" class="btn btn-success btn_ajtStock" id="btn_InvStock-'.$stock['idDesignation'].'" onclick="inventairePermanentDepot('.$stock["idDesignation"].')"><i class="glyphicon glyphicon-plus">
          </i>CORRIGER
        </button></td>&nbsp;';
      echo '</tr>';
    }
...
/*****  Tri **/
  ...
  if ($idEntrepotInventaire != '') {
    if ($query =="") {
      ...
      $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableEntrepotStock."` s, `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
            WHERE s.idDesignation = d.idDesignation and d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 ORDER BY d.designation";
      $res=mysql_query($sql);
    } else {
      # code...
        //Limit our results within a specified range. 
      $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableEntrepotStock."` s, `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
          WHERE s.idDesignation = d.idDesignation and d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 and (d.designation LIKE '%".$query."%' or d.codeBarreDesignation LIKE '%".$query."%') ORDER BY d.designation";
      $res=mysql_query($sql);
    }
  } else {
  ...
  while($stock=mysql_fetch_array($res)){
    if (in_array($stock['idDesignation'], $tabIdDesigantion)) {
      ...
      if ($idEntrepotInventaire != '') {
        ...
        if (($S_stock[0]/$designation['nbreArticleUniteStock'] > 0) && ($S_stock[0]/$stock['nbreArticleUniteStock'] <= 20)) {
          # code...
          $tabIdDesigantion[]=$stock['idDesignation'];
          // $tabIdStock[]=$stock['idStock'];
          $produits[]=$stock;
...
/****** Rechercher *****/
  ...
  $nbEntreeInvJour = @$_POST["nbEntreeInvJour"];
  $query = @$_POST["query"];
  $item_per_page 		= $nbEntreeInvJour; //item to display per page
  $page_number 		= 0; //page number
