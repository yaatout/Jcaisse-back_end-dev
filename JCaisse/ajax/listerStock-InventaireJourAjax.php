...
// $sql="SELECT * FROM `".$nomtableStock."` s
  ...
  //   $rows[] = '<button type="button" class="btn btn-success btn_ajtStock" id="btn_InvStock-'.$stock['idDesignation'].'" onclick="inventaireDepot('.$stock["idDesignation"].')"><i class="glyphicon glyphicon-plus">
  //     </i>CORRIGER
  //   </button>&nbsp;';
...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  if ($query =="") {
    ...
    $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
          WHERE d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 ORDER BY p.datepagej DESC";
    $res=mysql_query($sql);
  } else {
    # code... 
    if ($cb==1) {
      ...
      $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
          WHERE d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 and d.codeBarreDesignation='".$query."' ORDER BY p.datepagej DESC";
      $res=mysql_query($sql);
    }else {
      //Limit our results within a specified range. 
      $sql="SELECT DISTINCT d.idDesignation FROM `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
          WHERE d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 and (d.designation LIKE '%".$query."%' or d.codeBarreDesignation LIKE '%".$query."%') ORDER BY p.datepagej DESC";
      $res=mysql_query($sql);
    }
  }
...
/*****  Tri **/
  ...
  if ($query =="") {
    ...
    $sql="SELECT DISTINCT d.idDesignation, d.designation FROM `".$nomtableDesignation."` d, `".$nomtableLigne."` l, `".$nomtablePagnet."` p
          WHERE d.idDesignation = l.idDesignation and l.idPagnet=p.idPagnet and p.datepagej='".$dateString."' and d.classe=0 ORDER BY p.datepagej DESC";
    $res=mysql_query($sql);
  } else {
    # code...
