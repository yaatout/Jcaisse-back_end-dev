...
where m.verrouiller=1 && m.type=0 && l.idDesignation='".$idDesignation."'  ORDER BY m.idMutuellePagnet DESC ";
...
while($stock=mysql_fetch_array($res)){
  ...
  if($i==1){
    ...
    $rows[] = '<span style="color:blue;">#'.$stock['idMutuellePagnet'].'</span>';
    $rows[] = '<span style="color:blue;">'.$stock['adherant'].'</span>';
  }
    ...
    $rows[] = '<span style="color:#ffcc00;">#'.$stock['idMutuellePagnet'].'</span>';
    $rows[] = '<span style="color:#ffcc00;">'.$stock['adherant'].'</span>';
  }
  else{
    $rows[] = $stock['dateJour'];
