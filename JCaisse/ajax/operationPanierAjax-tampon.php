...
// var_dump($nomtablePagnetTampon);
// var_dump($nomtableLigneTampon);
// die();
...
if (isset($_POST['btnSavePagnetVente'])) {
    ...
    if(mysql_num_rows($res0)){
        if($jour > 4){
            ...
            $query = mysql_query("SELECT idPagnet FROM `".$nomtablePagnetTampon."` where  iduser='".$_SESSION['iduser']."' && type=0 && idClient=0 && datepagej ='".$dateString2."' && verrouiller=0");
            ...
            if(mysql_num_rows($res1)){
                ...
                if(mysql_num_rows($res2)){
                    if($nbre_fois<2){
                        $sql6="insert into `".$nomtablePagnetTampon."` (datepagej,heurePagnet,iduser,type,totalp,remise,apayerPagnet,restourne,versement,verrouiller,idClient) values('".$dateString2."','".$heureString."',".$_SESSION['iduser'].",0,0,0,0,0,0,0,0)";
...
/**Debut Button Terminer Pagnet**/
    ...
    if (isset($_POST['remise']) || isset($_POST['versement']) || isset($_POST['compte'])) {
        ...
        $sql="SELECT * FROM `".$nomtablePagnetTampon."` where idPagnet=".$idPagnet." ";
        ...
        $query = mysql_query("SELECT * FROM `".$nomtableLigneTampon."` where idPagnet=".$idPagnet." ");
        $nbre_fois=mysql_num_rows($query);
                ...
                if($difference>=0){
                    if($pagnet['type']==0 || $pagnet['type']==30){
                        ...
                        if($_SESSION['Pays']=='Canada'){ 
                            if ($dejaTerminer == 0) {
                                # code...
                                $sql3="UPDATE `".$nomtablePagnetTampon."` set verrouiller='1', remise=".$remise.",totalp=".$totalp.",apayerPagnet=".$apayerPagnet.",apayerPagnetTvaP=".$totalTvaP.",apayerPagnetTvaR=".$totalTvaR.",versement=".$versement.",restourne=".$monaie.", datepagej='".$dateString2."', heurePagnet='".$heureString."', dejaTerminer=1 where idPagnet=".$idPagnet;
                                $res3=@mysql_query($sql3) or die ("mise à jour verouillage  impossible".mysql_error());
                            } else {
                                # code...
                                $sql3="UPDATE `".$nomtablePagnetTampon."` set verrouiller='1', remise=".$remise.",totalp=".$totalp.",apayerPagnet=".$apayerPagnet.",apayerPagnetTvaP=".$totalTvaP.",apayerPagnetTvaR=".$totalTvaR.",versement=".$versement.",restourne=".$monaie." where idPagnet=".$idPagnet;
                                $res3=@mysql_query($sql3) or die ("mise à jour verouillage  impossible".mysql_error());
                            }
                            ...
                            if ($dejaTerminer == 0) {
                                # code...
                                $sql3="UPDATE `".$nomtablePagnetTampon."` set verrouiller='1', remise=".$remise.",totalp=".$totalp.",apayerPagnet=".$apayerPagnet.",versement=".$versement.",restourne=".$monaie.", datepagej='".$dateString2."', heurePagnet='".$heureString."', dejaTerminer=1 where idPagnet=".$idPagnet;
                                $res3=@mysql_query($sql3) or die ("mise à jour verouillage  impossible".mysql_error());
                            } else {
                                # code...
                                $sql3="UPDATE `".$nomtablePagnetTampon."` set verrouiller='1', remise=".$remise.",totalp=".$totalp.",apayerPagnet=".$apayerPagnet.",versement=".$versement.",restourne=".$monaie." where idPagnet=".$idPagnet;
                                $res3=@mysql_query($sql3) or die ("mise à jour verouillage  impossible".mysql_error());
                            }
                        ...
                        if ($_SESSION['compte']==1) {
                            if(isset($_POST['compte'])) {
                                ...
                                $sqlL="SELECT * FROM `".$nomtableLigneTampon."` where idPagnet=".$idPagnet." ORDER BY numLigne LIMIT 1 ";
                                $resL = mysql_query($sqlL) or die ("persoonel requête 2".mysql_error());
                                ...
                                if ($lignes['classe'] == '2') {
                                    ...
                                    $sql8="UPDATE `".$nomtablePagnetTampon."` set idCompte=".$idCompte." where  idPagnet=".$idPagnet;
                                    ...
                                    if(isset($_POST['avanceInput']) && $_POST['avanceInput'] > 0 && $idCompte == '2'){
                                        ...
                                        $sql30="UPDATE `".$nomtablePagnetTampon."` set avance=".$montant." where idPagnet=".$idPagnet;
                                    ...
                                    if ($idCompte == '2') {
                                        ...
                                        $sql18="SELECT SUM(apayerPagnet-avance) FROM `".$nomtablePagnetTampon."` where verrouiller='1' && idClient=".$idClient." && (type=0 || type=30) ";
                                    ...
                                    if ($idCompte == '1000') {
                                        ...
                                        $sql8="UPDATE `".$nomtablePagnetTampon."`  set  idCompte=".$idCompte.", idsCpteMultiple='".$idsCpteMultiple."'  where  idPagnet=".$idPagnet;
                                        $res8=@mysql_query($sql8) or die ("mise à jour pour activer ou pas ".mysql_error());
...
/**Debut Button Annuler Pagnet**/
    ...
    while ($ligne=mysql_fetch_assoc($resL)){
        ...
        $sql3="DELETE FROM `".$nomtableLigneTampon."` where idPagnet=".$idPagnet." ";
    ...
    $sqlR="DELETE FROM `".$nomtablePagnetTampon."` where idPagnet=".$idPagnet;
...
/**Debut Button Retourner Pagnet**/
    ...
    if($pagnet['type']==0 || $pagnet['type']==30){
        ...
        if ($pagnet['synchronise']==1 && $pagnet['idPagnet']!=0) {
        
            $idPagnetTampon=$pagnet['idPagnet'];
            $qtyVendu=0;
            
            $sqlP="SELECT * FROM `".$nomtablePagnet."` where idPagnetTampon=".$idPagnetTampon." ";
            $resP = mysql_query($sqlP) or die ("persoonel requête 2".mysql_error());
            $pagnet = mysql_fetch_assoc($resP) ;
...
/**Debut Button Annuler Ligne d'un Pagnet**/
    ...
    //on fait la suppression de cette ligne dans la table ligne
    $sql3="DELETE FROM `".$nomtableLigneTampon."` where numligne=".$numligne;
    $res3=@mysql_query($sql3) or die ("mise à jour client  impossible".mysql_error());
...
/**Debut Button Retourner Ligne d'un Pagnet**/
    ...
    $sqlL="SELECT * FROM `".$nomtableLigneTampon."` where numligne=".$numligne."";
    $resL = mysql_query($sqlL) or die ("persoonel requête 2".mysql_error());
    $ligne = mysql_fetch_assoc($resL) ;
    ...
    $sqlT="SELECT SUM(prixtotal) FROM `".$nomtableLigneTampon."` where idPagnet='".$idPagnet."' && numligne<>".$numligne."";
    $resT = mysql_query($sqlT) or die ("persoonel requête 2".mysql_error());
    $TotalT = mysql_fetch_array($resT) ;
    ...
    if($difference >= 0 && $ligne['quantite'] >= $quantite){
        if($pagnet['type']==0 || $pagnet['type']==30){
                ...
                $designation = mysql_fetch_assoc($resS) ;
                    if(mysql_num_rows($resS)){
                        if (($ligne['unitevente']!="Article")&&($ligne['unitevente']!="article")) {
                            ...
                            if($quantite==0){
                                $sqlD="DELETE FROM `".$nomtableLigneTampon."` where numligne=".$numligne;
                                $resD=@mysql_query($sqlD) or die ("mise à jour client  impossible".mysql_error());
                            }
                            else{
                                $prixTotal=$quantite * $ligne['prixunitevente'];
                                $sql3="UPDATE `".$nomtableLigneTampon."` set quantite=".$quantite.", prixtotal=".$prixTotal."  where numligne=".$ligne['numligne']."  ";
                                $res3=@mysql_query($sql3) or die ("mise à jour client  impossible".mysql_error());
                            }
            ...
            $sqlTP="SELECT SUM(prixtotal) FROM `".$nomtableLigneTampon."` where idPagnet=".$idPagnet." ";
            $resTP = mysql_query($sqlTP) or die ("persoonel requête 2".mysql_error());
            $TotalTP = mysql_fetch_array($resTP) ;
            ...
            if($TotalTP[0]!=null){
                ...
                $sql16="update `".$nomtablePagnetTampon."` set totalp=".$TotalTP[0].",apayerPagnet=".$apayerPagnet.",
    ...
    if ($pagnet['synchronise']==1 && $pagnet['idPagnet']!=0 && $msg_info==="1" && $numligne!=0) {
        ...
        if($quantite==0){
            $sqlD="DELETE FROM `".$nomtableLigne."` where numligneTampon=".$numligne;
            $resD=@mysql_query($sqlD) or die ("mise à jour client  impossible".mysql_error());
        }
        else{
            
            $prixTotal=$quantite * $ligne['prixunitevente'];
            $sql3="UPDATE `".$nomtableLigneTampon."` set quantite=".$quantite.", prixtotal=".$prixTotal."  where numligneTampon=".$ligne['numligne']."  ";
            $res3=@mysql_query($sql3) or die ("mise à jour client  impossible".mysql_error());
        }
...
/**Debut Button ajout info supp**/
    ...
    $sqlS="UPDATE `".$nomtableLigneTampon."` set numeroChassis='".$numeroChassis."', numeroMoteur='".$numeroMoteur."', couleur='".$couleur."'  where numligne=".$numLigne;
...
/**Debut Button somme quantite d'un produit par periode**/
    ...
    $sql="SELECT SUM(l.quantite) as sumQtyByPeriod FROM  `".$nomtableLigneTampon."` l
...
//     $sql3="UPDATE `".$nomtablePagnetTampon."` set numContainer='".$numContainerChanged."' where idPagnet=".$idPagnetContainer;
...
// }
    ...
    $sql3="UPDATE `".$nomtablePagnetTampon."` set numContainer='".$idContainer."' where idPagnet=".$idPagnetContainer;
    ...
    try {
        ...
        if ($_SESSION['compte']==1) {
            ...
            $req4 = $bdd->prepare("INSERT INTO `".$nomtablePagnetTampon."` (datepagej,heurePagnet,iduser,type,totalp,apayerPagnet,verrouiller,idCompte,idClient,numContainer,nbPcsInContainer)
            ...
            $req4 = $bdd->prepare("INSERT INTO `".$nomtablePagnetTampon."` (datepagej,heurePagnet,iduser,type,totalp,apayerPagnet,verrouiller,idClient,numContainer,nbPcsInContainer)
        ...
        if ($qty_cbm!=0) {
            ...
            $preparedStatementCbm = $bdd->prepare(
                "insert into `".$nomtableLigneTampon."` (designation, idDesignation, unitevente, prixunitevente, quantite, prixtotal, idPagnet, classe) values (:d,:idd,:uv,:pu,:qty,:p,:idp,:c)"
            );
        ...
        $sqlU="UPDATE `".$nomtablePagnetTampon."` SET codeBarrePcsInContainer='".$code."' where idPagnet=".$idPagnet;
