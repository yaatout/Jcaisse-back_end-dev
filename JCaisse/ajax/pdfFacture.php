...
//$var_id_facture = $_GET['id_param'];
    ...
    $sql2 = "SELECT count(*) FROM `".$nomtablePagnet."` where idPagnet=".$idPagnet." ";
    $res2 = mysql_query($sql2) or die ("persoonel requête 1".mysql_error());
    $nbre_ligne = mysql_fetch_array($res2);
    $nb_page = $nbre_ligne[0];
   ...
   // $pdf->SetFont( "Arial", "", 10 ); $pdf->SetXY( 27, 38 ) ; $pdf->MultiCell(190, 4, '#'.$pagnet["idPagnet"], 0, "L");
    ...
    //$pdf->SetFont( "Arial", "", 10 ); $pdf->SetXY( 5, 46 ) ; $pdf->MultiCell(190, 4, 'Mode de Reglement : '.$dateString2, 0, "L");
        ...
        $nom_file = "fact_" . $annee .'-' . str_pad($pagnet['datepagej'], 4, '0', STR_PAD_LEFT) . ".pdf";
        ...
        $pdf->Line(130, 70, 130, 240); $pdf->Line(150, 70, 150, 230);  $pdf->Line(174, 70, 174, 230);
        // titre colonne
        ...
        if ($num_page == $nb_page)
        {
            $nombre_format_francais = "Net a payer TTC : " . number_format($pagnet['apayerPagnet'], 2, ',', ' ') . " FCFA";
        ...
        $pdf->Cell(40,8,number_format($pagnet['totalp'], 2, ',', ' ')." FCFA",1,1,'L',0);
        $pdf->SetX(130);
        $pdf->Cell(33,8,'Remise',1,0,'L',1);
        $pdf->Cell(40,8,number_format($pagnet['remise'], 2, ',', ' ')." FCFA",1,1,'L',0);
    ...
    if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste") && ($_SESSION['proprietaire']==1)) {
        ...
        if (file_exists($filename)) {
            ...
            $message .= "Bonjour, \nvoici la facture numéro : ".$num_fact." .\n \nCordialement.\nL'équipe JCaisse.\r\n\n\n"; 
            $message .= "\r\n";   // ligne vide 
            ...
            $message .= "\r\n";
            $message .= "<html><body>";
            $message .= "<p style='color: #36a629; font-family: Arial,Helvetica,sans-serif;'> ";
            $message .= "<img src='https://lisette-mag.fr/Documents_telechargeables/eco-logo1-1.png' width='27' height='27' align='left' border='0' hspace='2' />";
            $message .= "<span style=' font-size: small;'><strong>Pensez environnement !</strong></span> <br />"; 
            $message .= "<span style='font-size: xx-small;'><strong>N'imprimez ce mail que si c'est vraiment nécessaire</strong></span>";
            $message .= "</p>";
            $message .= "</body></html>\r\n";
            $message .= "\r\n";
