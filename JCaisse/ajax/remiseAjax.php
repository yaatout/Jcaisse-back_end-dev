...
require('../declarationVariables.php');
		...
		if($nbre_fois>0){
			...
			if (isset($_POST['rms']) && isset($_POST['aPP'])) {
				...
				if($remise>=$totalp){
					...
					$remise=0;
					$sql3="UPDATE `".$nomtablePagnet."` set verrouiller=0, remise=".$remise.",apayerPagnet=".$apayerPagnet.",restourne=".$restourne.",versement=".$versement." where idPagnet=".$idPagnet;
				...
				// code...
                $sql3="UPDATE `".$nomtablePagnet."` set verrouiller=1,apayerPagnet=".$totalp." where idPagnet=".$idPagnet;
		        $res3=@mysql_query($sql3) or die ("mise à jour client  impossible".mysql_error());
				die($sql3);
			}
  ...
  if (isset($_POST['verrouiller'])) {
	...
	/*****Debut Nombre de Panier ouvert****/
	$query = mysql_query("SELECT * FROM `".$nomtableLigne."` where idPagnet=".$_POST['prm']." ");
	$nbre_fois=mysql_num_rows($query);
	/*****Fin Nombre de Panier ouvert****/
		//
	if($nbre_fois>0){
