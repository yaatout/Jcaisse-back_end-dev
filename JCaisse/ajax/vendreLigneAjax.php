...
if ($_SESSION['tampon'] == 1 && $_SESSION['page']=='vente') {
    require('vendreLigneAjax-tampon.php');
} else {
...
if($operation == 1){
  ...
  if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
    ...
    if($_SESSION['proprietaire']==1){
      ...
      $sqlV="SELECT DISTINCT d.designation
        ...
        LEFT JOIN `".$nomtablePagnet."` p ON p.idPagnet = l.idPagnet
  ...
  $reqP="SELECT * from `".$nomtablePagnet."` where idPagnet=".$idPagnet;
            ...
            if($ligne1 != null){
                ...
                if($ligne['classe']==1 || $ligne['classe']==0){
                    ...
                    if($design['uniteStock']!='Transaction'){
                        ...
                        $sql7="insert into `".$nomtableLigne."` (designation, idStock,idDesignation, unitevente,prixunitevente,quantite,prixtotal,idPagnet,classe,idEntrepot)values('".$design['designation']."','".$design['idDesignation']."','".$design['idDesignation']."','".$design['uniteStock']."',".$design['prix'].",1,".$design['prix'].",".$idPagnet.",1,".$_SESSION['entrepot'].")";
  ...
  // $sql3="UPDATE `".$nomtablePagnet."` SET idClient = 0 where idPagnet=".$idPanier;
  ...
  if ($dataType == 'simple') {
    ...
    if ($idClient == '0') {
      if ($_SESSION['compte'] == 1) {
        ...
        # code...
        $sql3="UPDATE `".$nomtablePagnet."` SET type=0, idClient=".$idClient." where idPagnet='".$idPanier."'";
  ...
  // echo $nomtableClient.' / '.$nomtablePagnet;
        ...
        if($mutuelle['type']==0 || $mutuelle['type']==30){
            ...
            if ($_SESSION['compte']==1) {
                ...
                if ($mutuelle['idCompte'] == 2) {
                    ...
                    $sql18="SELECT SUM(apayerPagnet) FROM `".$nomtablePagnet."` where verrouiller='1' && idClient=".$mutuelle['idClient']." && (type=0 || type=30) ";
  ...
  $idPanier=htmlspecialchars(trim($_POST['idPanier']));
        ...
        # code...
    $sql3="UPDATE `".$nomtablePagnet."` SET nomClient='".$nomClient."', idClient='".$idClient."' where idPagnet=".$idPanier;
    // var_dump($sql3);
	...
	$sql0="SELECT * FROM `".$nomtablePagnet."` where idPagnet=".$idPanierPro." ";
	$res0 = mysql_query($sql0) or die ("persoonel requête 2".mysql_error());
	$pagnet = mysql_fetch_assoc($res0) ;
  ...
  $idClient = $pagnet['idClient'];
	...
	$sql01="SELECT * FROM `".$nomtableLigne."` where idPagnet=".$idPanierPro." and quantite>0 ";
	$res01 = mysql_query($sql01) or die ("persoonel requête 2".mysql_error());

	$date = date("d-m-Y");
	$heure = date("H:i:s");

	$insertNewPanier = "INSERT INTO `".$nomtablePagnet."`(datepagej, type, idClient, heurePagnet, totalp, apayerPagnet, verrouiller, iduser) values ('".$date."',0,".$idClient.",'".$heure."',".$totalp.",".$totalp.",1,".$_SESSION['iduser'].")";
	$result1 = @mysql_query($insertNewPanier) or die ("insertion new panier impossible".mysql_error()) ;

	$sql1="SELECT * FROM `".$nomtablePagnet."` ORDER BY idPagnet DESC LIMIT 1";
	$res1 = mysql_query($sql1) or die ("persoonel requête 2".mysql_error());
  ...
  $idPagnet = $pagnet['idPagnet'];
    ...
    $designation = mysql_fetch_assoc($res02) ;
		...
		$insertNewLignes = "INSERT INTO `".$nomtableLigne."`(idPagnet,idDesignation,idEntrepot, designation, unitevente, prixunitevente, quantite, prixtotal, classe) values (".$idPagnet.",".$key['idDesignation'].",".$key['idEntrepot'].",'".$key['designation']."','".$key['unitevente']."',".$key['prixunitevente'].",".$key['quantite'].",".$key['prixtotal'].", 0)";
  ...
  $sql4="UPDATE `".$nomtablePagnet."` SET validerProforma=1 where idPagnet=".$idPanierPro;
  ...
  $sql3="SELECT * FROM `".$nomtablePagnet."` where type=10 and verrouiller=1 and  (nomClient LIKE '%$query%') ORDER BY nomClient";
