...
if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
 ...
 require('ajouterStock-pharmacie.php');
...
if (isset($_POST['btnAjouterStock-'.$i])) {
	...
	if ($_POST['quantiteAStocke-'.$i] > 0){
		...
		$uniteStockInitial= $_POST['uniteStock-'.$i];
		$prixunitaireInitial= $_POST['prixUnitaire-'.$i];
		$prixuniteStockInitial= @$_POST['prixuniteStock-'.$i];
		...
		if(mysql_num_rows($res)){
			if($tab=mysql_fetch_array($res))
				$nombreArticleUniteStock  = $tab["nbreArticleUniteStock"];

			if (($uniteStockInitial=="Article")||($uniteStockInitial=="article")){
				if ($dateExpiration){
				//echo 'je suis un';
				$sql1='INSERT INTO `'.$nomtableStock.'`(idDesignation,designation,quantiteStockinitial,uniteStock,prixunitaire,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant,dateExpiration) VALUES('.$i.',"'.mysql_real_escape_string($designationInitiale).'",'.$stockInitial.',"'.mysql_real_escape_string($uniteStockInitial).'",'.$prixunitaireInitial.','.$nombreArticleUniteStock.','.$stockInitial.',"'.$dateString.'",'.$stockInitial.',"'.$dateExpiration.'")';
				//echo $sql1;
				$res1=@mysql_query($sql1) or die ("insertion stock 2 impossible".mysql_error()) ;
        $msg_info="<p></br> LE STOCK ".mysql_real_escape_string($designationInitiale)." EST INITIALISE AVEC SUCCESS.</br></br>VEUILLEZ LE VERIFIER DANS GESTION DE STOCK DANS LE MENU STOCK ....</p>";
				}else{
				//echo 'je suis deux';
					$sql1='INSERT INTO `'.$nomtableStock.'`(idDesignation,designation,quantiteStockinitial,uniteStock,prixunitaire,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant) VALUES('.$i.',"'.mysql_real_escape_string($designationInitiale).'",'.$stockInitial.',"'.mysql_real_escape_string($uniteStockInitial).'",'.$prixunitaireInitial.','.$nombreArticleUniteStock.','.$stockInitial.',"'.$dateString.'",'.$stockInitial.')';
				//echo $sql1;
				$res1=@mysql_query($sql1) or die ("insertion stock 2 impossible".mysql_error()) ;
        $msg_info="<p></br> LE STOCK ".mysql_real_escape_string($designationInitiale)." EST INITIALISE AVEC SUCCESS.</br></br>VEUILLEZ LE VERIFIER DANS GESTION DE STOCK DANS LE MENU STOCK ....</p>";
				...
				$totalArticleStock=$stockInitial*$nombreArticleUniteStock;
				if ($dateExpiration){
				//echo 'je suis trois';
					$sql1='INSERT INTO `'.$nomtableStock.'`(idDesignation,designation,quantiteStockinitial,uniteStock,prixunitaire,prixuniteStock,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant,dateExpiration) VALUES('.$i.',"'.mysql_real_escape_string($designationInitiale).'",'.$stockInitial.',"'.mysql_real_escape_string($uniteStockInitial).'",'.$prixunitaireInitial.','.$prixuniteStockInitial.','.$nombreArticleUniteStock.','.$totalArticleStock.',"'.$dateString.'",'.$totalArticleStock.',"'.$dateExpiration.'")';
					//echo $sql1;
					$res1=@mysql_query($sql1) or die ("insertion stock 2 impossible".mysql_error()) ;
          $msg_info="<p></br> LE STOCK ".mysql_real_escape_string($designationInitiale)." EST INITIALISE AVEC SUCCESS.</br></br>VEUILLEZ LE VERIFIER DANS GESTION DE STOCK DANS LE MENU STOCK ....</p>";
				}else{
				//echo 'je suis quatre';
					$sql1='INSERT INTO `'.$nomtableStock.'`(idDesignation,designation,quantiteStockinitial,uniteStock,prixunitaire,prixuniteStock,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant) VALUES('.$i.',"'.mysql_real_escape_string($designationInitiale).'",'.$stockInitial.',"'.mysql_real_escape_string($uniteStockInitial).'",'.$prixunitaireInitial.','.$prixuniteStockInitial.','.$nombreArticleUniteStock.','.$totalArticleStock.',"'.$dateString.'",'.$totalArticleStock.')';
					//echo $sql1;
					$res1=@mysql_query($sql1) or die ("insertion stock 2 impossible".mysql_error()) ;
          $msg_info="<p></br> LE STOCK ".mysql_real_escape_string($designationInitiale)." EST INITIALISE AVEC SUCCESS.</br></br>VEUILLEZ LE VERIFIER DANS GESTION DE STOCK DANS LE MENU STOCK ....</p>";
  ...
  if(!$modifier){
      if($insererStock){
          ...
          if ($uniteStock=="article")
			...
			if($dateExpiration){
				if($prixuniteStock){
				$sql1="INSERT INTO `".$nomtableStock."`(idDesignation,designation,quantiteStockinitial,uniteStock,prixuniteStock,prixunitaire,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant,dateExpiration) VALUES (".$tab['idDesignation'].",'".mysql_real_escape_string($designation)."',".$stock.",'".mysql_real_escape_string($uniteStock)."',".$prixuniteStock.",".$prixunitaire.",".$nombreArticle.",".$total.",'".$dateString."',".$total.",'".$dateExpiration."')";
				//echo $sql1;
				$res1=@mysql_query($sql1) or die ("insertion stock 1 impossible".mysql_error()) ;}
				else{
				$sql1="INSERT INTO `".$nomtableStock."`(idDesignation,designation,quantiteStockinitial,uniteStock,prixunitaire,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant,dateExpiration) VALUES (".$tab['idDesignation'].",'".mysql_real_escape_string($designation)."',".$stock.",'".mysql_real_escape_string($uniteStock)."',".$prixunitaire.",".$nombreArticle.",".$total.",'".$dateString."',".$total.",'".$dateExpiration."')";
				//echo $sql1;
				$res1=@mysql_query($sql1) or die ("insertion stock 1 impossible".mysql_error()) ;
				}
			}else{
			if($prixuniteStock){
            $sql1='INSERT INTO `'.$nomtableStock.'`(idDesignation,designation,quantiteStockinitial,uniteStock,prixuniteStock,prixunitaire,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant) VALUES('.$tab["idDesignation"].',"'.mysql_real_escape_string($designation).'",'.$stock.',"'.mysql_real_escape_string($uniteStock).'",'.$prixuniteStock.','.$prixunitaire.','.$nombreArticle.','.$total.',"'.$dateString.'",'.$total.')';
			//echo $sql1;
            $res1=@mysql_query($sql1) or die ("insertion stock 2 impossible".mysql_error()) ;
			}else{
			$sql1='INSERT INTO `'.$nomtableStock.'`(idDesignation,designation,quantiteStockinitial,uniteStock,prixunitaire,nbreArticleUniteStock, totalArticleStock, dateStockage, quantiteStockCourant) VALUES('.$tab["idDesignation"].',"'.mysql_real_escape_string($designation).'",'.$stock.',"'.mysql_real_escape_string($uniteStock).'",'.$prixunitaire.','.$nombreArticle.','.$total.',"'.$dateString.'",'.$total.')';
			//echo $sql1;
            $res1=@mysql_query($sql1) or die ("insertion stock 2 impossible".mysql_error()) ;
			}
		}
          ...
          $msg_info="<p></br>LA REFERENCE DU STOCK QUE VOUS VOULEZ AJOUTER EST INEXISTANTE DANS LE CATALOGUE ...</p>";
    ...
    if($idStock){
      ...
      $total=$stock*$nombreArticle;
      $sql="update `".$nomtableStock."` set prixuniteStock=".$prixuniteStock.",prixunitaire=".$prixunitaire.",dateExpiration='".$dateExpiration."' where idStock=".$idStock;
	  //echo $sql;
      $res=@mysql_query($sql)or die ("modification impossible");
		 ...
		 $nombreArticleT=0;
          ...
          if(mysql_num_rows($res))
          	 if($tab=mysql_fetch_array($res)){
				 ...
				 if($dateE){
					 $sql3="INSERT INTO `".$nomtableStock."` (idDesignation,designation,quantiteStockinitial,uniteStock,nbreArticleUniteStock,prixunitaire,prixuniteStock, totalArticleStock, dateStockage, quantiteStockCourant,dateExpiration)
					 ...
					 $res3=@mysql_query($sql3) or die ("insertion stock 1 CSV impossible".mysql_error());
						...
						$sql3="INSERT INTO `".$nomtableStock."` (idDesignation,designation,quantiteStockinitial,uniteStock,nbreArticleUniteStock,prixunitaire,prixuniteStock, totalArticleStock, dateStockage, quantiteStockCourant) VALUES(".$tab["idDesignation"].",'".mysql_real_escape_string($reference)."',".$quantite.",'".mysql_real_escape_string($uQuantite)."',".$tab["nbreArticleUniteStock"].",".$prixU.",".$prixUS.",".$total.",'".$dateString."',".$total.")";
						//echo $sql3;
					 	//var_dump($sql3);
...
if(isset($_POST["Export"])){
      ...
      $fields=array('REFERENCE', 'QUANTITE','UNITE-QUANTITE-STOCK','PRIX-UNITAIRE', 'PRIX-UNITE-STOCK', 'DATE-EXPIRATION');
...
/*echo ' <center>
<table><tr><td>
          <form class="form-horizontal" action="ajouterStock.php" method="post" name="upload_excel"
                          enctype="multipart/form-data">
                                    <input type="submit" name="Export" class="btn btn-success" value="Générer le modéle CSV d\'Importation de Stocks"/>
...
<form role="form" class="" name="formulaire2" id="ajouterStockForm" method="post" action="ajouterStock.php">
                 ...
                 <div class="modal-dialog">
                     <div class="modal-content">
                         ...
                         <div class="modal-body">
                               <form role="form" class="" id="form" name="formulaire2" method="post" action="ajouterStock.php">
...
echo'</br></br><div class="container" align="center">
        ...
        <div class="tab-content">
            <div class="tab-pane fade in active" id="STOCKPRODUITDETAIL">';
					 ...
					 $j=0;
					  ...
					  $j=$j+1;
							...
							if(!mysql_num_rows($res5)){
                              echo'<tr><form class="form" id="form"><td>'.$tab3["designation"].'</td>
               ...
               <td><input type="hidden" name="idDesignation-btnAjouterStock" value="'.$tab3["idDesignation"].'"/><input type="hidden" name="designation-'.$tab3['idDesignation'].'" value="'.$tab3["designation"].'"/>
               <button type="button" onclick="init_Stock('.$tab3["idDesignation"].')" id="btn_InitStock-'.$tab3['idDesignation'].'" class="btn btn-success"><i class="glyphicon glyphicon-plus"></i>AJOUTER</button></td>
							 
							 </form></tr>';

                      }else{
					  $tab5=mysql_fetch_array($res5);
					  echo'<tr><form class="form" id="form" method="post" action="ajouterStock.php"><td>'.$tab5["designation"].'</td>
                              <td>'.$tab3["categorie"].'</td> 
							  ...
							  echo'<td><input type="text" name="prixUnitaire-'.$tab3['idDesignation'].'" id="prixUnitaire-'.$tab3['idDesignation'].'" value="'.($tab5["prixunitaire"] * $_SESSION['devise']).' '.$_SESSION['symbole'].'" disabled=""/></td>';
							  
							 if(($tab3["uniteStock"]!='Article')&&($tab3["uniteStock"]!='article')){
							  echo'<td><input type="text" name="prixuniteStock-'.$tab3['idDesignation'].'" id="prixuniteStock-'.$tab3['idDesignation'].'" value="'.($tab5["prixuniteStock"] * $_SESSION['devise']).' '.$_SESSION['symbole'].'" disabled="" /></td>';
							  }else{
							  echo'<td><input type="number" name="prixuniteStock-'.$tab3['idDesignation'].'" id="prixuniteStock-'.$tab3['idDesignation'].'" value="" disabled=""/></td>';
							  }
