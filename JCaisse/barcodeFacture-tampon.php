...
p.inline {display: inline-block;}
	...
	<?php if ($_SESSION['idBoutique']==175) {?>
    span { font-size: 14px; font-weight : bold; font-family: "Lucida Console"; }
...
if (isset($_POST['idPagnet'])) {
  ...
  $sql1="SELECT * FROM `".$nomtablePagnetTampon."` where idPagnet=".$idPagnet."";
	$res1 = mysql_query($sql1) or die ("persoonel requête 12222".mysql_error());
  ...
  if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
    ...
    $idClient=$pagnet['idClient'];
          ...
          echo  '<table class="table"> '; 
            ...
            if ($type==10) { 
              echo '
                <tr align="center">
                  <td colspan="4" style="border-bottom: 1px solid black;border-top: 1px solid black;"><b>TICKET PROFORMA</b></td>
                </tr>
              ';
            }

              $sql2="SELECT * FROM `".$nomtableClient."` where idClient=".$idClient;
              $res2 = mysql_query($sql2) or die ("persoonel requête 133".mysql_error());
              ...
              while ($vente = mysql_fetch_array($resV)) { 
                ...
                $entrepot = mysql_fetch_array($resE);
                    ...
                    $sql2="SELECT * FROM `".$nomtableLigneTampon."` where idPagnet=".$idPagnet." AND idEntrepot=".$entrepot['idEntrepot']." ORDER BY numligne DESC";
              ...
              <tr>
                <td colspan="4">
                    <span style="float: left"> TOTAL </span> <span style="float: right">'.abs($pagnet['totalp']).' </span> <br> ';
                    if ($pagnet['remise']!=0) {
                      echo '<span style="float: left"> REMISE </span> <span style="float: right">'.$pagnet['remise'].'  </span> <br> ';
                    }
                    echo '<span style="float: left"> NET A PAYER </span> <span style="float: right">'.abs($pagnet['apayerPagnet']).' </span> <br> ';
                    if ($pagnet['versement']!=0) {
                    echo '<span style="float: left"> ESPECES </span> <span style="float: right">'.$pagnet['versement'].' </span> <br>
                    <span style="float: left"> RENDU </span> <span style="float: right">'.($pagnet['versement'] - $pagnet['apayerPagnet']).' </span>';
                    } 
        ...
        if ($_SESSION['adresseB']=='Bignona'){
            ...
            ';
            $sql2="SELECT * FROM `".$nomtableClient."` where idClient=".$idClient;
            $res2 = mysql_query($sql2) or die ("persoonel requête 1444".mysql_error());
            $client = mysql_fetch_array($res2);
            if ($idClient==0) {
              echo  '
                    <tr>
    ...
    echo  '<table class="table" > ';
              ...
              */
              ...
              ';
                  $sql2="SELECT * FROM `".$nomtableClient."` where idClient=".$idClient;
                  $res2 = mysql_query($sql2) or die ("persoonel requête 1555".mysql_error());
                  $client = mysql_fetch_array($res2);
              if ($idClient==0) {
                echo  '
                      <tr>
              ...
              $sql2="SELECT * FROM `".$nomtableLigneTampon."` where idPagnet=".$idPagnet." ORDER BY numligne DESC";
              $res2 = mysql_query($sql2) or die ("persoonel requête 2".mysql_error());
                    while ($ligne = mysql_fetch_array($res2)) {
                      /*
              ...
              <tr>
                <td colspan="4">
                    <span style="float: left"> TOTAL </span> <span style="float: right">'.abs($total).' </span> <br> ';
                    if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
                      if ($pagnet['taux']!=0) {
                    ...
                    echo '<span style="float: left"> NET A PAYER </span> <span style="float: right">'.abs($netPayer).' </span> <br> ';
                    if ($pagnet['versement']!=0) {
                    echo '<span style="float: left"> ESPECES </span> <span style="float: right">'.$pagnet['versement'].' </span> <br>
...
if (isset($_POST['idMutuellePagnet'])) {
  ...
  $sql1="SELECT * FROM `".$nomtableMutuellePagnet."` where idMutuellePagnet=".$idMutuellePagnet."";
	$res1 = mysql_query($sql1) or die ("persoonel requête 1666".mysql_error());
  	$pagnet = mysql_fetch_array($res1);
	$heureB=$pagnet['heurePagnet'];
      $date1=$pagnet['datepagej'];
  ...
  ';
    $sql2="SELECT * FROM `".$nomtableLigneTampon."` where idMutuellePagnet=".$idMutuellePagnet." ORDER BY numligne DESC";
    $res2 = mysql_query($sql2) or die ("persoonel requête 2".mysql_error());
        echo ' 
        <tr  style="border-bottom: 1px solid black;border-top: 1px solid black;">
          <td colspan="2"><span><strong>REFERENCE<strong></span></td>
...
elseif (isset($_POST['barcodeFactureV'])) {
  ...
  $res1 = mysql_query($sql1) or die ("persoonel requête 1777".mysql_error());
  ...
  echo  '<table class="table"> ';
    ...
    */
    ...
    $res2 = mysql_query($sql2) or die ("persoonel requête 1888".mysql_error());
    $client = mysql_fetch_array($res2);
    if ($client!=null) {
      $sql5="SELECT *
      FROM `aaa-utilisateur`  
...
elseif (isset($_POST['idClient'])) {
  ...
  $sql2="SELECT * FROM `".$nomtableClient."` where idClient=".$idClient;
  $res2 = mysql_query($sql2) or die ("persoonel requête 1999".mysql_error());
  $client = mysql_fetch_array($res2);
  $sql12="SELECT SUM(apayerPagnet) FROM `".$nomtablePagnetTampon."` where idClient=".$idClient." AND (type=0 || type=30) ";
 ...
 <?php
  ...
  /**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
    ...
    FROM
    (SELECT p.idClient,p.idPagnet,p.datepagej,p.heurePagnet FROM `".$nomtablePagnetTampon."` p where p.idClient='".$idClient."' AND p.verrouiller=1 AND (p.type=0 || p.type=30)
    UNION 
  ...
  if(mysql_num_rows($resP1)){
    while ($bons = mysql_fetch_array($resP1)) { 
      ...
      $sqlP="select * from `".$nomtablePagnetTampon."` where idPagnet='".$bons["idPagnet"]."' AND idClient='".$idClient."' ";
      $resP=mysql_query($sqlP);
      $pagnet = mysql_fetch_array($resP); 
      if($pagnet!=null){
        echo ' BON N : '.$bons["idPagnet"].'<br>';
        $sql2="SELECT * FROM `".$nomtableLigneTampon."` where idPagnet=".$pagnet['idPagnet']." ORDER BY numligne DESC";
        $res2 = mysql_query($sql2) or die ("persoonel requête 2".mysql_error());
        if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
          echo ' 
        ...
        echo '<b> TOTAL : '.number_format(abs($pagnet['totalp']), 2, ',', ' ').'</b><br>';
        echo '<b> NET A PAYER : '.number_format(abs($pagnet['apayerPagnet']), 2, ',', ' ').'</b><br>';
      }
