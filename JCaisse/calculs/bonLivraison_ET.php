...
try{
    ...
    /**Debut Ajouter Stock**/
        if($operation=='ajouter_Stock'){
            ...
            $calcul=0;
            foreach ($bls as $bl) {
                $calcul = $calcul + ($bl['prixachat'] * $bl['quantiteStockinitial']);
            }
    ...
    /**Debut Transferer Stock**/
        if($operation=='transferer_Stock'){
            ...
            foreach ($bls as $bl) {
                if($bl['idBl']==$idBl){
                    $montant1 = $montant1 + ($bl['prixachat'] * $bl['quantiteStockinitial']);
                }
                else if($bl['idBl']==$idBon){
                    $montant2 = $montant2 + ($bl['prixachat'] * $bl['quantiteStockinitial']);
                }
            }
