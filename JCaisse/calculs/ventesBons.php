...
try{
    ...
    /**Debut Ajouter Panier**/
        if($operation=='ajouter_Panier'){
            ...
            $stmPanier_Ajouter = $bdd->prepare("INSERT INTO `".$nomtablePagnet."` (datepagej, type, heurePagnet, totalp, apayerPagnet, verrouiller, idClient, idCompte, iduser)
            VALUES (:datepagej, :type, :heurePagnet, :totalp, :apayerPagnet, :verrouiller, :idClient, :idCompte, :iduser)");
    ...
    /**Debut Choisir Compte**/
        if($operation=='choisir_Compte'){
            if($idCompte!=2){
                $stmPanier_Modifier = $bdd->prepare("UPDATE `".$nomtablePagnet."` SET idCompte =:idCompte
                WHERE idPagnet =:idPagnet ");
                $stmPanier_Modifier->execute(array(
                    ':idPagnet' => $idPagnet,
                    ':idCompte' => $idCompte
        ...
        if($operation=='ajouter_Ligne'){
            ...
            if($reference['classe']!=10){
                ...
                if($ligne_Existe){
                    ...
                    if($nombre_Ligne!=0){
                        if(($type==$reference['classe']) || (($type==0 || $type==10) && ($reference['classe']==1 || $reference['classe']==0)) || ($type==6 && $reference['classe']==6)){
        ...
        if($operation=='valider_Panier'){
            ...
            try {
                ...
                if($panier['type']==0){
                    ...
                    verrouiller =:verrouiller, totalp=:totalp, remise=:remise, apayerPagnet = totalp - remise, versement=:versement, restourne = versement - apayerPagnet
                    WHERE idPagnet =:idPagnet ");
                    $stmPanier_Modifier->execute(array(
                        ':idPagnet' => $idPagnet,
                        ':totalp' => $panier_Total, 
                ...
                if($_SESSION['compte']==1 && $panier['type']!=10){
                    ...
                    if(is_numeric($avance)){
                        $versement=depot_Compte_Avance($idPagnet,$panier['idClient'],$avance,$idCompte);
                    } 
                } 

                mise_A_jour_Client($idClient);
                $montant = calcul_Montant_Client($idClient,$dateDebut,$dateFin);
        ...
        if($operation=='detailsPanier'){
            ...
            if($panier){
                ...
                if($panier['idClient']!=0){
                    ...
                    $datas[] = $client['prenom'].' '.strtoupper($client['nom']).'<>'.$client['adresse'].'<>'.$client['telephone'];
                }
                else{ $datas[] = 0; }
    ...
    /**Debut Ajouter Versement**/
        if($operation=='ajouter_Versement'){
            ...
            (idClient,montant,dateVersement,heureVersement,idCompte, iduser) 
            VALUES (:idClient, :montant, :dateVersement, :heureVersement, :idCompte, :iduser)");
            ...
            $stmVersement= $bdd->prepare("SELECT  * FROM `".$nomtablePagnet."` 
            WHERE idVersement = :idVersement ");
            $stmVersement->execute(array(
                ':idVersement' => $last_idVersement
            ));
            $versement = $stmVersement->fetch(PDO::FETCH_ASSOC);

            $rows = array();
            $rows[] = $last_idVersement;
            $rows[] = $dateString2;	
            $rows[] = $heureString;
            $rows[] = 0;	
    ...
    /**Debut Terminer Versement**/
        if($operation=='valider_Versement'){
    ...
    /**Debut Retourner Versement**/
        if($operation=='retourner_Versement'){

            $stmLigne_Supprimer = $bdd->prepare("DELETE FROM `".$nomtableVersement."` WHERE idVersement = :idVersement ");
            $stmLigne_Supprimer->execute(array(':idVersement' => $idVersement ));
    ...
    /**Fin Retourner Versement**/

    /**Debut Upload Image Versement**/
        if($operation=='upload_Image_Versement'){
            $format='';
            $image='';
            if(isset($_FILES['file'])){
                ...
                if(in_array($extension, $extensions) && $error == 0){
                    ...
                    $stmVersement_Modifier = $bdd->prepare("UPDATE `".$nomtableVersement."` SET  
                    image=:image  WHERE idVersement =:idVersement ");
                    $stmVersement_Modifier->execute(array(
                        ':idVersement' => $idVersement,
                        ':image' => $file
                    ));
    ...
    function depot_Compte_Avance($idPagnet,$idClient,$avance,$idCompte){
        require('../connectionPDO.php');
        require('../declarationVariables.php');
        $stmPanier_Modifier = $bdd->prepare("UPDATE `".$nomtablePagnet."` SET avance = 1
        WHERE idPagnet =:idPagnet ");
        $stmPanier_Modifier->execute(array(
            ':idPagnet' => $idPagnet
        ));
    ...
    function mise_A_jour_Client($idClient){
        require('../connectionPDO.php');
        require('../declarationVariables.php');

        $stmtVersement= $bdd->prepare("SELECT  SUM(montant) as total FROM `".$nomtableVersement."`  WHERE idClient = :idClient ");
        $stmtVersement->execute(array(
            ':idClient' => $idClient
        ));
        $versement = $stmtVersement->fetch(PDO::FETCH_ASSOC);

        $stmtPanier= $bdd->prepare("SELECT  SUM(apayerPagnet) as total FROM `".$nomtablePagnet."`  WHERE idClient = :idClient AND type<>2 ");
        $stmtPanier->execute(array(
            ':idClient' => $idClient
        ));
        $panier = $stmtPanier->fetch(PDO::FETCH_ASSOC);

        $solde = $panier['total'] - $versement['total'];
        $stmtClient = $bdd->prepare("UPDATE `".$nomtableClient."` SET solde=:solde WHERE idClient = :idClient ");
        $stmtClient->execute(array(
            ':idClient' => $idClient,
            ':solde' => $solde
        ));
    ...
    function calcul_Montant_Client($idClient,$dateDebut,$dateFin){
        require('../connectionPDO.php');
        require('../declarationVariables.php');

        $stmtVersement= $bdd->prepare("SELECT  SUM(montant) as total FROM `".$nomtableVersement."` v  WHERE v.idClient = :idClient
        ...
        $stmtPanier= $bdd->prepare("SELECT  SUM(apayerPagnet) as total FROM `".$nomtablePagnet."` p WHERE p.idClient = :idClient AND p.type<>2 
        ...
        $solde = $panier['total'] - $versement['total'];
        $montant=number_format($solde, 2, ',', ' ').'<>'.number_format($panier['total'], 2, ',', ' ').'<>'.number_format($versement['total'], 2, ',', ' ');
        return $montant;
    }
