...
if ($operation=="1" || $operation=="3" || $operation=="4") {
  ...
  if ($query =="") {
    ...
    $sql="SELECT *, SUM(apayerPagnet) as total_cbm FROM `".$nomtablePagnet."` p, `".$nomtableClient."` c where p.idClient=c.idClient and p.numContainer='".$numContainer."' and p.type=0 and c.archiver=0 GROUP BY p.idClient";
    ...
    $sql="SELECT *, SUM(apayerPagnet) as total_cbm FROM `".$nomtablePagnet."` p, `".$nomtableClient."` c where p.idClient=c.idClient and p.numContainer='".$numContainer."' and p.type=0 and c.archiver=0 and (CONCAT(c.prenom,' ', c.nom) LIKE '%".$query."%' or c.telephone LIKE '%".$query."%') GROUP BY p.idClient";
  ...
  $sqlV="SELECT *, SUM(montant) as total_versement FROM `".$nomtableVersement."` WHERE idContainer='".$numContainer."' GROUP BY idClient";
