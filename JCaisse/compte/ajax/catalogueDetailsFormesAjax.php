...
while($tab5=mysql_fetch_array($res15)){
  ...
  $rows = array();
	$rows[] = $i;
  $rows[] = strtoupper($tab5['nomForme']);
  $rows[] = '<a onclick="mdf_Forme_PH('.$tab5["id"].','.$id.','.$i.')" ><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a>
						<a onclick="spm_Forme_PH('.$tab5["id"].','.$id.','.$i.')" ><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>';
  $data[] = $rows;
	$i= $i + 1;
}
