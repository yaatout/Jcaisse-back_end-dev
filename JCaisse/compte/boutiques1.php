...
if (isset($_POST['btnActiver'])) {
 ...
 /*$labelB						=@htmlentities($_POST['labelB']);
	...
	$sql2="SELECT * FROM `aaa-acces` WHERE idBoutique=".$idBoutique;
	$res2 = mysql_query($sql2) or die ("personel requête 2".mysql_error());
	while ($acces = mysql_fetch_array($res2)) { 
		$sql="DELETE FROM `aaa-utilisateur` WHERE idutilisateur=".$acces['idutilisateur'];
		$res=@mysql_query($sql) or die ("suppression impossible personnel".mysql_error());
		
		$sql="DELETE FROM `aaa-acces` WHERE idutilisateur=".$acces['idutilisateur'];
		$res=@mysql_query($sql) or die ("suppression impossible personnel".mysql_error());
	}
	
	$sql="DELETE FROM `aaa-boutique` WHERE idBoutique=".$idBoutique;
  	$res=@mysql_query($sql) or die ("suppression impossible personnel".mysql_error());
...
?>
	...
	<!----------------------------------------------------------->
					...
					$sql4="SELECT * FROM `aaa-utilisateur` where idadmin=".$_SESSION['iduser'];
