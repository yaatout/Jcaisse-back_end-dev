...
if (isset($_POST['btnSupprimer'])) {
	...
	$nom         =@$_POST["nom"];
	$type         =@$_POST["type"];
	$catalogueDetail='aaa-catalogue-'.$_POST["nom"];
	$catalogueDetailCategorie='aaa-categorie-'.$_POST["nom"];

	$sql="DELETE FROM `aaa-catalogueTotal` WHERE id=".$id;
	$res=@mysql_query($sql) ;

	$sql2="DROP TABLE `".$catalogueDetail."` ";
	$res2=@mysql_query($sql2)  ;

	$sql3="DROP TABLE `".$catalogueDetailCategorie."` ";
	$res3=@mysql_query($sql3) ;

	if ($type=="Pharmacie") {

		$catalogueDetailForme='aaa-forme-'.$_POST["nom"];
		$sql4="DROP TABLE `".$catalogueDetailForme."` ";
