...
if(!$modifier and !$supprimer){
  if($nomTransaction) {
		...
		if(!@mysql_num_rows($res11)){
		    if($aliasTransaction){
				$sql="insert into `aaa-transaction` (nomTransaction,aliasTransaction,typeTransaction) values ('".mysql_real_escape_string($nomTransaction)."','".mysql_real_escape_string($aliasTransaction)."','".mysql_real_escape_string($typeTransaction)."')";
				//echo $sql;
       			$res=@mysql_query($sql) or die ("insertion unite stock impossible".mysql_error());
                }else{
                $sql="insert into `aaa-transaction` (nomTransaction,aliasTransaction,typeTransaction) values ('".mysql_real_escape_string($nomTransaction)."','".mysql_real_escape_string($nomTransaction)."','".mysql_real_escape_string($typeTransaction)."')";
    				//echo $sql;
           			$res=@mysql_query($sql) or die ("insertion unite stock impossible".mysql_error());
		...
		echo'<script language="JavaScript">document.location="insertionTransfert.php"</script>';
		echo'</head>';
		echo'</html>';
		}
...
elseif($modifier){
       $sql1="update `aaa-transaction` set nomTransaction='".mysql_real_escape_string($nomTransaction)."',aliasTransaction='".mysql_real_escape_string($aliasTransaction)."',typeTransaction='".mysql_real_escape_string($typeTransaction)."' WHERE idTransaction =".$idTransaction ;
 ...
 // echo $sql2;
  $res2=@mysql_query($sql2) or die ("suppression impossible transaction     ".mysql_error());
}
				...
				if(is_array($_FILES)) {
						...
						switch ($uploadImageType) {
								case IMAGETYPE_JPEG:
										...
										$sql="update `aaa-transaction` set image='".mysql_real_escape_string($fileNameNew)."' where idTransaction='".$id."'";
...
if(isset($_POST["btnSupImg"])) {
			 ...
			 if(unlink("images/".$_POST['image'])) {
						...
						//$catTypeCateg     =@$_POST["tab"];
						$fileNameNew='';
						$sql="update `aaa-transaction` set image='".$fileNameNew."' where idTransaction='".$id."'";
						$res=@mysql_query($sql) or die ("upload impossible1 ".mysql_error());
...
<div class="container">
        ...
        <center><button type="button" class="btn btn-primary btn-success" data-toggle="modal" data-target="#categorieModal" id="AjoutDesignation">Ajouter un Transfert </button></center>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Ajout d'un Transfert </h4>
                    </div>
                    <div class="modal-body">
                       <form role="form" class=" formulaire2" name="formulaire2" method="post" action="insertionTransfert.php">
                             <input type="hidden" class="inputbasic" id="dateAjout" name="dateAjout" value="<?php echo $dateString2; ?> " >
							    ...
							    <div class="form-group row">
							                 <div class="col-sm-6">
                                                  <input type="hidden" class="inputbasic" id="aliasTransaction" name="typeTransaction" size="35" value="Transaction">
								             </div>
								</div>
...
echo'<li class="active"><a data-toggle="tab" href="#PRODUIT">LISTE DES TRANSFERTS </a></li>';
...
$sql="select * from `aaa-transaction` where typeTransaction='Transaction'  order by idTransaction ASC";
$res=mysql_query($sql);
		...
		echo '<div id="imgsup'.$tab["idTransaction"].'" class="modal fade" role="dialog">
				<div class="modal-dialog">
				   <div class="modal-content">
					   <div class="modal-header">
							<button type="button" class="close" data-dismiss="modal">&times;</button>
							<h4 class="modal-title">Confirmation Suppression Transfert </h4>
						</div>
						<div class="modal-body" style="padding:40px 50px;">


                            <form role="form" class="" id="form" name="formulaire2" method="post" action="insertionTransfert.php">
							    ...
							    <div class="form-group row">
											<label for="ALIAS" class="col-sm-4 col-form-label">Type <font color="red">* </font>:</label>
							                 <div class="col-sm-6">
                                                <input type="hidden" class="inputbasic" id="aliasTransaction" name="typeTransaction" size="35" value="Transaction">
											</div>
								</div>
		...
		'<div id="imgmodifier'.$tab["idTransaction"].'"  class="modal fade " role="dialog">
			...
			<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal">&times;</button>
							<h4 class="modal-title">Modifier Transfert</h4>
						</div>
						<div class="modal-body" style="padding:40px 50px;">
