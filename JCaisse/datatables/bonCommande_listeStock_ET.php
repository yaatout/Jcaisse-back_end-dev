<?php
   ...
   */
   ...
   // Fetch records
   $stmt = $bdd->prepare("SELECT *, quantiteCommande as quantite, totalArticleStock * prixachat as montantachat FROM `".$nomtableStock."` 
   ...
   foreach ($empRecords as $row) {
      ...
      $cols_quantite = '<input type="number" class="form-control" id="inpt_mdf_Stock_Quantite_'.$row['idStock'].'" min=1 style="width: 70px;" value="'.$row['quantite'].'"/>';
      ...
      if($row['quantiteStockCourant']!=0){
         if($row['quantiteStockCourant']==$row['totalArticleStock']){
         ...
         $cols_operations = '<a><img src="images/drop.png" align="middle" alt="supprimer" onclick="supprimer_Stock('.$row["idStock"].')"  data-toggle="modal" data-target="#imgsup'.$row["idStock"].'" /></a>&nbsp
            <button type="button" onclick="modifier_Stock('.$row["idStock"].')" id="btn_modifier_Stock-'.$row['idStock'].'" class="btn btn-success">
