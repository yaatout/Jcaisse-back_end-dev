<?php
   ...
   // Fetch records
   $stmt = $bdd->prepare("SELECT *, quantiteStockCourant as restant, quantiteStockinitial as initial, quantiteStockinitial * prixSession as montantachat FROM `".$nomtableStock."` 
   ...
   foreach ($empRecords as $row) {
      ...
      if($row['quantiteStockinitial']==$row['quantiteStockCourant']){
         ...
         $cols_prixSession = '<input type="number" class="form-control" id="inpt_mdf_Stock_PrixSession_'.$row['idStock'].'" min=1 style="width: 100px;" value="'.$row['prixSession'].'"/>';
         $cols_prixPublic = '<input type="number" class="form-control" id="inpt_mdf_Stock_PrixPublic_'.$row['idStock'].'" min=1 style="width: 100px;" value="'.$row['prixPublic'].'"/>';
