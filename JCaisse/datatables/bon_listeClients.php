<?php
   ...
   foreach ($clients as $client) {
      ...
      $stmtBons = $bdd->prepare("SELECT SUM(apayerPagnet) AS total FROM `".$nomtablePagnet."` WHERE idClient=:idClient AND verrouiller=1 AND (type=0 OR type=30 OR type=11) ");
      ...
      if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
         ...
         $stmtMutuelles = $bdd->prepare("SELECT SUM(apayerPagnet) AS total FROM `".$nomtableMutuellePagnet."` WHERE idClient=:idClient AND verrouiller=1 AND (type=0 OR type=30 OR type=11) ");
      ...
      //Colonne Plafond
      if ($client['plafond']>$solde || $client['plafond']==null || $client['plafond']==0) {          
         $cols_plafond = $client['plafond'];
      } else {
         $cols_plafond = '<td style="background-color:red">'.$client['plafond'].'</td>';
      }
      ...
      if ($_SESSION['proprietaire']==1 || $_SESSION['gerant']==1) {
         if ($client['activer']==0 && ($bons['total']==0  &&  $client['solde']==0 )){
            if ($client['activer']==0){
      ...
      $data[] = array(
         ...
         "plafond"=>$cols_plafond,
         "montant"=> $cols_montant,
         "operations"=>$cols_operations
      );

      //Mise a jour du solde de tout les clients non archives
