<?php
   ...
   $stmt = $bdd->prepare("SELECT COUNT(*) AS allcount FROM `".$nomtableClient."`  WHERE archiver<>1 AND solde<0 ");
   $stmt->execute();
   $records = $stmt->fetch();
   $totalRecords = $records['allcount'];

   // Total number of records with filtering
   $stmt = $bdd->prepare("SELECT COUNT(*) AS allcount FROM `".$nomtableClient."` WHERE 1 ".$searchQuery." AND archiver<>1 AND solde<0  ");
   $stmt->execute($searchArray);
   $records = $stmt->fetch();
   $totalRecordwithFilter = $records['allcount'];

   // Fetch records
   $stmt = $bdd->prepare("SELECT * FROM `".$nomtableClient."` WHERE 1 ".$searchQuery." AND archiver<>1 AND solde<0  ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
