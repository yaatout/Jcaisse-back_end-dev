<?php
   ...
   $stmt = $bdd->prepare("SELECT COUNT(*) AS allcount FROM `".$nomtableClient."` WHERE archiver<>1 AND personnel=1");
   $stmt->execute();
   $records = $stmt->fetch();
   $totalRecords = $records['allcount'];

   // Total number of records with filtering
   $stmt = $bdd->prepare("SELECT COUNT(*) AS allcount FROM `".$nomtableClient."` WHERE 1 ".$searchQuery." AND archiver<>1 AND personnel=1 ");
   $stmt->execute($searchArray);
   $records = $stmt->fetch();
   $totalRecordwithFilter = $records['allcount'];

   // Fetch records
   $stmt = $bdd->prepare("SELECT * FROM `".$nomtableClient."` WHERE 1 ".$searchQuery." AND archiver<>1  AND personnel=1 ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
   ...
   foreach ($clients as $client) {
      ...
      if ($_SESSION['proprietaire']==1 || $_SESSION['gerant']==1) {
         if ($client['activer']==0 && ($client['solde']==0 )){
            if ($client['activer']==0){
