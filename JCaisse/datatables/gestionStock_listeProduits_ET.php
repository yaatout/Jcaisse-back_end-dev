<?php
   ...
   $cols_entrepot_options = '';
   $stmtEntrepot = $bdd->prepare("SELECT * FROM `".$nomtableEntrepot."`  ");
   $stmtEntrepot->execute();
   $entrepots = $stmtEntrepot->fetchAll(); 
   foreach ($entrepots as $entrepot) {
      $cols_entrepot_options = $cols_entrepot_options.'<option value= "'.$entrepot["idEntrepot"].'">'.$entrepot["nomEntrepot"].'</option>';
   }
   ...
   foreach ($empRecords as $row) {
      ...
      if ($_SESSION['idBoutique']==194) {
         $cols_prixachat = '<input type="number" class="form-control" id="inpt_Stock_PrixAchat_'.$row['idDesignation'].'" style="width: 100px;"  min=1 value="'.round($row['prixachat']*$row['nbreArticleUniteStock']).'" required=""/>';
      } else {
         $cols_prixachat = '<input type="number" class="form-control" id="inpt_Stock_PrixAchat_'.$row['idDesignation'].'" style="width: 100px;"  min=1 value="'.$row['prixachat'].'" required=""/>';
      }
      $cols_expiration = '<input type="date" class="form-control" id="inpt_Stock_DateExpiration_'.$row['idDesignation'].'" />';

      $cols_entrepot = '<select style="width: 100px;" class="form-control"  id="slct_Stock_Entrepot_'.$row['idDesignation'].'">
