<?php
   ...
   */
   ...
   // Fetch records
   $stmt = $bdd->prepare("SELECT MAX(idStock) as idStock, SUM(quantiteStockCourant) as quantite, MAX(dateStockage) as dateStockage, idDesignation, designation FROM `".$nomtableStock."` 
   ...
   foreach ($empRecords as $row) {
      ...
      if($row['dateStockage']==$dateString){
         $cols_operations =  '<a href="stock-Pharmacie.php?iDS='.$row["idDesignation"].'"><span style="color:green;">Details</span>';

         $data[] = array(
            "idStock"=> $row['idStock'],
            "designation"=>'<span style="color:#901E06;">'.$row['designation'].'</span>',
            "codeBarreDesignation"=>'<span style="color:#901E06;">'.$produit['codeBarreDesignation'].'</span>',
            "quantite"=>'<span style="color:#901E06;">'.$row['quantite'].'</span>',
            "forme"=>'<span style="color:#901E06;">'.$produit['forme'].'</span>',
            "prixSession"=> '<span style="color:#901E06;">'.$produit['prixSession'].'</span>',
            "prixPublic"=> '<span style="color:#901E06;">'.$produit['prixPublic'].'</span>',
         ...
         $data[] = array(
            ...
            "codeBarreDesignation"=>$produit['codeBarreDesignation'],
