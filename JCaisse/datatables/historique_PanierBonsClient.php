...
try{
    ...
    if (($_SESSION['mutuelle']==1) AND ($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) { 
      $stmt = $bdd->prepare("SELECT COUNT(codePanier) AS allcount
        FROM
        (SELECT CONCAT(p.idPagnet,'+1') as codePanier FROM `".$nomtablePagnet."` p  WHERE p.verrouiller=1 AND p.idClient<>0 AND (p.type=0 || p.type=30)  AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') )
        UNION ALL
        SELECT CONCAT(m.idMutuellePagnet,'+2') as codeMutuelle FROM `".$nomtableMutuellePagnet."` m WHERE m.verrouiller=1 AND m.idClient<>0 AND (m.type=0 || m.type=30)  AND ((CONCAT(CONCAT(SUBSTR(m.datepagej,7, 10),'',SUBSTR(m.datepagej,3, 4)),'',SUBSTR(m.datepagej,1, 2)) ='".$dateJour."') or (m.datepagej ='".$dateJour."') )
        ) AS a ");
      $stmt->execute();
      $records = $stmt->fetch();
      ...
      $stmtOperation = $bdd->prepare("SELECT *, codePanier 
        FROM
        (SELECT CONCAT(p.idPagnet,'+1') as codePanier, p.idPagnet, p.idClient, p.heurePagnet, p.apayerPagnet FROM `".$nomtablePagnet."` p  WHERE p.verrouiller=1 AND p.idClient<>0 AND (p.type=0 || p.type=30)  AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') )
        UNION ALL
        SELECT CONCAT(m.idMutuellePagnet,'+2') as codeMutuelle, m.idMutuellePagnet, m.idClient, m.heurePagnet, m.apayerPagnet FROM `".$nomtableMutuellePagnet."` m WHERE m.verrouiller=1 AND m.idClient<>0 AND (m.type=0 || m.type=30)  AND ((CONCAT(CONCAT(SUBSTR(m.datepagej,7, 10),'',SUBSTR(m.datepagej,3, 4)),'',SUBSTR(m.datepagej,1, 2)) ='".$dateJour."') or (m.datepagej ='".$dateJour."') )
        ) AS a ORDER BY codePanier DESC LIMIT ".$page_position.",".$item_per_page."  ");
      ...
      WHERE p.verrouiller=1 AND p.idClient<>0 AND (p.type=0 || p.type=30) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') )  ");
      $stmt->execute();
      $records = $stmt->fetch();
      $total_rows = $records['allcount'];
      ...
      $stmtOperation = $bdd->prepare("SELECT DISTINCT p.idPagnet, p.heurePagnet, p.totalp, p.apayerPagnet, p.verrouiller, p.idClient FROM `".$nomtablePagnet."` p
      WHERE p.verrouiller=1 AND p.idClient<>0  AND  (p.type=0 || p.type=30) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') ) 
    ...
    foreach ($operations as $vente) {
      ...
      $stmtClient->bindValue(':idClient', $vente['idClient'], PDO::PARAM_INT);
      $stmtClient->execute();
      $client = $stmtClient->fetch(); 
      $cols_client = strtoupper($client["nom"]);

      if (($_SESSION['mutuelle']==1) AND ($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) { 

          $type = explode("+", $vente['codePanier']);
          ...
          if($type[1]==1){
            echo '<div class="panel panel-success">
                  <div class="panel-heading">
                    <h4 style="padding-bottom: 18px;"  class="panel-title expand">
                      ...
                      <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#BonsClient'.$vente['idPagnet'].'" onclick="panier_BonsClient('.$vente['idPagnet'].')">  
                        ...
                        <span class="hidden-xs hidden-sm hidden-md col-lg-2"> '.$cols_client.'</span>
                        <span class="col-xs-5 col-sm-3 col-md-3 col-lg-2 pull-right"><span class="mot_Facture">Facture</span> : '.$vente['idPagnet'].'</span>
                      </a>
                    </h4>
                  </div>
                  <div id="BonsClient'.$vente['idPagnet'].'" class="panel-collapse collapse">
                    <div class="content-panel">
                      ...
                      <button type="submit" class="btn btn-warning pull-right" >
                        ...
                        <table class="table table-hover" id="panier_BonsClient'.$vente['idPagnet'].'">
                          <thead>
                            <tr>
            ...
            echo '<div class="panel panel-info">
                  <div class="panel-heading">
                    <h4 style="padding-bottom: 18px;" class="panel-title expand">
                      <div class="right-arrow pull-right">+</div>
                      <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#BonsMutuelle'.$vente['idPagnet'].'" onclick="panier_BonsMutuelle('.$vente['idPagnet'].')">  
                        <span class="hidden-xs col-sm-2 col-md-2 col-lg-2"><span class="mot_Panier">Panier</span> : '.$cpt.'</span>
                        <span class="hidden-xs col-sm-3 col-md-3 col-lg-2"><span class="mot_Heure">Heure</span> : '.$vente['heurePagnet'].'</span>
                        <span class="col-xs-6 col-sm-3 col-md-3 col-lg-3"> <span class="mot_Total">A Payer Panier</span> <span class="mot_à"></span> <span class="mot_Payer"></span> : '.number_format(($vente['apayerPagnet'] * $_SESSION['devise']), 0, ',', ' ').'</span>
                        <span class="hidden-xs hidden-sm hidden-md col-lg-2"> '.$cols_client.'</span>
                  ...
                  <div id="BonsMutuelle'.$vente['idPagnet'].'" class="panel-collapse collapse">
                    <div class="content-panel">
                      <div class="panel-body">
                      <form class="form-inline pull-right noImpr" style="margin-bottom:10px; margin-right:10px;" method="post" action="barcodeFacture.php" target="_blank" >
                      <input type="hidden" name="idPagnet" id="idPagnet"  value="'.$vente['idPagnet'].'" />
                      <button type="submit" class="btn btn-warning pull-right" >
                        ...
                        <table class="table table-hover" id="panier_BonsMutuelle'.$vente['idPagnet'].'">
                          <thead>
                            <tr>
                              <th><span class="mot_Reference">Reference</span></th>
                              <th><span class="mot_Quantite">Quantite</span></th>
    ...
    if($total_rows>0){
      ...
      echo '<div class="col-md-1"> 
          <select class="form-control pull-left" id="slct_Nbre_BonsClient">
          <optgroup>
