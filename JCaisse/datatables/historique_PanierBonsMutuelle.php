...
try{
    ...
    if ($query =="") {
        ...
        $stmt = $bdd->prepare("SELECT COUNT(*) AS allcount
          FROM
          (SELECT *FROM `".$nomtablePagnet."` p  WHERE p.verrouiller=1 AND p.idClient<>0 AND (p.type=0 || p.type=30)  AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') )
          UNION ALL
          SELECT * FROM `".$nomtableMutuellePagnet."` m WHERE m.verrouiller=1 AND m.idClient<>0 AND (m.type=0 || m.type=30)  AND ((CONCAT(CONCAT(SUBSTR(m.datepagej,7, 10),'',SUBSTR(m.datepagej,3, 4)),'',SUBSTR(m.datepagej,1, 2)) ='".$dateJour."') or (m.datepagej ='".$dateJour."') )
          ) AS a ");
        $stmt->execute();
        $records = $stmt->fetch();
