...
try{
    ...
    if ($query =="") {
        ...
        WHERE p.verrouiller=1 AND p.idClient=0 AND (p.idCompte<>0 && p.idCompte<>1 && p.idCompte<>2 && p.idCompte<>3 && p.idCompte<>1000) AND  (p.type=0 || p.type=30) AND (l.classe=0 || l.classe=1) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') )  ");
        $stmt->execute();
        $records = $stmt->fetch();
        ...
        $stmtOperation = $bdd->prepare("SELECT DISTINCT p.idPagnet, p.heurePagnet, p.totalp, p.apayerPagnet, p.verrouiller, p.idCompte, p.idUser FROM `".$nomtablePagnet."` p
        LEFT JOIN `".$nomtableLigne."` l ON l.idPagnet = p.idPagnet
        WHERE p.verrouiller=1 AND p.idClient=0 AND (p.idCompte<>0 && p.idCompte<>1 && p.idCompte<>2 && p.idCompte<>3 && p.idCompte<>1000) AND  (p.type=0 || p.type=30) AND (l.classe=0 || l.classe=1) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') ) 
    ...
    foreach ($operations as $vente) {
      ...
      $stmtCompte->bindValue(':idCompte', $vente['idCompte'], PDO::PARAM_INT);
      $stmtCompte->execute();
      ...
      echo '<div class="panel panel-success">
            <div class="panel-heading">
              <h4 style="padding-bottom: 18px;" class="panel-title expand">
                ...
                <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#VenteMobile'.$vente['idPagnet'].'" onclick="panier_VenteMobile('.$vente['idPagnet'].')">  
                  ...
                  <span class="hidden-xs hidden-sm hidden-md col-lg-2">'.$cols_compte.' ('.$nom_user.')</span>
                  <span class="col-xs-5 col-sm-3 col-md-3 col-lg-2 pull-right"><span class="mot_Facture">Facture</span> : '.$vente['idPagnet'].'</span>
                </a>
              </h4>
            </div>
            <div id="VenteMobile'.$vente['idPagnet'].'" class="panel-collapse collapse">
              <div class="content-panel">
                ...
                <button type="submit" class="btn btn-warning pull-right" >
                  ...
                  <table class="table table-hover" id="panier_VenteMobile'.$vente['idPagnet'].'">
                    <thead>
                      <tr>
    ...
    if($total_rows>0){
      ...
      echo '<div class="col-md-1"> 
          <select class="form-control pull-left" id="slct_Nbre_VenteMobile">
          <optgroup>
