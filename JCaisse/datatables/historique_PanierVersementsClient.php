...
try{
    ...
    if ($query =="") {
        ...
        WHERE v.idClient<>0 AND ((CONCAT(CONCAT(SUBSTR(v.dateVersement,7, 10),'',SUBSTR(v.dateVersement,3, 4)),'',SUBSTR(v.dateVersement,1, 2)) ='".$dateJour."') or (v.dateVersement ='".$dateJour."')) ");
        $stmt->execute();
        $records = $stmt->fetch();
        ...
        $stmt = $bdd->prepare("SELECT COUNT(*) AS allcount FROM `".$nomtableVersement."` v 
        WHERE (v.montant LIKE :query OR v.dateVersement LIKE :query) AND v.idClient<>0 AND ((CONCAT(CONCAT(SUBSTR(v.dateVersement,7, 10),'',SUBSTR(v.dateVersement,3, 4)),'',SUBSTR(v.dateVersement,1, 2)) ='".$dateJour."') or (v.dateVersement ='".$dateJour."')) ");
        $stmt->execute(array(
          ':query' => '%'.$query.'%'
        ));
        $records = $stmt->fetch();
    ...
    foreach ($operations as $versement) {
      ...
      $stmtClient->bindValue(':idClient', $versement['idClient'], PDO::PARAM_INT);
      $stmtClient->execute();
      ...
      $cols_client = strtoupper($client["nom"]);
        ...
        if($versement['montant']!=0){
          $cols_montant = $versement['montant'];
        }
        else {
          $cols_montant = $versement['montantAvoir'];
        }
            ...
            <div class="panel-heading">
              <h4 style="padding-bottom: 18px;" data-toggle="collapse" data-parent="#accordion" href="#versement'.$versement['idVersement'].'" class="panel-title expand">
                ...
                <a href="#" onclick="detailsVente('.$versement['idVersement'].')">  
                  ...
                  <span class="col-xs-6 col-sm-3 col-md-3 col-lg-3"> Montant : '.number_format(($cols_montant * $_SESSION['devise']), 0, ',', ' ').'</span>
                  <span class="hidden-xs hidden-sm hidden-md col-lg-2"><span class="mot_Total"></span> '.$cols_client.' </span>
    ...
    if($total_rows>0){
      ...
      echo '<div class="col-md-1"> 
          <select class="form-control pull-left" id="slct_Nbre_VersementsClient">
          <optgroup>
