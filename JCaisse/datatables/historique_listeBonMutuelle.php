<?php
   ...
   $stmt = $bdd->prepare("SELECT COUNT(*) AS allcount FROM `".$nomtableLigne."` l
   INNER JOIN `".$nomtableMutuellePagnet."` p ON p.idMutuellePagnet = l.idMutuellePagnet
   WHERE 1 ".$searchQuery." AND p.verrouiller=1 AND (p.type=0 || p.type=30) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."'))   ");
   ...
   INNER JOIN `".$nomtableMutuellePagnet."` p ON p.idMutuellePagnet = l.idMutuellePagnet
   WHERE 1 ".$searchQuery." AND p.verrouiller=1 AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."'))  ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
   // Bind values
   ...
   foreach ($empRecords as $row) {
      ...
      $data[] = array(
         ...
         "forme"=> $row['forme'] ,
         "prixPublic"=> $row['prixPublic'].' (-'.$taux.'%)' ,
         "prixtotal"=>  $row['prixtotal'] - (($row['prixtotal'] * $taux) / 100) ,
