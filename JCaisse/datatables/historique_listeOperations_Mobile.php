<?php
   ...
   foreach ($empRecords as $row) {
      ...
      WHERE p.iduser=:idutilisateur AND (p.idCompte<>0 && p.idCompte<>1 && p.idCompte<>2 && p.idCompte<>3 && p.idCompte<>1000) AND p.verrouiller=1 AND  (p.type=0 || p.type=30) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') )  ");
      ...
      foreach ($lignes as $ligne) {
            if($ligne['idClient']!=0){
               ...
               if($ligne['classe']==0 || $ligne['classe']==1){
                  ...
                  $depenses_Mobiles = $depenses_Mobiles + $ligne['prixtotal'];
               }
      ...
      WHERE p.iduser=:idutilisateur AND (p.idCompte<>0 && p.idCompte<>1 && p.idCompte<>2 && p.idCompte<>3 && p.idCompte<>1000) AND  p.idClient=0 AND p.verrouiller=1 AND (p.type=0 || p.type=30) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."') ) ");
      ...
      WHERE v.iduser=:idutilisateur AND (v.idCompte<>0 && v.idCompte<>1 && v.idCompte<>2 && v.idCompte<>3 && v.idCompte<>1000) AND ((CONCAT(CONCAT(SUBSTR(v.dateVersement,7, 10),'',SUBSTR(v.dateVersement,3, 4)),'',SUBSTR(v.dateVersement,1, 2))  ='".$dateJour."') or (v.dateVersement  ='".$dateJour."') )  ");
      ...
      foreach ($versements as $versement) {
            if($versement['idClient']!=0){
               ...
               $versement_Fournisseurs_Mobile = $versement_Fournisseurs_Mobile + $versement['montant'];
            }
      }
      ...
      if (($_SESSION['mutuelle']==1) AND ($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) { 
         ...
         WHERE p.iduser=:idutilisateur AND (p.idCompte<>0 && p.idCompte<>1 && p.idCompte<>2 && p.idCompte<>3 && p.idCompte<>1000)  AND p.verrouiller=1 AND p.idClient=0 AND (p.type=0 || p.type=30) AND ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) ='".$dateJour."') or (p.datepagej ='".$dateJour."'))  ");
      ...
      $total = ($approvisionnement + $ventes_Mobiles + $versement_Clients_Mobile + $mutuelle_Panier_A_Payer) - 
      ($retrait + $depenses_Mobiles + $versement_Fournisseurs_Mobile + $bons_En_Espesces + $remise_Ventes) ;
      ...
      $data[] = array(
         ...
         "vente"=>  number_format((($ventes_Mobiles + $mutuelle_Panier_A_Payer) * $_SESSION['devise']), 0, ',', ' '),
         "depense"=>  number_format(($depenses_Mobiles * $_SESSION['devise']), 0, ',', ' '),
         "versementClient"=>  number_format(($versement_Clients_Mobile * $_SESSION['devise']), 0, ',', ' '),
         "versementFournisseur"=>  number_format(($versement_Fournisseurs_Mobile * $_SESSION['devise']), 0, ',', ' '),
