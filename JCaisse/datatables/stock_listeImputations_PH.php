<?php
   ...
   $stmt = $bdd->prepare("SELECT *,CONCAT(CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)),'',p.heurePagnet) as idLigne,
      CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4)),'',SUBSTR(p.datepagej,1, 2)) as dateJour, p.idMutuellePagnet as facture
     FROM `".$nomtableLigne."` l  INNER JOIN `".$nomtableMutuellePagnet."` p ON p.idMutuellePagnet = l.idMutuellePagnet
   ...
   foreach ($empRecords as $row) {
      ...
      $mutulelle = $stmtMutuelle->fetch(); 
      $cols_mutuelle = $mutulelle['nomMutuelle'].' ['.$row['taux'].'%]' ; 
      ...
      if($row['datepagej']==$dateString){
         ...
         $data[] = array(
            ...
            "unitevente"=>'<span style="color:#901E06;">'.$row['forme'].'</span>',
            "prixPublic"=>'<span style="color:#901E06;">'.$row['prixPublic'].'</span>',
            "mutuelle"=> '<span style="color:#901E06;">'.$cols_mutuelle .'</span>',
            "facture"=> '<span style="color:#901E06;"> #'.$row['idMutuellePagnet'].'</span>',
            "adherant"=> '<span style="color:#901E06;">'.$row['adherant'].'</span>',
         ...
         $data[] = array(
            ...
            "mutuelle"=>  $cols_mutuelle ,
            "facture"=> '#'.$row['idMutuellePagnet'] ,
            "adherant"=> $row['adherant'],
