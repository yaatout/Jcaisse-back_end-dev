...
if (isset($_POST['btnSavePagnetVente'])) {
    ...
    if(mysql_num_rows($res1)){
	   ...
	   if(mysql_num_rows($res2)){
	          $sql6="insert into `".$nomtablePagnet."` (datepagej,heurePagnet) values('".$dateString2."','".$heureString."')";
...
if (isset($_POST['btnAnnulerPagnet']) || isset($_POST['btnRetournerPagnet'])) {
	...
	//selection de toutes les lignes du panier � annuler ou � retourner
...
$sql2="SELECT * FROM `".$nomtablePagnet."` where idClient=0 && datepagej ='".$dateString."' ORDER BY idPagnet DESC";
                    ...
                    <div class="panel panel-info" <?php echo  "id=panel".$pagnet['idPagnet']."" ; ?>>
                      <div class="panel-heading">
                        ...
                        <a class="accordion-toggle " <?php echo  "href=#item".$pagnet['idPagnet']."" ; ?>  onclick='display("<?php echo  $pagnet['idPagnet'] ; ?>");'
                          ...
                          > Panier <?php echo $pagnet['idPagnet']; ?>
                          ...
                          <?php
                            if ($pagnet['verrouiller']==0) {  ?>
                              <form  class="form-inline pull-left noImpr" id="ajouterProdForm" method="post" >
							   ...
							   <input type="text" class="inputbasic" name="codeBarre" id="codeBarreLigne" autofocus="" pattern="([0-9]{1,5}-){1,3}([0-9]{1,5}){1}" required />
...
<form class="form-inline pull-right noImpr" style="margin-right:20px;" id="factForm" method="post">
    ...
    <button type="submit" name="btnImprimerFacture" <?php echo  "id=btnImprimerFacture".$pagnet['idPagnet']."" ; ?> class="btn btn-success pull-right" data-toggle="modal" onclick='remiseB("<?php echo  $pagnet['idPagnet'] ; ?>");' ><span class="glyphicon glyphicon-lock"></span>Terminer</button>
</form>
<?php else: ?>
<form class="form-inline noImpr" id="factForm" method="post">
	<input type="hidden" name="idPagnet" id="idPagnet"  <?php echo  "value=".$pagnet['idPagnet']."" ; ?>>
	...
	<button class="btn btn-success  pull-right" style="margin-right:20px;" onclick="document.getElementById('barcode').submit();">Facture</button><br>
	...
	while ($ligne = mysql_fetch_array($res8)) {?>
	  <tr>
		...
		<td> <?php if ($pagnet['verrouiller']==0): ?>
				...
				<input class="quantite" disabled="" size="3" type="number" <?php echo  "id=quantite".$ligne['numligne'].""; ?>  <?php echo  "value=".$ligne['quantite'].""; ?>
		...
		<td class="prixunitevente" <?php echo  "id=prixunitevente".$ligne['numligne'].""; ?> >
		<?php if ($pagnet['verrouiller']==0): ?>
		<input class="prixunitevente" disabled="" size="5" type="number" <?php echo  "id=prixunitevente".$ligne['numligne'].""; ?>  <?php echo  "value=".$ligne['prixunitevente'].""; ?> onclick="masLigne(<?php echo $ligne['numligne']  ; ?>, <?php echo $pagnet['idPagnet'] ?> );" > 
			<?php else: ?>
		...
		<td>
						    ...
						    <div>
									...
									<div>
										<?php if ($pagnet['verrouiller']==0): ?>
											<input class="noImpr" disabled="" size="7" type="number" placeholder="remise" name="remise" <?php echo  "id=remise".$pagnet['idPagnet'].""; ?>>
									...
									<div>
										<?php if ($pagnet['verrouiller']==0): ?>
											...
											<input class="noImpr" disabled="" size="7" type="number" placeholder="Espéce" name="versement" <?php echo  "id=versement".$pagnet['idPagnet'].""; ?>>
											<?php else: ?>
