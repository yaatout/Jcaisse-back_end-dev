...
where dateStockage='".$dateString."' AND quantiteStockCourant!=0 order by idStock desc";
$res=mysql_query($sql);

$sql="SELECT * from `".$nomtableStock."` 
where dateStockage!=0 order by idStock desc";
$res=mysql_query($sql);
...
while($stock=mysql_fetch_array($res)){
	...
	VALUES('.$stock['idStock'].',3,'.$stock['idDesignation'].',"'.mysql_real_escape_string($stock["designation"]).'",'.$stock['quantiteStockinitial'].',"'.mysql_real_escape_string($stock['uniteStock']).'",'.$stock['nbreArticleUniteStock'].','.$stock['totalArticleStock'].',"'.$dateString.'",'.$stock['totalArticleStock'].')';
...
if (isset($_POST['btnSaveFrais'])) {
    ...
    if(mysql_num_rows($res1)){
	   ...
	   if(mysql_num_rows($res2)){
	          $sql6="insert into `".$nomtablePagnet."` (datepagej,type) values('".$dateString2."',2)";
						var_dump($sql6);
              $res6=mysql_query($sql6) or die ("insertion pagnier impossible 61 =>".mysql_error());
    	 }
    	 else{
    	     $sql1="insert into `".$nomtablePage."` (datepage) values('".$dateString2."')";
             $res1=@mysql_query($sql1) or die ("insertion page journal impossible-1".mysql_error());
			 ...
			 $sql6="insert into `".$nomtablePagnet."` (datepagej,type) values('".$dateString2."',2)";
             $res6=mysql_query($sql6) or die ("insertion pagnier impossible 62=>".mysql_error() );
    	 }
	}
	else{
	   $sql1="insert into `".$nomtableJournal."` (mois,annee) values(".$mois.",".$annee.")";
	   ...
	   $sql6="insert into `".$nomtablePagnet."` (datepagej,type) values('".$dateString2."',2)";
	   $res6=mysql_query($sql6) or die ("insertion pagnier impossible 63=>".mysql_error() );
	}
}
...
if (isset($_POST['btnEnregistrerCodeBarre'])) {
	...
	if (isset($_POST['codeBarre']) && isset($_POST['idPagnet'])) {
		 ...
		 //echo $sql4;
			 ...
			 $stock = mysql_fetch_array($res4) or die ("select stock impossible 7=>".mysql_error());
			 ...
			 $res6=mysql_query($sql6) or die ("select stock impossible 6=>".mysql_error());
					 ...
					 $numero=$codeBrute[2];
						 if ( $numero==2) { // PAquet, douzaine, ....
							 ...
							 if ($quantiteCourant>=0){
								 ...
								 $res14=mysql_query($sql14) or die ("select stock impossible 5=>".mysql_error());
						 ...
						 {  // Article
							 ...
							 $res5=mysql_query($sql5) or die ("update quantiteStockCourant impossible =>".mysql_error());
												 ...
												 $sql14="SELECT totalp FROM `".$nomtablePagnet."` where idPagnet=".$idPagnet;
							 $res14=mysql_query($sql14) or die ("select stock impossible 4 =>".mysql_error());
							 $pagnet = mysql_fetch_array($res14);
			 ...
			 elseif ($tailleTableau==4) {
						 ...
						 $res5=mysql_query($sql5) or die ("update quantiteStockCourant impossible =>".mysql_error());
						 $sql14="SELECT totalp FROM `".$nomtablePagnet."` where idPagnet=".$idPagnet;
						 $res14=mysql_query($sql14) or die ("select stock impossible 3=>".mysql_error());
						 $pagnet = mysql_fetch_array($res14);
						 $totalp=$pagnet['totalp']+$stock['prixunitaire'];
			 }
			 //$totalp=$Total[0];
		...
		if ($tailleTableau==2) {
			  ...
			  $res17=mysql_query($sql17) or die ("select stock impossible 1 =>".mysql_error());
				...
				$sql18="SELECT totalp,type FROM `".$nomtablePagnet."` where idPagnet=".$idPagnet;
...
if (isset($_POST['btnEnregistrerCodeBarreFrais'])) {

	if (isset($_POST['codeBarreFrais']) && isset($_POST['idPagnet'])) {
		 $codeBarre=htmlspecialchars(trim($_POST['codeBarreFrais']));
		 $idPagnet=htmlspecialchars(trim($_POST['idPagnet']));
		 $codeBrute=explode('-', $codeBarre);
		 $idStock=$codeBrute[0];
		 $idDesignation=$codeBrute[1];
...
if (isset($_POST['btnImprimerFactureFr'])) {
	if(isset($_POST['idPagnet'])){
...
while ($pagnet0 = mysql_fetch_array($res2))
  ...
  <form name="formulairePagnet" method="post" >
      <button type="submit" class="btn btn-success noImpr" name="btnSavePagnetVente">
			...
			<button type="submit" class="btn btn-danger noImpr" name="btnSaveFrais">
                      <i class="glyphicon glyphicon-plus"></i>Ajouter un Frais
      </button><br><br>
  </form>
<div id="monaccordeon" class="panel-group ">
			  ...
			  $date = new DateTime();
			    ...
			    $sql2="SELECT * FROM `".$nomtablePagnet."` where idClient=0  && type!=2 && datepagej ='".$dateString2."' ORDER BY idPagnet DESC";
                $res2 = mysql_query($sql2) or die ("persoonel requête 2".mysql_error());
                    ...
                    <div class="panel panel-info" <?php echo  "id=panel".$pagnet['idPagnet']."" ; ?>>
                      <div class="panel-heading">
                        ...
                        <a class="accordion-toggle " <?php echo  'href="#item'.$pagnet['idPagnet'].'"' ; ?>
													  onclick='display(<?php echo  $pagnet['idPagnet'] ; ?>)'
                        ...
                        <div class="panel-body" >
                          ...
                          <table class="table ">
                                              ...
                                              while ($ligne = mysql_fetch_array($res8)) {?>
                                                  <tr>
										            ...
										            <td> <?php if ($pagnet['verrouiller']==0): ?>
													        <input class="quantite" size="3" type="number" <?php echo  "id=quantite".$ligne['numligne'].""; ?>  <?php echo  "value=".$ligne['quantite'].""; ?>
						    ...
						    <div>
									...
									<div>
										<?php if ($pagnet['verrouiller']==0): ?>
											<input class="noImpr" size="7" type="number" placeholder="remise" name="remise" <?php echo  "id=remise".$pagnet['idPagnet'].""; ?>>
									...
									<div>
										<?php if ($pagnet['verrouiller']==0): ?>
											<!--<input size="7" type="number" placeholder="Espéce " name="apayerPagnet" <?php echo  "id=apayerPagnet".$pagnet['idPagnet'].""; ?>> -->
											<input class="noImpr" size="7" type="number" placeholder="Espéce" name="versement" <?php echo  "id=versement".$pagnet['idPagnet'].""; ?>>
											<?php else: ?>
				 ...
				 $sql2="SELECT * FROM `".$nomtablePagnet."` where idClient=0 && type=2 && datepagej ='".$dateString2."' ORDER BY idPagnet DESC";
								$res2 = mysql_query($sql2) or die ("persoonel requête 2".mysql_error());
									while ($pagnet = mysql_fetch_array($res2)) {   ?>
										...
										<div class="panel panel-danger" <?php echo  "id=panel".$pagnet['idPagnet']."" ; ?>>
											<div class="panel-heading">

												<a class="accordion-toggle " <?php echo  'href="#item'.$pagnet['idPagnet'].'"' ; ?>

													dataparent="#monaccordeon2"  data-toggle="collapse"
													> Pagnet Frais: <?php echo $pagnet['idPagnet']; ?>
											...
											<?php if ($pagnet['verrouiller']==0)  { ?> class="panel-collapse collapse in" <?php }
												...
												<div class="panel-body" >
													<?php
														if ($pagnet['verrouiller']==0) {  ?>
															<form  class="form-inline pull-left noImpr"  method="post" >
																<input type="text" class="inputbasic" name="codeBarreFrais"  autofocus="" >
																<input type="hidden" name="idPagnet" id="idPagnet"
																 <?php echo  "value=".$pagnet['idPagnet']."" ; ?> >
															 <button type="submit" name="btnEnregistrerCodeBarreFrais"
																 class="btn btn-danger btnxs " ><span class="glyphicon glyphicon-plus" ></span>
															Ajouter </button><div id="reponseS"></div>
															</form>
															<?php
														}
												 ...
												 <?php if ($pagnet['verrouiller']==0): ?>
													 ...
													 <form class="form-inline pull-right noImpr" style="margin-right:20px;" id="factFormFr" method="post"  >
																					 <input type="hidden" name="idPagnet"   <?php echo  "value=".$pagnet['idPagnet']."" ; ?>>
																					 <input type="hidden" name="totalp" <?php echo  "id=totalpFr".$pagnet['idPagnet']."" ; ?> <?php echo  "value=".$pagnet['totalp']."" ; ?> >

																					 <button type="submit" name="btnImprimerFactureFr" <?php echo  "id=btnImprimerFactureFr".$pagnet['idPagnet']."" ; ?>
																							class="btn btn-success pull-right" data-toggle="modal" >
																							<span class="glyphicon glyphicon-lock"></span>Terminer
																					 </button>
														</form>
