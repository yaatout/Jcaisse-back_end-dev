...
$(document).ready(function () {
  ...
  $(document).on("click", ".btnRetourAvant", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        //************Debut supprimer ligne offline *********************/

        if (sessionStorage.iduserSession) {
          var lignes = JSON.parse(localStorage.getItem("Ligne")) || [];
          searchLignes = lignes.filter((l) => l.numligne != numligne);

          localStorage.setItem("Ligne", JSON.stringify(searchLignes));
        }

        //************Fin supprimer ligne offline *********************/
      },
  ...
  $(document).on("click", ".btnRetournerPagnet", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        //************Debut Retourner panier offline *********************/

        if (sessionStorage.iduserSession) {
          var paniers = JSON.parse(localStorage.getItem("Panier")) || [];
          //searchPaniers=paniers.filter(p =>p.idPagnet!=idPanier);
          ...
          // var lignes = JSON.parse(localStorage.getItem('ligne')) || [];
          // searchLignes=lignes.filter(l =>l.idPagnet!=idPanier);
          // localStorage.setItem("ligne", JSON.stringify(searchLignes));
        }

        //************Fin Retourner panier offline *********************/
      },
  ...
  $(document).on("click", ".btn_Termine_Panier", function (e) {
    ...
    //*********** Debut modifier compte avance offline *****************/

    idcompteAvance = $("#compteAvance" + idPanier).val();

    //localStorage.setItem("idCompte2", idcompteAvance);
    ...
    if (searchPanier != undefined) {
      ...
      searchPanier.idCompteAvance = idcompteAvance;
    }
    savePanier(paniers);

    //***********Fin modifier compte avance offline *****************/
      ...
      if (compte == "2" && clientInput == "") {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            /*************************** Debut terminer panier offline ****************************/

            if (sessionStorage.iduserSession) {
              $.ajax({
                url: "offline/ajax/panierOnLineToOffLineAjax.php",
                ...
                success: function (data) {
                  ...
                  if (result == null) {
                    ...
                    var paniers = JSON.parse(localStorage.getItem("Panier"));
                    var searchPanier = paniers.find(
                      (p) => p.idPagnet == result.idPagnet
                    );

                    if (searchPanier != undefined) {
                      searchPanier.verrouiller = result.verrouiller;

                      searchPanier.totalp = result.totalp;

                      searchPanier.apayerPagnet = result.apayerPagnet;

                      searchPanier.remise = result.remise;

                      searchPanier.versement = result.versement;

                      searchPanier.identifiantP = result.identifiantP;

                      searchPanier.restourne = result.restourne;

                      searchPanier.dejaTerminer = result.dejaTerminer;

                      searchPanier.idClient = result.idClient;

                      searchPanier.idCompte = result.idCompte;

                      searchPanier.idcompteAvance = result.idcompteAvance;

                      searchPanier.type = result.type;

                      searchPanier.avance = result.avance;
                    ...
                    if (localStorage.IdPanier) {
                      id = localStorage.getItem("IdPanier");
                      var searchPanier1 = paniers.find((p) => p.idPagnet == id);

                      if (searchPanier1 != undefined) {
                        searchPanier1.verrouiller = 1;

                        searchPanier1.totalp = result.totalp;

                        searchPanier1.apayerPagnet = result.apayerPagnet;

                        searchPanier1.remise = result.remise;

                        searchPanier1.versement = result.versement;

                        searchPanier1.identifiantP = result.identifiantP;

                        searchPanier1.restourne = result.restourne;

                        searchPanier1.dejaTerminer = result.dejaTerminer;

                        searchPanier1.idClient = result.idClient;

                        searchPanier1.idCompte = result.idCompte;

                        searchPanier1.idcompteAvance = result.idcompteAvance;

                        searchPanier1.type = result.type;

                        searchPanier1.avance = result.avance;

                        //searchPanier.online=2;
                      } else {
                        //alert("Erreur");
                      }
                      //savePanier(paniers);
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if (result == null) {
                ...
                result.offline = 2;
                result.synchronise = 0;
                //result.idcompteAvance=1;
                paniers.push(result);
                savePanier(paniers);
              }
            },
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      /*    
            ...
            */
      //************Debut modifier unite stock offline *********************/
      if (sessionStorage.iduserSession) {
        var lignes = JSON.parse(localStorage.getItem("Ligne")) || [];

        var searchLigne = lignes.find(
          (l) => l.numligne == ligne && l.idPagnet == pagnet
        );

        if (searchLigne) {
          searchLigne.prixunitevente = tab1[0];
          searchLigne.prixtotal =
            searchLigne.prixunitevente * searchLigne.quantite;
          searchLigne.unitevente = unitevente;

          localStorage.setItem("Ligne", JSON.stringify(lignes));
        }
      }
      //************Fin modifier modifier unite stock offline *************/
    ...
    if (query.length > 0) {
      ...
      if (keycode == "13") {
        ...
        $.ajax({
          ...
          success: function (data) {
            result = data.split("<>");
            //******************* Debut création ligne Offline **************** */

            var ligneOf = new Object();
            ligneOf.numligne = result[5];
            ligneOf.idPagnet = idPanier;
            ligneOf.idDesignation = result[1];
            ligneOf.designation = result[2];
            ligneOf.idStock = 0;
            ligneOf.unitevente = result[3];
            ligneOf.prixunitevente = result[4];
            ligneOf.quantite = 1;
            ligneOf.prixtotal = ligneOf.quantite * result[4];
            ligneOf.classe = result[9];
            ligneOf.synchronise = 0;
            ligneOf.prixtotalTvaP = 0;
            ligneOf.prixtotalTvaR = 0;
            var lignes = JSON.parse(localStorage.getItem("Ligne")) || [];
            ...
            if (result[0] == 1) {
              ...
              //******************* Début ajouter ligne Offline **************** */

              lignes.push(ligneOf);
              localStorage.setItem("Ligne", JSON.stringify(lignes));

              //******************* Fin ajouter ligne Offline **************** */
            }
            ...
            if (result[0] == 2) {
              $("#tablePanier" + idPanier + " tr").each(function (row, tr) {
                ...
                if (reference != "" && reference == result[2]) {
                  ...
                  //******************* Debut ajouter ligne Offline **************** */

                  searchLigne = lignes.find(
                    (l) =>
                      l.designation == ligneOf.designation &&
                      l.idPagnet == ligneOf.idPagnet
                  );
                  ...
                  if (searchLigne) {
                    ...
                    searchLigne.prixtotal =
                      searchLigne.quantite * searchLigne.prixunitevente;
    ...
    success: function (data) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          /**Debut modifier la quantite dans la ligne offline d'un Pagnet**/
          var lignes = JSON.parse(localStorage.getItem("Ligne")) || [];
          searchLigne = lignes.find((l) => l.numligne == ligne);
          if (searchLigne) {
          ...
          /**Fin modifier la quantite dans la ligne offline d'un Pagnet**/
...
function modif_Prix(prix, ligne, pagnet) {
  $.ajax({
    ...
    success: function (data) {
      ...
      /**Debut modifier le prix dans la ligne offline d'un Pagnet**/
      var lignes = JSON.parse(localStorage.getItem("Ligne")) || [];
      var searchLigne = lignes.find((l) => l.numligne == ligne);

      if (searchLigne) {
        searchLigne.prixunitevente = prix;
        searchLigne.prixtotal = searchLigne.quantite * prix;
        searchLigne.setItem("Ligne", JSON.stringify(lignes));
      }

      /**Fin modifier le prix dans la ligne offline d'un Pagnet**/
...
function init_Produit(id) {
  ...
  var prixcommercant = $("#prixcommercant-" + id).val();
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#prixcommercant-" + id).prop("disabled", true);
...
$(function () {
  $("#btn_init_Produit").click(function () {
    ...
    var prixcommercant = $("#prixcommercant").val();
...
$(function () {
  $(".codeBarreLigneMutuelle").keyup(function (e) {
    ...
    var tabidMutuellePagnet = $(this).attr("id");

    tab = tabidMutuellePagnet.split("_");
    ...
    if (query.length > 0) {
      ...
      if (keycode == "13") {
        // var designation = $("#ajouterProdFormPh"+idMutuellePagnet+" .typeahead li.active").text();
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (result[0] == 1) {
              ...
              // $(".codeBarreLigneMutuelle").val('valueeeeeeeeee');
              ...
              $("#somme_Apayer" + idMutuellePagnet).text(
                result[6] -
                  $("#val_remise" + idMutuellePagnet).val() -
                  (result[6] * result[7]) / 100
              );
            }
...
//     numContainerChanged = $("#numContainerChanged"+id).val();

//     if (numContainerChanged=="" || numContainerChanged==null) {

//         confirm("Mettez le numéro de container avant de valider.");

//     } else {

//         // $("#numContainerChangedValidate"+id).prop('disabled', true);
...
//                 numContainerChanged: numContainerChanged,
...
//                     $("#numContainerdetail"+id).text('Container : '+numContainerChanged);
...
function changeNumContainer(id) {
  ...
  c = numContainerChanged.split(" . ");
  idContainer = c[0];
  numContainer = c[1];

  if (
    numContainerChanged == "" ||
    numContainerChanged == null ||
    numContainerChanged.indexOf(" . ") < 0
  ) {
    confirm(
    ...
    $.ajax({
      ...
      data: {
        ...
        numContainerChanged: numContainer,
...
$(function () {
  $(document).on("keyup", "#client_cbm", function (e) {
    e.preventDefault();
    // alert(1)
    // idPanier = $(this).attr('data-idPanier');

    var query = $(this).val();
    $("#numContainer").css("background", "white");

    if (query.length > 0) {
      $("#client_cbm").typeahead({
        source: function (query, result) {
          $.ajax({
            url: "ajax/vendreLigneAjax.php",
...
function validerCbm() {
  Client_text = $("#client_cbm").val().split(" . ");
  idClient = Client_text[0];
  qty_cbm = $("#qty_cbm").val();
  qty_bal = $("#qty_bal").val();
  numContainer = $("#numContainer").val().split(" . ")[0];
  nbPcsInContainer = $("#nbPcsInContainer").val();
  $("#numContainer").css("background", "white");

  // alert($("#client_cbm").val().indexOf(' . '));
  if (numContainer && numContainer !== "" && numContainer !== null) {
    if (
      $("#client_cbm").val().indexOf(" . ") > 0 &&
      $("#numContainer").val().indexOf(" . ") > 0
    ) {
      if (
        (qty_cbm == 0 || qty_cbm == null) &&
        (qty_bal == 0 || qty_bal == null)
      ) {
        confirm("Erreur : CBM et BAL sont tous à 0.");
      } else {
        $.ajax({
          url: "ajax/operationPanierAjax.php",
          ...
          data: {
            cbmManager: "cbmManager",

            idClient: idClient,

            numContainer: numContainer,

            nbPcsInContainer: nbPcsInContainer,
    ...
    $("#numContainer").css("background", "red");
...
$(function () {
  $(document).on("keyup", ".numContainerChanged", function (e) {
    e.preventDefault();
    // idPanier = $(this).attr('data-idPanier');
      ...
      $(this).typeahead({
        source: function (query, result) {
          $.ajax({
            ...
            data: {
              numContainerChange: "numContainerChange",
...
function confimrEditContainer() {
  ...
  $.ajax({
    ...
    data: {
      confimrEditContainer: "confimrEditContainer",
      idContainer: idContainer,
      numContainer: numContainer,
...
function confimrDeleteContainer() {
  ...
  $.ajax({
    ...
    data: {
      confimrDeleteContainer: "confimrDeleteContainer",
