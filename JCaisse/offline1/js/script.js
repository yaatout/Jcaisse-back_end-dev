...
$(document).ready(function() {
    ...
    $(document).on("click", ".btnAnnulerPagnet", function(e) {  
        ...
        searchPanier=paniers.find(p=>p.idPagnet==idPanier);
...
function savePanier(pagnet){
    ...
    localStorage.setItem("paniers_"+iduser+"", JSON.stringify(pagnet)); 
    //$("#idPanier").load("container.html");   
}

function saveLigne(ligne){
    iduser=localStorage.getItem('Iduser')
    localStorage.setItem("lignes_"+iduser+"", JSON.stringify(ligne));
...
function getPagnet(){
    ...
    // datejour=date.toLocaleDateString("es-CL");
    iduser=localStorage.getItem('Iduser')
    const panier= localStorage.getItem("paniers_"+iduser+"");
    if (panier==null) {  
        return [];
    } 
...
function getLigne(){    
    ...
    // datejour=date.toLocaleDateString("es-CL");
    iduser=localStorage.getItem('Iduser')   
    const ligne= localStorage.getItem("lignes_"+iduser+"");
    if (ligne==null) {  
        return [];
    }
...
function addPagnet(){
    ...
    if(i<=2  && users!=null){
        ...
        Panier.identifiantP=tabDate[0]+''+tabDate[1]+''+tabDate[2]+''+tabHeure[0]+''+tabHeure[1]+''+tabHeure[2];
        ...
        Panier.idClient =0;
        Panier.idVitrine =0;
        compte=comptes.find(c =>c.nomCompte=='Caisse' || c.nomCompte=='caisse')
                            ...
                            '<div class="col-md-8 col-sm-8 col-xs-12 content2">'+
                                ...
                                '<form class="form-inline noImpr" id="factForm" method="post">'+
                                    ...
                                    '<input type="number" min="0" oninput="this.value = !!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null" id="val_remise1'+Panier.idPagnet+'" onkeyup="remise_Panier(this.value,' +Panier.idPagnet+')" name="remise"  class="remise form-control col-md-2 col-sm-2 col-xs-3" placeholder="Remise....">'+
                                        ...
                                        '<select class="compte '+Panier.idPagnet+' compteCaisse form-control col-md-2 col-sm-2 col-xs-3" onchange="modif_CompteCaisse(this.value,'+Panier.idPagnet+')" data-idPanier='+Panier.idPagnet+' name="compte'+Panier.idPagnet+'"  id="compteCaisse'+Panier.idPagnet+'">'+
                                        ...
                                        '<input type="number" style="display:none;" class="avanceInput form-control col-md-2 col-sm-2 col-xs-3"  data-idPanier="'+Panier.idPagnet+'" name="avanceInput" id="avanceInput'+Panier.idPagnet+'" autocomplete="off" placeholder="Avance"/>'+

                                        '<select class="compte compteBon form-control col-md-2 col-sm-2 col-xs-3" style="display:none;" onchange="modif_CompteBon(this.value,'+Panier.idPagnet+')"  name="compte"  id="compteBon'+Panier.idPagnet+'">'+
...
function addLigne(idPanier){
        ...
        for(var i=0; i<design.length;i++){
            ...
            if(design[i].designation==designation){
                ...
                if(searchLigne != undefined){
                    ...
                    ligne.identifiantL=tabDate[0]+''+tabDate[1]+''+tabDate[2]+''+tabHeure[0]+''+tabHeure[1]+''+tabHeure[2];
    ...
    $("#somme_Apayer" + idPanier).text(somme_A_payer(idPanier) - remise_Panier(remise,idPanier));
...
function modif_UniteStock(unitevente,numligne,nbreArticleUniteStk) {
    ...
    if(panier!=undefined){
        ...
        $("#somme_Total" + searchLigne.idPagnet).text(somme_A_payer(searchLigne.idPagnet));
        $("#somme_Apayer" + searchLigne.idPagnet).text(somme_A_payer(searchLigne.idPagnet)-remise);
                    ...
                    success: function(data) {
                        ...
                        // else{ 
                            ...
                            $.ajax({
                                ...
                                url: "ajax/insertionLigneAjax.php",
                                ...
                                success: function(data) {
                                    ...
                                    if(resultat==0){
                                        ...
                                        if(resultat==nbreLignes){
                                            paniers=getPagnet();
                                            panierE= paniers.find(p =>p.idPagnet==idPanier);

                                            //lignesLocale=getLigne();
                                            ...
                                            if (panierE!=undefined) {
                                                ...
                                                // lesLignes= lignesLocale.filter(l =>l.idPagnet==idPanier);
                                                // for(i=0;i<lesLignes.length;i++){
                                                //     lesLignes[i].synchronise=1
                                                //     saveLigne(lignesLocale)
                                                // }
