    ...
    $(document).on("click", ".btn_Termine_Panier", function(e) { 
       ...
       // alert(versement);
        ...
        if(searchPanier != undefined){
            ...
            searchPanier.identifiantP=tabDate[0]+''+tabDate[1]+''+tabDate[2]+''+tabHeure[0]+''+tabHeure[1]+''+tabHeure[2]+''+searchPanier.iduser; 
...
function savePanier(pagnet){
    ...
    localStorage.setItem("paniers", JSON.stringify(pagnet)); 
...
function getPagnet(){
    ...
    const pagnier= localStorage.getItem("paniers");
    if (pagnier==null) {  
        return [];
    } 
...
function getLigne(){ 
    ...
    datejour=date.toLocaleDateString("es-CL");
    iduser=localStorage.getItem('Iduser')   
    const ligne= localStorage.getItem("lignes");
    if (ligne==null) {  
        return [];
    }
...
function addPagnet(){
    ...
    if(i<=2  && users!=null){
        ...
        Panier.identifiantP=tabDate[0]+''+tabDate[1]+''+tabDate[2]+''+tabHeure[0]+''+tabHeure[1]+''+tabHeure[2]+''+sessionStorage.iduserSession;
        ...
        var p= '<div class="panel panel-primary" id=panelPanier'+Panier.idPagnet+'>'+

                '<div class="panel-heading">'+

                    '<h4 data-toggle="collapse" data-parent="#accordion" href="#pagnet'+Panier.idPagnet+'"  class="panel-title expand">'+

                        '<div class="right-arrow pull-right">+</div>'+

                        '<a class="row" href="#">'+ 

                            '<span class="noImpr col-md-2 col-sm-2 col-xs-12" id="panierEncours'+Panier.idPagnet+'"> Panier : En cours... </span>'+

                            '<span class="noImpr col-md-3 col-sm-3 col-xs-12">Heure: '+Panier.heurePagnet+'</span>'+

                            '<span class="noImpr col-md-3 col-sm-3 col-xs-12" >Total panier: <span id="somme_Total'+Panier.idPagnet+'" > '+Panier.totalp+' </span></span>'+

                            '<span class="noImpr col-md-3 col-sm-3 col-xs-12" >Total A  payer: <span id="somme_Apayer'+Panier.idPagnet+'" >'+(Panier.apayerPagnet-Panier.remise)+'</span></span>'+                 
                ...
                '</div>'+
                '<div id="pagnet'+Panier.idPagnet+'" class="panel-collapse collapse in panelVente" > '+

                    '<div class="panel-body" >'+

                        '<div class="cache_btn_Terminer row" id="cache_btn_Terminer'+Panier.idPagnet+'">'+

                            //<!--*******************************Debut Ajouter Ligne****************************************-->

                                '<form  class="form-inline noImpr ajouterProdForm col-md-4 col-sm-4 col-xs-12" id="ajouterProdFormB'+Panier.idPagnet+'" onsubmit="return false" >'+

                                    '<input type="text" class="inputbasic form-control codeBarreLigneVt" name="codeBarre" id="panier_'+Panier.idPagnet+'" data-idPanier='+Panier.idPagnet+' style="width:100%;" autofocus="" autocomplete="off"  required />'+
                            ...
                            '<div class="col-md-8 col-sm-8 col-xs-12 content2">'+
                                ...
                                '<form class="form-inline noImpr" id="factForm" method="post">'+
                                    ...
                                    '<input type="number" min="0" oninput="this.value = !!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null" id="val_remise1'+Panier.idPagnet+'" onkeyup="remise_Panier(this.value,' +Panier.idPagnet+')" name="remise"  class="remise form-control col-md-2 col-sm-2 col-xs-3" placeholder="Remise....">'+

                                        
                                        '<select class="compte '+Panier.idPagnet+' compteCaisse form-control col-md-2 col-sm-2 col-xs-3" style="display:none;" onchange="modif_CompteCaisse(this.value,'+Panier.idPagnet+')" data-idPanier='+Panier.idPagnet+' name="compte'+Panier.idPagnet+'"  id="compteCaisse'+Panier.idPagnet+'">'+
                                        
                                        '</select>'+

                                        '<input type="number" min="0" oninput="this.value = !!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null" name="versement" id="versement'+Panier.idPagnet+'" onkeyup="modif_Versement(this.value,'+Panier.idPagnet+')"  placeholder="Espèce..."'+
                                        'class="versement form-control col-md-2 col-sm-2 col-xs-3" style="display:none;">'+

                                        '<input type="text" class="client clientInput form-control col-md-2 col-sm-2 col-xs-3" style="display:none;"  data-type="simple" data-idPanier="'+Panier.idPagnet+'" name="clientInput" id="clientInput'+Panier.idPagnet+'" autocomplete="off" placeholder="Client"/>'+

                                        '<input type="number"  class="avanceInput form-control col-md-2 col-sm-2 col-xs-3"  data-idPanier="'+Panier.idPagnet+'" name="avanceInput" id="avanceInput'+Panier.idPagnet+'" autocomplete="off" placeholder="Avance"/>'+

                                        '<select class="compte compteBon form-control col-md-2 col-sm-2 col-xs-3" onchange="modif_CompteBon(this.value,'+Panier.idPagnet+')"  name="compte"  id="compteBon'+Panier.idPagnet+'">'+
                                            
                                        '</select>'+    

                                    '<input type="hidden" name="idPagnet"   value='+Panier.idPagnet+'>'+

                                    '<input type="hidden" name="totalp" id=totalp'+Panier.idPagnet+' value="0" >'+

                                    '<button type="button" style="" disabled="true" name="btnImprimerFacture" data-idPanier='+Panier.idPagnet+' id=btnImprimerFacture'+Panier.idPagnet+' class="btn btn-success btn_Termine_Panier btn_Termine_Panier'+Panier.idPagnet+' terminer" data-toggle="modal" ><span class="glyphicon glyphicon-ok"></span>'+

                                        '<img src="images/loading-gif3.gif" class="img-load-terminer" style="height: 30px;width: 30px;display:none;" alt="GIF" srcset=""></button>'+

                                    '<button tabindex="1" type="button" class="btn btn-danger annuler" data-toggle="modal" data-target=#msg_ann_pagnet'+Panier.idPagnet+'>'+
                                        ...
                                        '<div class="modal-content">'+
                                            ...
                                            '<form class="form-inline noImpr"  id="factForm" method="post"  >'+
                                                ...
                                                '<div class="modal-footer">'+
                                                    ...
                                                    '<button type="button" name="btnAnnulerPagnet" data-idPanier='+Panier.idPagnet+' class="btn btn-success btnAnnulerPagnet">Confirmer<img src="images/loading-gif3.gif" class="img-load-annulerPanier" style="height: 30px;width: 30px; display:none;" alt="GIF" srcset=""></button>'+
                        ...
                        '<table  id=tablePanier'+Panier.idPagnet+' class="tabPanier table"  width="100%" >'+
        ...
        for(var i=0; i<design.length;i++){
            ...
            if(design[i].designation==designation){
                ...
                if(searchLigne != undefined){
                    ...
                    $("#ligneQuantite"+idPanier+""+searchLigne.numligne).val(searchLigne.quantite);
                    saveLigne(lignes);
                    ...
                    if(design[i].uniteStock!="Article" && design[i].uniteStock!="article"){
                        ...
                        var lignePanier = "<tr>" +
                            ...
                            "<td><input type='number' min='1' oninput='this.value = !!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null' class='form-control filedReadonly"+idPanier+"' id='ligneQuantite"+idPanier+""+ligne.numligne+"' onkeyup='modif_Quantite(this.value," + ligne.numligne + " )' value='1' style='width: 30%' type='number' ></input></td>" +
                            "<input type='hidden' id='nbreArticleUniteStock' value='"+design[i].nbreArticleUniteStock+"'/>"+
                            "<td>" +
                    
                                "<select class='form-control filedReadonly"+idPanier+"' id='uniteVente"+idPanier+""+ligne.numligne+"'  onchange='modif_UniteStock(this.value," + ligne.numligne + "," + design[i].nbreArticleUniteStock + ")' >" +
                    
                                        "<option id='default"+design[i].uniteStock+"' value='Article<>"+design[i].prix+"<>"+idPanier+"'>Article</option>"+
                    
                                        "<option value='"+design[i].uniteStock+"<>"+design[i].prixuniteStock+"<>"+idPanier+"'>"+design[i].uniteStock+"</option>"+
                    
                                "</select>"+
                    
                            "</td>" +
                    
                            "<td><input type='number' min='0' oninput='this.value = !!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null' class='form-control' id='prixUniteStock"+idPanier+""+ligne.numligne+"' onkeyup='modif_Prix(this.value," + ligne.numligne + ")' value='" + design[i].prix + "' style='width: 70%' type='number'></input></td>" +
                        ...
                        var lignePanier = "<tr>" +
                            ...
                            "<td><input type='number' min='0' oninput='this.value = !!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null' disabled='true' class='form-control' id='prixUniteStock"+idPanier+""+ligne.numligne+"' onkeyup='modif_Prix(this.value,"+ ligne.numligne +")' value='" + design[i].prixuniteStock + "' style='width: 70%' type='number'></input></td>" +
    ...
    if(design!=null){
        for(var i=0; i<design.length;i++){
            ...
            if(parseInt(design[i].codeBarreDesignation)==codebarre){
                ...
                var lignes=getLigne();
                var searchLigne=lignes.find(l =>l.designation==searchDesignation.designation && l.idPagnet==idPanier);
                if(searchLigne != undefined){
                    ...
                    if(design[i].uniteStock!="Article" && design[i].uniteStock!="article"){
                        ...
                        var lignePanier = "<tr>" +
                            ...
                            "<td><input class='form-control filedReadonly"+idPanier+"' id='ligneQuantite"+idPanier+""+ligne.numligne+"' onkeyup='modif_Quantite(this.value," + ligne.numligne + " )' value='1' style='width: 30%' type='number' ></input></td>" +
                        "<input type='hidden' id='nbreArticleUniteStock' value='"+design[i].nbreArticleUniteStock+"'/>"+
                            "<td>" +
                            ...
                            "<td><input class='form-control' id='prixUniteStock"+idPanier+""+ligne.numligne+"' onkeyup='modif_Prix(this.value," + ligne.numligne + ")' value='" + design[i].prix + "' style='width: 70%' type='number'></input></td>" +
                        ...
                        var lignePanier = "<tr>" +
                            ...
                            "<td><input disabled='true' class='form-control' id='prixUniteStock"+idPanier+""+ligne.numligne+"' onkeyup='modif_Prix(this.value,"+ ligne.numligne +")' value='" + design[i].prixuniteStock + "' style='width: 70%' type='number'></input></td>" +
...
function addPanierV(){
    ...
    //if(i<=1  && users!=null){
        ...
        comptev=document.getElementById('compteV').value;
        if(comptev=='Caisse'){
            compte=comptes.find(c =>c.nomCompte=='Caisse' || c.nomCompte=='caisse' || c.nomCompte=='CAISSE')  
            Panier.idCompte =compte.idCompte;
        }
        if(comptev=='Wave'){
            compte=comptes.find(c =>c.nomCompte=='Wave' || c.nomCompte=='wave' || c.nomCompte=='WAVE')    
            Panier.idCompte =compte.idCompte;
        }
        if(comptev=='Orange money'){
            compte=comptes.find(c =>c.nomCompte=='Orange money' || c.nomCompte=='Orange Money' || c.nomCompte=='ORANGE MONEY')  
            Panier.idCompte =compte.idCompte;
        }
...
function modif_Quantite(quantite,ligne) {
    ...
    $("#somme_Total" + searchLigne.idPagnet).text(somme_A_payer(searchLigne.idPagnet));
                    

    $("#somme_Apayer" + searchLigne.idPagnet).text((somme_A_payer(searchLigne.idPagnet)));
...
function modif_UniteStock(unitevente,numligne,nbreArticleUniteStk, idPanier) {
    ...
    if(uniteStock!='Article' && uniteStock!='article'){
        ...
        var searchLigne=lignes.find(l =>l.numligne==numligne && l.idPagnet==idPanier); 
        ...
        if(searchLigne != undefined){
            ...
            $("#prixUniteStock"+searchLigne.idPagnet+""+searchLigne.numligne).val(searchLigne.prixunitevente);
            saveLigne(lignes);
...
function modif_Prix(prix,ligne) {
    ...
    $("#somme_Apayer" + searchLigne.idPagnet).text(somme_A_payer(searchLigne.idPagnet)- $("#val_remise1" + searchLigne.idPagnet).val());
...
function compte_Panier(panier) {
    ...
    if(panier.type==30){
        ...
        compteCpt=lescomptes.filter(cpt =>cpt.nomCompte!='Bon' && cpt.nomCompte!='Caisse');
        ...
        document.getElementById("compteCaisse"+panier.idPagnet).style.fontWeight = 'bold';
        document.getElementById("versement"+panier.idPagnet).style.fontWeight = "bold";
        document.getElementById("val_remise1"+panier.idPagnet).style.fontWeight = "bold";
    ...
    $('#compteCaisse'+idPanier+' option').each(function() {
            ...
            if(this.selected){
                if(this.text=='Bon'){
                    ...
                    localStorage.setItem('typeVente','simple');
                    compte=lesComptes.find(c =>c.nomCompte=='Caisse' || c.nomCompte=='caisse')
