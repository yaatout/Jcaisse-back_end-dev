...
try {
  ...
  $sql = "DELETE FROM `".$nomtablePagnet."` WHERE idPagnet = ?";        
  $p = $pdo->prepare($sql);
 
  $response = $p->execute(array($idPanier));

  if($response){
    $sql1 = "DELETE FROM `".$nomtableLigne."` WHERE idPagnet = ?";        
    $l = $pdo->prepare($sql1);
  
    $l->execute(array($idPanier));
    ...
    $sql2 = "insert into `".$nomtablePagnet."` (idPagnet,identifiantP,datepagej,heurePagnet,iduser,type,classe,totalp,remise,apayerPagnet,restourne,versement,verrouiller,idClient,idCompte,avance,dejaTerminer,synchronise)
    values (:idPagnet,:identifiantP,:datepagej,:heurePagnet,:iduser,:type,:classe,:totalp,:remise,:apayerPagnet,:restourne,:versement,:verrouiller,:idClient,:idCompte,:avance,:dejaTerminer,:synchronise)";
