...
require('code128.php');
    ...
    /**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
		$sqlP1="SELECT DISTINCT designation
		FROM `".$nomtableDesignation."`
		WHERE classe = 3  GROUP BY designation ";
	...
	/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
		$sqlC="SELECT COUNT(designation)
		FROM `".$nomtableDesignation."` 
		WHERE classe = 3  ORDER BY designation ASC";
...
if(mysql_num_rows($resP1)){
    while ($service = mysql_fetch_array($resP1)) { 

        $sql="SELECT *
		FROM `".$nomtableDesignation."`
		WHERE classe = 3 AND designation='".$service['designation']."' ";
        $res = mysql_query($sql) or die ("persoonel requête 2".mysql_error());
        $produit = mysql_fetch_array($res);

        $reqT="SELECT * from `aaa-transaction` where idTransaction='".$produit['description']."' ";
        $resT=mysql_query($reqT) or die ("select stock impossible =>".mysql_error());
        $transfert = mysql_fetch_array($resT);

        $sqlD="SELECT *
		FROM `".$nomtableDesignation."`
		WHERE classe = 3 AND designation='".$produit['designation']."' AND uniteStock='Depot'  ";
        $resD = mysql_query($sqlD) or die ("persoonel requête 2".mysql_error());
        $depot = mysql_fetch_array($resD);

        $sqlR="SELECT *
		FROM `".$nomtableDesignation."`
		WHERE classe = 3 AND designation='".$produit['designation']."' AND uniteStock='Retrait'  ";
        $resR = mysql_query($sqlR) or die ("persoonel requête 2".mysql_error());
        $retrait = mysql_fetch_array($resR);

        $sqlF="SELECT *
		FROM `".$nomtableDesignation."`
		WHERE classe = 3 AND designation='".$produit['designation']."' AND uniteStock='Facture'  ";
        ...
        $facture = mysql_fetch_array($resF);
                    ...
                    $image='images/'.$transfert['aliasTransaction'].'.png';
                    ...
                    if($c==1){
                       ...
                       // $pdf->SetXY( 8, ($y_axis + 5)); $pdf->SetFont( "Times", "B",  6);
                        ...
                        $pdf->SetTextColor(0,0,0);
                        $pdf->MultiCell(106,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",'DEPOT'),0,'C',false);
						$pdf->SetFillColor(0); 
						$pdf->SetFont('Arial','',10);
						$code=$depot['codeBarreDesignation'];
                        $pdf->Code128(38,($y_axis + 10 + 35),$code,45,20); 
                        //Code Retrait
                        //$pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(9, ($y_axis + 70), 105, $row_height, "DF");
                        $pdf->SetXY( 8, ($y_axis + 4 + 70)); $pdf->SetFont( "Times", "B", 10 ); 
                        $pdf->SetTextColor(0,0,0);
                        $pdf->MultiCell(106,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",'RETRAIT'),0,'C',false);
						$pdf->SetFillColor(0); 
						$pdf->SetFont('Arial','',10);
						$code=$retrait['codeBarreDesignation'];
                        $pdf->Code128(38,($y_axis + 10 + 70),$code,45,20);
                        //Code Facture
                        //$pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(9, ($y_axis + 105), 105, $row_height, "DF");
                        $pdf->SetXY( 8, ($y_axis + 4 + 105)); $pdf->SetFont( "Times", "B", 10 ); 
                        $pdf->SetTextColor(0,0,0);
                        $pdf->MultiCell(106,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",'FACTURE'),0,'C',false);
						$pdf->SetFillColor(0); 
						$pdf->SetFont('Arial','',10);
						$code=$facture['codeBarreDesignation'];
                        $pdf->Code128(38,($y_axis + 10 + 105),$code,45,20);
                    }
                    else if($c==2){
                        //Nom Transfert
                        ...
                        //Code Retrait
                        //$pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(9, ($y_axis + 70), 105, $row_height, "DF");
                        $pdf->SetXY( 105, ($y_axis + 4 + 70)); $pdf->SetFont( "Times", "B", 10 ); 
                        $pdf->SetTextColor(0,0,0);
                        $pdf->MultiCell(106,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",'RETRAIT'),0,'C',false);
                        $pdf->SetFillColor(0); 
                        $pdf->SetFont('Arial','',10);
                        $code=$retrait['codeBarreDesignation'];
                        $pdf->Code128(135,($y_axis + 10 + 70),$code,45,20);
                        //Code Facture
                        //$pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(9, ($y_axis + 105), 105, $row_height, "DF");
                        $pdf->SetXY( 105, ($y_axis + 4 + 105)); $pdf->SetFont( "Times", "B", 10 ); 
                        $pdf->SetTextColor(0,0,0);
                        $pdf->MultiCell(106,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",'FACTURE'),0,'C',false);
                        $pdf->SetFillColor(0); 
                        $pdf->SetFont('Arial','',10);
                        $code=$facture['codeBarreDesignation'];
                        $pdf->Code128(135,($y_axis + 10 + 105),$code,45,20);
                    }

                       
                       // $pdf->Image($image,40,10,130);                  
                       //
