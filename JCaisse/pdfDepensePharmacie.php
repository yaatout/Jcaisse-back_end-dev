...
require('fpdf/fpdf.php');
    ...
    WHERE l.classe = 2 && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY p.idPagnet ASC";
...
/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
    ...
    WHERE l.classe = 2 && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."' ORDER BY l.numligne ASC ";
...
$pdf->SetFont( "Arial", "BU", 10 ); $pdf->SetXY( 5, 38 ) ; $pdf->Cell($pdf->GetStringWidth("Depenses"), 0, 'Depenses', 0, "L");
...
$pdf->Cell(30,6,'Unite',1,0,'R',1);
$pdf->Cell(40,6,'Prix',1,0,'L',1);
$pdf->Cell(40,6,'Prix TT',1,0,'L',1);
$pdf->Cell(20,6,'Heure',1,0,'R',1);
...
while ($pagnets = mysql_fetch_array($resP1)) { 
    ...
    if ($nbre[0]==($l - 1))
    {
        ...
        $pdf->Cell(50,8,number_format($T_depenses, 2, ',', ' '),1,0,'L',0);
