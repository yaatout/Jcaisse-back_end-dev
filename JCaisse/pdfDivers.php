...
/**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
    ...
    WHERE (l.classe = 2 or l.classe = 5 or l.classe = 7)  && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY p.idPagnet ASC";
...
/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
    ...
    WHERE (l.classe = 2 or l.classe = 5 or l.classe = 7) && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY l.numligne ASC ";
...
/**Debut requete pour Calculer la Somme d' Approvisionnement d'Aujourd'hui  **/
...
/**Fin requete pour Calculer la Somme d' Approvisionnement d'Aujourd'hui  **/

/**Debut requete pour Calculer la Somme de Retrait d'Aujourd'hui  **/
...
/**Fin requete pour Calculer la Somme de Retrait d'Aujourd'hui  **/

/**Debut requete pour Calculer la Somme des Depenses d'Aujourd'hui  **/
    $sqlD="SELECT DISTINCT p.idPagnet
        FROM `".$nomtablePagnet."` p
        INNER JOIN `".$nomtableLigne."` l ON l.idPagnet = p.idPagnet
        WHERE l.classe=2 && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."' ORDER BY p.idPagnet DESC";
...
$pdf->SetFont( "Arial", "BU", 10 ); $pdf->SetXY( 5, 38 ) ; $pdf->Cell($pdf->GetStringWidth("Divers"), 0, 'Divers', 0, "L");
...
if(mysql_num_rows($resP1)){
    while ($pagnets = mysql_fetch_array($resP1)) { 
        ...
        while ($ligne = mysql_fetch_array($res8)) {
            ...
            $prix = number_format($pagnet['totalp'], 2, ',', ' ');
        ...
        if ($nbre[0]==($l - 1))
        {
            ...
            $pdf->SetFont('Arial','B',8);
            $pdf->SetX(105);
            $pdf->Cell(50,8,'Total Approvisionnement Caisse',1,0,'L',1);
            $pdf->Cell(50,8,number_format($T_App, 2, ',', ' '),1,1,'L',0);
            $pdf->SetX(105);
            $pdf->Cell(50,8,'Total Retrait Caisse',1,0,'L',1);
            $pdf->Cell(50,8,number_format($T_Rcaisse, 2, ',', ' '),1,1,'L',0);
            $pdf->SetX(105);
            $pdf->Cell(50,8,'Total Depense',1,0,'L',1);
            $pdf->Cell(50,8,number_format($T_depenses, 2, ',', ' '),1,1,'L',0);
        /* $pdf->SetX(105);
            $pdf->Cell(50,8,'TOTAL',1,0,'L',1);
    ...
    $pdf->SetFont( "Arial", "BU", 12 ); $pdf->SetXY( 90, 70 ) ; $pdf->Cell($pdf->GetStringWidth("Aucun Divers"), 0, 'Aucun Divers', 0, "L");
}
