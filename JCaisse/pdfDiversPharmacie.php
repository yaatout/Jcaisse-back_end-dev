...
require('fpdf/fpdf.php');
    ...
    WHERE (l.classe = 5 or l.classe = 7)  && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY p.idPagnet ASC";
...
/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
    ...
    WHERE (l.classe = 5 or l.classe = 7) && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY l.numligne ASC ";
...
$pdf->SetX(5);
$pdf->Cell(90,6,'Operation',1,0,'L',1);
...
while ($pagnets = mysql_fetch_array($resP1)) { 
    ...
    if ($nbre[0]==($l - 1))
    {
        ...
        $pdf->Cell(50,8,number_format(($T_App - $T_Rcaisse), 2, ',', ' '),1,0,'L',0);
