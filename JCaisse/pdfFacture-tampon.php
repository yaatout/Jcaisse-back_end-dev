...
if (isset($_POST['idPagnet'])) {
    ...
    $sql1="SELECT * FROM `".$nomtablePagnetTampon."` where idPagnet=".$idPagnet." ";
...
if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
    ...
    if ($_SESSION['idBoutique']==110) {
        $image='logo_Yaatout.png';
    }
    else if ($_SESSION['idBoutique']==201) {
        $image='logo_Ets_Diallo_Fils.png';
    }
    else {
        $image='shop.png';
    }
...
if ($_SESSION['idBoutique']==110) {
    $pdf-> Image('images/'.$image,3,3,25,25);
}
else{
...
//$champ_date = date_create($dateString2); $annee = date_format($champ_date, 'Y');
    ...
    if ($_SESSION['idBoutique']==201) {
        ...
        if ($client!=null) {
            $numfact = "BORDEREAU DE LIVRAISON  : #".$pagnet["idPagnet"] ;
        }
        else {
...
if($utilisateur['idEntrepot']!=0 && $utilisateur['idEntrepot']!=null){
  ...
  if( strlen($depot['adresseEntrepot']) <= 30){

      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 14 ) ; $pdf->MultiCell(190, 4, 'Adresse : '.$depot['adresseEntrepot'], 0, "L");

      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 20 ) ; $pdf->MultiCell(190, 4, 'Telephone : '.$_SESSION["telPortable"], 0, "L");

  }

  else{

      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 14 ) ; $pdf->MultiCell(190, 4, 'Adresse : '.substr($depot['adresseEntrepot'],0,30), 0, "L");

      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 20 ) ; $pdf->MultiCell(190, 4, substr($depot['adresseEntrepot'],30,40), 0, "L");

      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 26 ) ; $pdf->MultiCell(190, 4, 'Telephone : '.$_SESSION["telPortable"], 0, "L");
    ...
    if( strlen($_SESSION["adresseB"]) <= 40){
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 27, 14 ) ; $pdf->MultiCell(190, 4, ''.$_SESSION["adresseB"], 0, "L");
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 27, 20 ) ; $pdf->MultiCell(190, 4, 'Tel. : '.$_SESSION["telBoutique"], 0, "L");
    }
    else{
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 27, 14 ) ; $pdf->MultiCell(190, 4, ''.substr($_SESSION["adresseB"],0,30), 0, "L");
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 27, 20 ) ; $pdf->MultiCell(190, 4, substr($_SESSION["adresseB"],30,40), 0, "L");
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 27, 26 ) ; $pdf->MultiCell(190, 4, 'Tel. : '.$_SESSION["telBoutique"], 0, "L");
    }
...
if( strlen($_SESSION["descriptionB"]) <= 30){
    ...
    $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 32 ) ; $pdf->MultiCell(190, 4, ''.$_SESSION["descriptionB"], 0, "L");
    ...
    $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 32 ) ; $pdf->MultiCell(190, 4, ''.substr($_SESSION["descriptionB"],0,30), 0, "L");

    $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 35 ) ; $pdf->MultiCell(190, 4, substr($_SESSION["descriptionB"],30,40), 0, "L");
  ...
  if ($client!=null) {
    ...
    $telephone=htmlspecialchars(trim($_POST['telephone']));
        ...
        if($rTableExist = mysql_fetch_array($result)) {
            ...
            $sql0="INSERT INTO `".$nomtableFacture."` (idPagnet,nom,prenom,adresse,telephone,dateFacture,heureFacture,iduser) 

            values ('".$idPagnet."','".$nom."','".$prenom."','".mysql_real_escape_string($adresse)."','".$telephone."','".$dateString."','".$heureString."','".$_SESSION['iduser']."')";

            $res0=mysql_query($sql0) or die ("insertion client impossible =>".mysql_error() );
...
if(mysql_num_rows($res1)){
    ...
    $sql3="SELECT * FROM `".$nomtableLigneTampon."` where idPagnet=".$idPagnet." ORDER BY numligne DESC";
    ...
    $nb_piece = 0;
        ...
        while ($ligne =  mysql_fetch_assoc($res3)) { 
            ...
            if (strtolower($ligne['unitevente'])=='douzaine') {
               $nb_douzaine = $nb_douzaine + $ligne['quantite'];
            } else if (strtolower($ligne['unitevente'])=='demi gros') {
                $nb_piece = $nb_piece + 6*$ligne['quantite'];
            } else {
                $nb_piece = $nb_piece + $ligne['quantite'];
            }
                ...
                if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
                    ...
                    if($ligne['classe']==6){
                        ...
                        $pdf->SetXY( 7, $y+10 ); $pdf->Cell( 140, 5, utf8_decode($ligne['unitevente']), 0, 0, 'L');

                    }

                    else{
                        if (@$ligne['numeroChassis']!='') {
                            # code...
                            $pdf->SetXY( 7, $y+10 ); $pdf->Cell( 140, 5, utf8_decode($ligne['designation']." - ".@$ligne['numeroChassis']." / ".@$ligne['numeroMoteur']), 0, 0, 'L');
                        ...
                        $pdf->SetXY( 119, $y+10 ); $pdf->Cell( 10, 5, '('.substr(utf8_decode($entrepot['nomEntrepot']),0,3).')', 0, 0, 'R');
                    ...
                    if($ligne['classe']==6){
                        ...
                        $pdf->SetXY( 119, $y+10 ); $pdf->Cell( 10, 5, '('.substr(utf8_decode($ligne['unitevente']),0,3).')', 0, 0, 'R');
                ...
                if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
                    ...
                    $nombre_format_francais = number_format($ligne['prixunitevente'], 0, ',', ' ');

                    $pdf->SetXY( 156, $y+10 ); $pdf->Cell( 18, 5, $nombre_format_francais, 0, 0, 'R');

                    // Taux

                    $nombre_format_francais = number_format($ligne['prixunitevente']*$ligne['quantite'], 0, ',', ' ');
...

        {
            ...
            if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
                $pdf->SetFont( "Arial", "B", 10 ); $pdf->SetXY( 8, 250 ) ; $pdf->MultiCell(75, 4, 'Nombre de douzaine : '.$nb_douzaine, 0, "L");
                $pdf->SetFont( "Arial", "B", 10 ); $pdf->SetXY( 8, 255 ) ; $pdf->MultiCell(75, 4, 'Nombre de piece : '.$nb_piece, 0, "L");
            }
