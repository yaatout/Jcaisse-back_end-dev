...
$num_fact = "BON LIVRAISON_#".$pagnet["idPagnet"] ;

$numfact = "BON LIVRAISON  : #".$pagnet["idPagnet"] ;
...
if($utilisateur['idEntrepot']!=0 && $utilisateur['idEntrepot']!=null){
  ...
  if( strlen($_SESSION["adresseB"]) <= 30){
      ...
      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 14 ) ; $pdf->MultiCell(190, 4, 'Adresse : '.substr($_SESSION["adresseB"],0,30), 0, "L");

      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 20 ) ; $pdf->MultiCell(190, 4, substr($_SESSION["adresseB"],30,40), 0, "L");
...
$pdf->Line(130, 70, 130, 232); $pdf->Line(160, 70, 160, 232); 

// titre colonne

$pdf->SetXY( 1, 71 ); $pdf->SetFont('Arial','B',10); $pdf->Cell( 140, 8, "DESIGNATION", 0, 0, 'C');

$pdf->SetXY( 137, 71 ); $pdf->SetFont('Arial','B',10); $pdf->Cell( 13, 8, "QUANTITE", 0, 0, 'C');

$pdf->SetXY( 165, 71 ); $pdf->SetFont('Arial','B',10); $pdf->Cell( 32, 8, "ENTREPOT", 0, 0, 'C');
...
if(mysql_num_rows($res1)){
    ...
    while ($ligne =  mysql_fetch_assoc($res3)) { 
                ...
                if($ligne['classe']==6){
                    ...
                    $pdf->SetXY( 7, $y+10 ); $pdf->Cell( 140, 5, strtoupper($ligne['unitevente']), 0, 0, 'L');
                    ...
                    if (@$ligne['numeroChassis']!='') {
                        # code...
                        $pdf->SetXY( 7, $y+10 ); $pdf->Cell( 140, 5, strtoupper($ligne['designation']." - ".@$ligne['numeroChassis']." / ".@$ligne['numeroMoteur']), 0, 0, 'L');
                ...
                $pdf->SetXY( 137, $y+10 ); $pdf->Cell( 13, 5, strrev(wordwrap(strrev($ligne['quantite']), 3, ' ', true)), 0, 0, 'C');

                $pdf->SetXY( 165, $y+10 ); $pdf->Cell( 32, 5, strtoupper($entrepot['nomEntrepot']), 0, 0, 'C');
...

        {
            ...
            $pdf->SetFont( "Arial", "B", 8 ); $pdf->SetXY( 8, 235 ) ; $pdf->MultiCell(75, 4, 'Fait par : '.$user['prenom'].' '.strtoupper($user['nom']), 0, "L");
