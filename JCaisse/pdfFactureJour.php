...
/**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
    ...
    WHERE p.idClient='".$pagnet["idClient"]."'  && p.verrouiller=1 && p.datepagej ='".$pagnet["datepagej"]."' && p.type=0  ORDER BY p.idPagnet ASC";
...
/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
    ...
    WHERE p.idClient='".$pagnet["idClient"]."'  && p.verrouiller=1 && p.datepagej ='".$pagnet["datepagej"]."' && p.type=0   ORDER BY l.numligne ASC ";
...
if(mysql_num_rows($res1)){
    while ($pagnets = mysql_fetch_array($resP1)) { 
        ...
        $vente = mysql_fetch_array($resP); 

        $sql8="SELECT * 
        FROM `".$nomtableLigne."` l
        WHERE l.idPagnet='".$vente['idPagnet']."' ORDER BY numligne ASC";
        ...
        $totalP=$totalP + $vente['totalp'];
        $apayerP=$apayerP + $vente['apayerPagnet'];
        $remiseP=$remiseP + $vente['remise'];
        ...
        if ($nbre[0]==($l - 1))
        {
            ...
            $nombre_format_francais = "Net a payer TTC : " . number_format($apayerP, 0, ',', ' ') . " FCFA";
            ...
            $pdf->Cell(40,8,number_format($totalP, 0, ',', ' ')." FCFA",1,1,'L',0);
            if($remiseP!=0){
                $pdf->SetX(130);
                $pdf->Cell(33,8,'Remise',1,0,'L',1);
                $pdf->Cell(40,8,number_format($remiseP, 0, ',', ' ')." FCFA",1,1,'L',0);
            }
        }
