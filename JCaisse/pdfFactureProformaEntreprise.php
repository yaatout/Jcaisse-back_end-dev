...
if($utilisateur['idEntrepot']!=0 && $utilisateur['idEntrepot']!=null){
  ...
  if( strlen($depot['adresseEntrepot']) <= 30){
      ...
      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 20 ) ; $pdf->MultiCell(190, 4, 'Tel : '.$_SESSION["telPortable"], 0, "L");
  }
  else{
      ...
      $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 25, 26 ) ; $pdf->MultiCell(190, 4, 'Tel : '.$_SESSION["telPortable"], 0, "L");
  }
}
else{
...
$client = mysql_fetch_array($res2);
  ...
  if ($client!=null) {
    ...
    if( strlen($entreprise) <= 40){
        $pdf->SetFont( "Arial", "B", 13 ); $pdf->SetXY( 120, 43 ) ; $pdf->MultiCell(190, 4, $entreprise, 0, "L");
        $pdf->SetFont( "Arial", "B", 12 ); $pdf->SetXY( 120, 49 ) ; $pdf->MultiCell(190, 4, 'Adresse :', 0, "L");
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 140, 49 ) ; $pdf->MultiCell(190, 4, $adresse, 0, "L");
        $pdf->SetFont( "Arial", "B", 12 ); $pdf->SetXY( 120, 54 ) ; $pdf->MultiCell(190, 4, 'Telephone :', 0, "L");
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 144, 54 ) ; $pdf->MultiCell(190, 4, $telephone, 0, "L");
    }
    else{
        $pdf->SetFont( "Arial", "B", 13 ); $pdf->SetXY( 120, 38 ) ; $pdf->MultiCell(190, 4, substr($entreprise,0,30), 0, "L");
        $pdf->SetFont( "Arial", "B", 13 ); $pdf->SetXY( 120, 43 ) ; $pdf->MultiCell(190, 4, substr($entreprise,30,40), 0, "L");
        $pdf->SetFont( "Arial", "B", 12 ); $pdf->SetXY( 120, 49 ) ; $pdf->MultiCell(190, 4, 'Adresse :', 0, "L");
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 140, 49 ) ; $pdf->MultiCell(190, 4, $adresse, 0, "L");
        $pdf->SetFont( "Arial", "B", 12 ); $pdf->SetXY( 120, 54 ) ; $pdf->MultiCell(190, 4, 'Telephone :', 0, "L");
        $pdf->SetFont( "Arial", "", 12 ); $pdf->SetXY( 144, 54 ) ; $pdf->MultiCell(190, 4, $telephone, 0, "L");
...
if(mysql_num_rows($res1)){
    ...
    while ($ligne =  mysql_fetch_assoc($res3)) { 
                ...
                if (($_SESSION['type']=="Entrepot") && ($_SESSION['categorie']=="Grossiste")) {
                    ...
                    // libelle
                    if($ligne['classe']==6){
                        $pdf->SetXY( 7, $y+10 ); $pdf->Cell( 140, 5, 'BON : '.strtoupper($ligne['unitevente']), 0, 0, 'L');
                    }
                    else{
                        $pdf->SetXY( 7, $y+10 ); $pdf->Cell( 140, 5, strtoupper($ligne['designation']), 0, 0, 'L');
                        $pdf->SetXY( 119, $y+10 ); $pdf->Cell( 10, 5, '('.substr(strtoupper($entrepot['nomEntrepot']),0,3).')', 0, 0, 'R');
                    }
                }
                else{
                    if($ligne['classe']==6){
                        ...
                        $pdf->SetXY( 7, $y+10 ); $pdf->Cell( 140, 5, strtoupper($ligne['designation']), 0, 0, 'L');
                        $pdf->SetXY( 119, $y+10 ); $pdf->Cell( 10, 5, '('.substr(strtoupper($ligne['unitevente']),0,3).')', 0, 0, 'R');
                    } 
                }
                // qte
  ...
  if ($client!=null) {
    $num_fact=strtoupper($entreprise).' Proforma';
    $pdf->Output("D",strtoupper($entreprise).' '.$num_fact.'.pdf');
   }
   else{
    $pdf->Output("D", 'Fature-'.$idPagnet.'.pdf');
   }
