...
WHERE d.classe=0 AND idDesignation NOT IN (
  SELECT d.idDesignation FROM `".$nomtableDesignation."` d
  LEFT JOIN `".$nomtableStock."` s ON s.idDesignation = d.idDesignation
  WHERE d.classe=0  AND (s.quantiteStockCourant<>0) GROUP BY d.idDesignation 
...
LEFT JOIN `".$nomtableStock."` s ON s.idDesignation = d.idDesignation
WHERE d.classe=0  AND (s.quantiteStockCourant<>0) GROUP BY d.idDesignation ORDER BY d.categorie ASC,d.designation ASC ";
$res=mysql_query($sql);
...
$pdf->Cell(38,6,'Categorie',1,0,'L',1);
...
foreach ($produits as $stock) {
    ...
    $codeBarre = strtoupper($produit['categorie']);
