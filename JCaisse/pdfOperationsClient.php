...
if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
    ...
    if (isset($_POST['idClient'])) {
        ...
        if(mysql_num_rows($resP1)){
            ...
            $pdf->SetFillColor(192);
            //$pdf->SetTextColor(255,255,255);
            $sql12="SELECT SUM(apayerPagnet) FROM `".$nomtablePagnet."` where idClient=".$idClient." AND verrouiller=1 AND (type=0 || type=30 || type=11) AND ( (CONCAT(CONCAT(SUBSTR(datepagej,7, 10),'',SUBSTR(datepagej,3, 4)),'',SUBSTR(datepagej,1, 2)) BETWEEN '".$dateDebut."' AND '".$dateFin."') or (datepagej BETWEEN '".$dateDebut."' AND '".$dateFin."') ) ";
            $res12 = mysql_query($sql12) or die ("persoonel requête 2".mysql_error());
            $TotalB = mysql_fetch_array($res12) ;
