...
/**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
    ...
    WHERE (l.classe = 3 || l.classe = 4)  && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY p.idPagnet ASC";
...
/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
    ...
    WHERE (l.classe = 3 || l.classe = 4) && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."' ORDER BY l.numligne ASC ";
...
/**Debut requete pour Calculer la Somme des Transactions d'Aujourd'hui  **/
    ...
    $T_depot = 0 ;
    $S_depot = 0;
    while ($pagnet = mysql_fetch_array($resTD)) {
    ...
    $T_retrait = 0 ;
    $S_retrait = 0;
    while ($pagnet = mysql_fetch_array($resTR)) {
        $sqlS="SELECT SUM(apayerPagnet)
        FROM `".$nomtablePagnet."`
        where idClient=0 &&  verrouiller=1 && datepagej ='".$dateString2."' && idPagnet='".$pagnet['idPagnet']."'  ORDER BY idPagnet DESC";
        $resS=mysql_query($sqlS) or die ("select stock impossible =>".mysql_error());
    ...
    $T_facture = 0 ;
    $S_facture = 0;
    while ($pagnet = mysql_fetch_array($resTF)) {
        $sqlS="SELECT SUM(apayerPagnet)
        FROM `".$nomtablePagnet."`
        where idClient=0 &&  verrouiller=1 && datepagej ='".$dateString2."' && idPagnet='".$pagnet['idPagnet']."'  ORDER BY idPagnet DESC";
        $resS=mysql_query($sqlS) or die ("select stock impossible =>".mysql_error());
    ...
    $T_credit = 0 ;
    $S_credit = 0;
    while ($pagnet = mysql_fetch_array($resTC)) {
        $sqlS="SELECT SUM(apayerPagnet)
        FROM `".$nomtablePagnet."`
        where idClient=0 &&  verrouiller=1 && datepagej ='".$dateString2."' && idPagnet='".$pagnet['idPagnet']."'  ORDER BY idPagnet DESC";
        $resS=mysql_query($sqlS) or die ("select stock impossible =>".mysql_error());
...
/**Fin requete pour Calculer la Somme des Transactions d'Aujourd'hui **/
...
$pdf->SetFont( "Arial", "BU", 10 ); $pdf->SetXY( 5, 38 ) ; $pdf->Cell($pdf->GetStringWidth("Transactions"), 0, 'Transactions', 0, "L");
...
$pdf->SetX(5);
$pdf->Cell(65,6,'Reference',1,0,'L',1);
$pdf->Cell(30,6,'Operation',1,0,'L',1);
$pdf->Cell(60,6,'Service',1,0,'L',1);
...
if(mysql_num_rows($resP1)){
    while ($pagnets = mysql_fetch_array($resP1)) { 
        ...
        $sql8="SELECT * 
        FROM `".$nomtableLigne."` l
        WHERE idPagnet='".$pagnet['idPagnet']."' ORDER BY numligne ASC";
        ...
        while ($ligne = mysql_fetch_array($res8)) {
            ...
            $unite = '';
            if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) { 
                if($ligne['forme']=='Transaction'){
                    $designation = $ligne['designation'];
                    $service = $ligne['designation'];
                    if($ligne['quantite']==1){
                        $unite = 'Depot';
                    }
                    if($ligne['quantite']==0){
                    ...
                    if($ligne['unitevente']=='Facture'){
                        ...
                        $designation = $transaction['nomTransaction'];
                        $service=$ligne['designation'];
                    }
                    else{
                        $reqT="SELECT * from `aaa-transaction` where idTransaction='".$ligne['quantite']."'";
                        ...
                        $transaction = mysql_fetch_array($resT);
                        $designation = $ligne['designation'];
                        $service=$transaction['nomTransaction'];
                    }
                }
            }
            else{
                if($ligne['unitevente']=='Transaction'){
                    $designation = $ligne['designation'];
                    $service = $ligne['designation'];
                    if($ligne['quantite']==1){
                        $unite = 'Depot';
            ...
            $pdf->Cell(65,6,$designation,1,0,'L',1);
            $pdf->Cell(30,6,$unite,1,0,'L',1);
            $pdf->Cell(60,6,$service,1,0,'L',1);
        ...
        if ($nbre[0]==($l - 1))
        {
            ...
            $pdf->SetFont('Arial','B',8);
            $pdf->SetX(120);
            $pdf->Cell(35,8,'Total Depot',1,0,'L',1);
            $pdf->Cell(50,8,number_format($T_depot, 2, ',', ' '),1,1,'L',0);
            $pdf->SetX(120);
            $pdf->Cell(35,8,'Total Retrait',1,0,'L',1);
            $pdf->Cell(50,8,number_format($T_retrait, 2, ',', ' '),1,1,'L',0);
            $pdf->SetX(120);
            $pdf->Cell(35,8,'Total Facture',1,0,'L',1);
            $pdf->Cell(50,8,number_format($T_facture, 2, ',', ' '),1,1,'L',0);
            $pdf->SetX(120);
            $pdf->Cell(35,8,'Total Credit',1,0,'L',1);
            $pdf->Cell(50,8,number_format($T_credit, 2, ',', ' '),1,1,'L',0);
            $pdf->SetX(120);
            $pdf->Cell(35,8,'TOTAL ',1,0,'L',1);
            $pdf->Cell(50,8,number_format(($T_depot + $T_facture + $T_credit - $T_retrait), 2, ',', ' '),1,0,'L',0);
    ...
    $pdf->SetFont( "Arial", "BU", 12); $pdf->SetXY( 90, 70 ) ; $pdf->Cell($pdf->GetStringWidth("Aucune Transaction"), 0, 'Aucune Transaction', 0, "L");
}
