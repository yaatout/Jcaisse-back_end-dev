...
/**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
    $sqlP1="SELECT * 
    FROM `".$nomtableDesignation."` WHERE classe = 0 ORDER BY designation ";
...
/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
    $sqlC="SELECT COUNT(DISTINCT idDesignation)
...
$pdf->SetX(5);
$pdf->Cell(120,6,'Reference',1,0,'L',1);
...
while ($produit = mysql_fetch_array($resP1)) { 
    ...
    $S_produit = mysql_fetch_array($resS);
            ...
            $quantite = $S_produit[0];
            $categorie = strtoupper($produit['categorie']);
            $prixU = number_format($produit['prixSession'], 2, ',', ' ');
            $prixT = number_format($produit['prixPublic'], 2, ',', ' ');
            $qte = ' ';
            ...
            $pdf->Cell(120,6,$designation.' '.$categorie,1,0,'L',1);
