...
WHERE d.classe=0 AND s.quantiteStockCourant<>0  ORDER BY s.designation";
$res=mysql_query($sql);

$sqlC="SELECT COUNT(DISTINCT d.idDesignation) FROM `".$nomtableStock."` s
    LEFT JOIN `".$nomtableDesignation."` d ON d.idDesignation = s.idDesignation 
    WHERE d.classe=0 AND s.quantiteStockCourant<>0  ORDER BY s.idStock DESC";
...
$pdf->SetX(5);
$pdf->Cell(75,6,'Reference',1,0,'L',1);
$pdf->Cell(20,6,'Quantite',1,0,'L',1);
$pdf->Cell(25,6,'Unite Stock',1,0,'L',1);
$pdf->Cell(25,6,'Prix U.S',1,0,'L',1);
$pdf->Cell(28,6,'Prix Unitaire',1,0,'L',1);
$pdf->Cell(25,6,'Prix Achat',1,0,'L',1);
...
while ($stock=mysql_fetch_array($res)) { 
    ...
    if(in_array($produit['idDesignation'], $produits)){
      // echo "Existe.";
            ...
            $prixUS = strtoupper($produit['prixuniteStock']);
            $prixU = number_format($produit['prix'], 2, ',', ' ');
            $prixachat = number_format($produit['prixachat'], 2, ',', ' ');
            // $qte = ' ';
            ...
            $pdf->Cell(20,6,$quantite,1,0,'L',1);
            $pdf->Cell(25,6,$uniteStock,1,0,'L',1);
            $pdf->Cell(25,6,$prixUS,1,0,'L',1);
            $pdf->Cell(28,6,$prixU,1,0,'L',1);
            $pdf->Cell(25,6,$prixachat,1,0,'L',1);
