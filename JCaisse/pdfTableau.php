...
require('fpdf/fpdf.php');
    ...
    $prixUS=htmlspecialchars(trim($_POST['prix']));

    if($categorie=='Tout'){
        /**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
            $sqlP1="SELECT *
            FROM `".$nomtableDesignation."`
            WHERE classe = 0  ORDER BY idDesignation ASC";
        ...
        /**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
            $sqlC="SELECT COUNT(idDesignation)
            FROM `".$nomtableDesignation."` 
            WHERE classe = 0  ORDER BY idDesignation ASC";
        ...
        $sqlP1="SELECT *
        FROM `".$nomtableDesignation."`
        WHERE classe = 0 AND categorie='".$categorie."' ORDER BY idDesignation ASC";
        $resP1 = mysql_query($sqlP1) or die ("persoonel requête 2".mysql_error());
        /**Fin requete pour Lister les Paniers vendus Aujourd'hui  **/
...
if(mysql_num_rows($resP1)){
    while ($produit = mysql_fetch_array($resP1)) { 
                    ...
                    if($prixUS!='Article'){
                        $prix = number_format($produit['prixuniteStock'], 0, ',', ' ');
                    }
                    else{
                    ...
                    $pdf->SetDrawColor(255,255,255);
                       if($c==1){
                        $pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(9, $y_axis, 48, $row_height, "DF");
                        $pdf->SetXY( 8, ($y_axis + 1)); $pdf->SetFont( "Times", "B",  6);
                        $pdf->SetTextColor(0,0,0); 
                        $pdf->MultiCell(49,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$designation),0,'C',false);
                        $pdf->SetXY( 8, ($y_axis + 15)); $pdf->SetFont( "Times", "B", 40 ); 
                        $pdf->SetTextColor(194,8,8);
                        $pdf->MultiCell(49,8,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$prix),0,'C',false);                    
                       }
                       else if($c==2){
                        $pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(57, $y_axis, 48, $row_height, "DF");
                        $pdf->SetXY( 58, ($y_axis + 1)); $pdf->SetFont( "Times", "B", 6 ); 
                        $pdf->SetTextColor(0,0,0);
                        $pdf->MultiCell(49,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$designation),0,'C',false);
                        $pdf->SetXY( 58, ($y_axis + 15)); $pdf->SetFont( "Times", "B", 40 ); 
                        $pdf->SetTextColor(194,8,8);
                        $pdf->MultiCell(49,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$prix),0,'C',false);
                       }
                       else if($c==3){
                        $pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(105, $y_axis, 49, $row_height, "DF");
                        $pdf->SetXY( 106, ($y_axis + 1)); $pdf->SetFont( "Times", "B", 6 ); 
                        $pdf->SetTextColor(0,0,0);
                        $pdf->MultiCell(49,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$designation),0,'C',false); 
                        $pdf->SetXY( 106, ($y_axis + 15)); $pdf->SetFont( "Times", "B", 40 ); 
                        $pdf->SetTextColor(194,8,8);
                        $pdf->MultiCell(49,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$prix),0,'C',false);                  
 
                       }
                       else if($c==4){
                        $pdf->SetLineWidth(0.1); $pdf->SetFillColor(255); $pdf->Rect(154, $y_axis, 49, $row_height, "DF");
                        $pdf->SetXY( 154, ($y_axis + 1)); $pdf->SetFont( "Times", "B", 6 );
                        $pdf->SetTextColor(0,0,0); 
                        $pdf->MultiCell(49,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$designation),0,'C',false);
                        $pdf->SetXY( 154, ($y_axis + 15)); $pdf->SetFont( "Times", "B", 40 ); 
                        $pdf->SetTextColor(194,8,8);
                        $pdf->MultiCell(49,5,iconv("UTF-8", "ISO-8859-1//TRANSLIT",$prix),0,'C',false);
                       }
