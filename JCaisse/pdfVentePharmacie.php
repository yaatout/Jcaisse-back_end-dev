...
/**Debut requete pour Lister les Paniers vendus Aujourd'hui  **/
    ...
    WHERE (l.classe = 0 or l.classe = 1)  && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY p.idPagnet ASC";
...
/**Debut requete pour Compter les Paniers vendus Aujourd'hui  **/
    ...
    WHERE (l.classe = 0 or l.classe = 1) && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY l.numligne ASC ";
...
/**Debut requete pour Calculer la Somme des Ventes d'Aujourd'hui  **/
    ...
    FROM `".$nomtablePagnet."` p
    INNER JOIN `".$nomtableLigne."` l ON l.idPagnet = p.idPagnet
    WHERE (l.classe=0 || l.classe=1) && p.idClient=0  && p.verrouiller=1 && p.datepagej ='".$dateString2."'  ORDER BY p.idPagnet DESC";
...
while ($pagnets = mysql_fetch_array($resP1)) { 
    ...
    if ($nbre[0]==($l - 1))
    {
        ...
        $pdf->Cell(50,8,number_format(($T_ventes - $T_remises), 2, ',', ' '),1,1,'L',0);
        $pdf->SetX(120);
        $pdf->Cell(35,8,'TOTAL TVA',1,0,'L',1);
