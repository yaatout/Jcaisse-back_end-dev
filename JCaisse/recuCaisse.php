...
if ($_SESSION['tampon'] == 1 && $_SESSION['page'] == 'vente') {
  require('barcodeFacture-tampon.php');
} else {
?>
...
<body onload="window.print();">
    <div style="margin-right: 4%;">
      ...
      if (isset($_POST['idPagnet'])) {
        ...
        $res1 = mysql_query($sql1) or die("persoonel requête 111" . mysql_error());
        ...
        if (($_SESSION['type'] == "Entrepot") && ($_SESSION['categorie'] == "Grossiste")) {
          ...
          // $sql1 = "SELECT * FROM `" . $nomtableLigne . "` where idPagnet=" . $idPagnet . "";
          // $res1 = mysql_query($sql1) or die("persoonel requête 111" . mysql_error());
          // $pagnet = mysql_fetch_array($res1);


          $sql = "SELECT * FROM `" . $nomtableLigne .
          ...
          **************************************************
          ...
          if ($classe == 2) {
            ...
            echo  '
                    <tr>
                    <td colspan="4" style="border-bottom: 1px solid black;border-top: 1px solid black;text-align:center">
                        DEPENSE
                    </td>
                  </tr>
              ';
          ...
          if ($_SESSION['compte'] == 1) {
            if ($pagnet['idCompte'] != 0) {
              if ($pagnet['idCompte'] == 1000) {
                $multipleCpt = explode("_", $pagnet['idsCpteMultiple']);
                foreach ($multipleCpt as $key) {

                  $sqlGetComptePay2 = "SELECT * FROM `" . $nomtableCompte . "` where idCompte = " . $key['idCompte'];
                  $resPay2 = mysql_query($sqlGetComptePay2) or die("persoonel requête 2" . mysql_error());
                  $cpt = mysql_fetch_array($resPay2);

                  $sqlGetComptePay3 = "SELECT * FROM `" . $nomtableComptemouvement . "` where idCompte = " . $key['idCompte'] . " and mouvementLink = " . $pagnet['idPagnet'];
                  $resPay3 = mysql_query($sqlGetComptePay3) or die("persoonel requête 2" . mysql_error());
                  $cpt3 = mysql_fetch_array($resPay3);

                  echo '<br><span style="float: left"> COMPTE </span> <span style="float: right"> ' . $cpt['nomCompte'] . ' = ' . $cpt3['montant'] . ' FCFA </span>';
                }
              } else {
          ...
          WHERE idutilisateur =" . $pagnet['iduser'];
          ...
          echo ' <span style="float: left"> Personnel </span> <span style="float: right"> ' . $utilisateur['prenom'] . " " . $utilisateur['nom'] . ' </span>';
