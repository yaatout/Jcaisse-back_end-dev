$(document).ready(function() {  
    ...
    $("#listerBons").load("datatables/bonsLivraisons.php",{"nbre_Entree":"","query":"","dateDebut":dateDebut,"dateFin":dateFin},  function (response, status, xhr){
        if(status == "success"){
            $(".loading-gif").hide();  
    ...
    $("#inpt_Search_ListerBons").on("keyup", function(e) {

        e.preventDefault();
        query = $('#inpt_Search_ListerBons').val();
        nbre_Entree = $('#slct_Nbre_ListerBons').val() ;

        var keycode = (e.keyCode ? e.keyCode : e.which);
            if (keycode == '13') {
                if (query.length > 0) {
                    $("#listerBons").load("datatables/bonsLivraisons.php",{"nbre_Entree":nbre_Entree,"query":query,"dateDebut":dateDebut,"dateFin":dateFin},  function (response, status, xhr){
                        if(status == "success"){
                                    
                        }
                    });
                }else{
                    $("#listerBons").load("datatables/bonsLivraisons.php",{"nbre_Entree":nbre_Entree,"query":"","dateDebut":dateDebut,"dateFin":dateFin},  function (response, status, xhr){
                        if(status == "success"){
    ...
    $("#listerBons").on( "change", "#slct_Nbre_ListerBons", function (e){
        e.preventDefault();
        query = $('#inpt_Search_ListerBons').val();
        nbre_Entree = $('#slct_Nbre_ListerBons').val() ;
        ...
        if (query.length == 0) {
            $("#listerBons").load("datatables/bonsLivraisons.php",{"nbre_Entree":nbre_Entree,"query":"","dateDebut":dateDebut,"dateFin":dateFin},  function (response, status, xhr){
                if(status == "success"){
                    $('#slct_Nbre_ListerBons').val(nbre_Entree) ;
                }
            });
        }else{
            $("#listerBons").load("datatables/bonsLivraisons.php",{"nbre_Entree":nbre_Entree,"query":query,"dateDebut":dateDebut,"dateFin":dateFin},  function (response, status, xhr){
    ...
    $("#listerBons").on( "click", ".pagination a", function (e){
        ...
        if (query.length == 0) {
            $("#listerBons").load("datatables/bonsLivraisons.php",{"page":page,"nbre_Entree":nbre_Entree,"query":"","dateDebut":dateDebut,"dateFin":dateFin},  function (response, status, xhr){
                if(status == "success"){
                    $('#slct_Nbre_ListerBons').val(nbre_Entree) ;        
                }
            });
        }else{
            $("#listerBons").load("datatables/bonsLivraisons.php",{"page":page,"nbre_Entree":nbre_Entree,"query":query,"dateDebut":dateDebut,"dateFin":dateFin},  function (response, status, xhr){
                if(status == "success"){
                    $('#slct_Nbre_ListerBons').val(nbre_Entree) ;       
                }
            });
    ...
    /**Debut Ajouter Livraison**/
        $(function(){
            ajouter_Livraison= function ()  {
                $.ajax({
                    url:"calculs/bonsLivraisons.php",
                    method:"POST",
                    data:{
                           ...
                           "<table class='table table-bordered' id='tableBon"+details[0]+"' >"+
                               ...
                               "<tbody>"+
                                   "<tr id='ligneBon"+details[0]+"'>" +
                                       "<td><select class='form-control champ_saisie_Ligne"+details[0]+"'  id='slct_Bon_Fournisseur_"+details[0]+"'></select></td>"+
    ...
    /*                                "<tr style='display:none' id='nouveau_Commande"+details[0]+"'>"+
                       ...
                       "</div>"+
                     "</div>";
                     $("#listerBons #accordion").prepend(bon);
                     var choix = details[3];
                     var taille = choix.length;
                     $('#slct_Bon_Fournisseur_'+details[0]+' option').remove();
         
                     $('#slct_Bon_Fournisseur_'+details[0]).append("<option selected >--------------</option>");
         
                     for(var i = 0; i<taille; i++){
                             $('#slct_Bon_Fournisseur_'+details[0]).append("<option value='"+choix[i][0]+"'>"+choix[i][1]+"</option>");
                     }
                    },
                    error: function() {
                        alert("La requête "); },
    ...
    /**Debut Terminer Bon Livraison et Commande**/
        $(function(){
            terminer_Bon= function(idBon) { 
                var idFournisseur=$('#slct_Bon_Fournisseur_'+idBon).val();
                var numero=$('#inpt_Bon_Numero_'+idBon).val();
                ...
                $('.champ_saisie_Ligne'+idBon).each(function() {
                    if(nbre==1){
                        fournisseur=$('#slct_Bon_Fournisseur_'+idBon+' option:selected').text();
                        $(this).replaceWith($("<span />").text(fournisseur));
                    }
                    else{
    ...
    /**Debut Modifier Bon Livraison et Commande**/
        ...
        $(function(){
            btn_modifier_Bon= function (idBon)  {
                idFournisseur=$('#slct_Bon_Fournisseur_'+idBon).val();
