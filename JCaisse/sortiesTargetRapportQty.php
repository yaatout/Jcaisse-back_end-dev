<?php
    ...
    $sql="SELECT idDesignation,designation,unitevente, SUM(quantite) as qs FROM  `".$nomtableLigne."` l , `".$nomtablePagnet."` p 
        ...
        OR (datepagej BETWEEN '".$debut."' AND '".$fin."')) GROUP BY l.idDesignation, l.unitevente";

    $res = mysql_query($sql) or die ("persoonel requête 2".mysql_error());

    $qte_S_fecth = mysql_fetch_array($res);
    
    // var_dump($qte_S_fecth);
            ...
            <tbody>
                <?php
                    ...
                    while ($line=mysql_fetch_array($res)) {
                        ...
                        if (in_array($line['idDesignation']."-".$line['unitevente'], $ids)) {
                                    
                        }

                        else {

                            // $sqlS="SELECT SUM(prixtotal) FROM
                        
                            //     `".$nomtableLigne."` l , `".$nomtablePagnet."` p 

                            //     where p.idPagnet = l.idPagnet && l.classe = 0 && p.verrouiller=1 && l.idDesignation='".$line["idDesignation"]."'
                            //     && (p.type=0 || p.type=30) && ((CONCAT(CONCAT(SUBSTR(p.datepagej,7, 10),'',SUBSTR(p.datepagej,3, 4))
                            //     ,'',SUBSTR(p.datepagej,1, 2)) BETWEEN '".$debut."' AND '".$fin."') 
                            ...
                            // $sqlQS="SELECT designation,unitevente, SUM(quantite) as qs
                    
                            //     FROM `".$nomtableLigne."` l , `".$nomtablePagnet."` p 
                            ...
                            $ids[] = $line['idDesignation']."-".$line['unitevente'];
                            $i++;
                        }
                    }
                ?>
