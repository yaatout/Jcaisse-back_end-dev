...
if ($operation=="2") {
  ...
  if ($tri==1) {
    ...
    # code...
    if ($query =="") {
      $results = $bddV->prepare("SELECT * from `".$nomtableDesignation."` WHERE (classe = 0 or classe = 1) and image!='' ORDER BY designation DESC LIMIT $page_position, $item_per_page");
      $results->execute(); //Execute prepared Query
    } else {
      //Limit our results within a specified range. 
      $results = $bddV->prepare("SELECT * from `".$nomtableDesignation."` WHERE (classe = 0 or classe = 1) and image!='' and designation LIKE '%".$query."%' ORDER BY designation DESC LIMIT $page_position, $item_per_page");
      $results->execute(); //Execute prepared Query
    }
  }
