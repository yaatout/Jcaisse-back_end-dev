...
/**Fin informations sur la date d'Aujourdhui **/ /**************************************************/
    ...
    if(isset($_POST["operation"]) and $operation == 1){
        ...
        $result .= '<>'.$design['image'].'<>'.$design['idBoutique'];
        exit($result);

    }
    if(isset($_POST["operation"]) and $operation == 2){
        ...
        // }
            $req0 = $bddV->prepare("SELECT * FROM `ligne` WHERE idPanier=:idP and barrer = 0");
            $req0->execute(array('idP' => $idPanier )) or die(print_r($req0->errorInfo()));
            $lignes=$req0->fetchAll();

            if (empty($lignes)) {
                $req2 = $bddV->prepare("UPDATE `panier` SET annuler = 1 WHERE idPanier = :idP ");
                $req2->execute(array(
                    'idP' => $idPanier
                )) or die(print_r($req2->errorInfo()));
            }
    ...
    if(isset($_POST["operation"]) and $operation == 3){
        $result = [];
        $designationJC = [];
    ...
    if(isset($_POST["operation"]) and $operation == 4){
        $idA = $_POST["idA"];
        $idP = $_POST["idP"];
    ...
    if(isset($_POST["operation"]) and $operation == 5){
        $oldIdA = $_POST["oldIdA"];
        $idP = $_POST["idP"];
        $newRef = $_POST["newRef"];
        $oldRef = $_POST["oldRef"];
        ...
        if ($oldRef === $newRef) {
            ...
            ligne(idBoutique,designation, prix,quantite,unite,prixTotal,image,idPanier,inPanier)
            VALUES(:idBoutique,:designation,:prix, :quantite, :unite, :prixTotal,:image,:idPanier,:inPanier)') ;
            $req4->execute(array(
            ...
            $req4->closeCursor();
            /****** Update old ligne *****/
            $req = $bddV->prepare("UPDATE `ligne` SET barrer = 1, updateQuantite = 0 WHERE idArticle = :oldIdA and idPanier = :idP ");
            $req->execute(array(
                'idP' => $idP,
                'oldIdA' => $oldIdA
            )) or die(print_r($req->errorInfo()));
    ...
    if(isset($_POST["operation"]) and $operation == 6){
    ...
    if(isset($_POST["operation"]) and $operation == 7){
        ...
        if (find_p_with_position($lignes, $ref, $unite) !==FALSE) {
            ...
            $req4 = $bddV->prepare('INSERT INTO
            ligne(idBoutique,designation, prix,quantite,prixTotal,image,idPanier,unite,inPanier)
            VALUES(:idBoutique,:designation,:prix, :quantite, :prixTotal,:image,:idPanier,:unite,:inPanier)') ;
            $req4->execute(array(
...
if(isset($_POST["operation"]) and ($operation == 12)){
  ...
  if(isset($_POST["op"]) and $_POST["op"] == 'croppe') {
