...
$sqlGetRefV = $bddV->prepare("SELECT idDesignation FROM `".$nomtableDesignation."`");

$sqlGetRefV->execute() or die(print_r($sqlGetRefV->errorInfo()));

$refVitrineId=$sqlGetRefV->fetchAll();

$idDTab = [];

foreach ($refVitrineId as $key) {
  # code...
  $idDTab[] = $key['idDesignation'];
}
// var_dump($idDTab);

if (isset($_POST['btncolor'])) {
  # code...
  // var_dump($_POST['favcolor']);
  // var_dump($_SESSION['idBoutique']);
  $codecolor1 = $_POST['favcolor1']; 
  $codecolor2 = $_POST['favcolor2']; 
  $codecolor = $_POST['favcolor']; 

  $req20 = $bddV->prepare("UPDATE `boutique` SET codeColorMenu1=:c1, codeColorMenu2=:c2, codeColorCategorie=:c WHERE idBoutique = :idB");
  ...
  $sqlUpdate="UPDATE `aaa-boutique` SET codeColor='".$codecolor."' WHERE idBoutique=".$_SESSION['idBoutique'];

  $updateColor = mysql_query($sqlUpdate) or die ("persoonel requête 1".mysql_error());
...
function find_p_with_position2($pns,$des,$idB) {

  foreach($pns as $index => $p) {

      if(($p['idDesignation'] == $des && $p['idBoutique'] == $idB)){
...
if (isset($_POST["upTodate"])) {
  ...
  $sqlGetRefJC="SELECT * FROM `".$nomtableDesignation."` WHERE classe = 0 and (categorie!='sans categorie' || categorie!='')";
  ...
  // $sqlGetLigneV = $bddV->prepare("SELECT * FROM ligne l where l.idBoutique=:idB");

  // $sqlGetLigneV->execute(array(

  //   'idB' => $_SESSION['idBoutique']

  // )) or die(print_r($sqlGetLigneV->errorInfo()));

  // $refLigne=$sqlGetLigneV->fetchAll();

  // var_dump($refLigne);

  while ($key = mysql_fetch_array($refJC)) {

    if (find_p_with_position($refVitrine, $key['idDesignation']) !==FALSE) {

        $i=find_p_with_position($refVitrine, $key['idDesignation']);
    ...
    // if (find_p_with_position2($refLigne, $key['idDesignation'], $_SESSION['idBoutique']) !==FALSE) {

    //   $i=find_p_with_position2($refLigne, $key['idDesignation'], $_SESSION['idBoutique']);


    //     /****** Update designation dans ligne *****/

    //   $req20 = $bddV->prepare("UPDATE ligne SET designation=:design WHERE idDesignation = :idD and idBoutique = :idB");
    ...
    //       'idD' => $refLigne[$i]['idDesignation']

    //   )) or die(print_r($req20->errorInfo()));

    //   // /****** Update panier *****/

    //   // $subtotal=$key['prix']*$refLigne[$i]['quantite']-$refLigne[$i]['prixTotal'];

    //   // $req22 = $bddV->prepare("UPDATE panier SET total=:t WHERE idPanier = :idP");

    //   // $req22->execute(array(

    //   //     't' => $refLigne[$i]['total']+$subtotal,

    //   //     'idP' => $refLigne[$i]['idPanier']
...
if (($_SESSION['type']=="Pharmacie") && ($_SESSION['categorie']=="Detaillant")) {
  ...
  require('vitrine-pharmacie.php');
    ...
    if (isset($_POST['modification'])) {
        ...
        $req6 = $bddV->prepare("UPDATE `".$nomtableDesignation."` SET designation = :d, uniteDetails = :ud, prixuniteStock = :pus, prix = :p WHERE id=:idV ");
      ...
      // var_dump(344443);
        ...
        $imageProcess = 0;
            ...
            if(is_array($_FILES)) {
                ...
                $remotePath = "public_html/uploads/logo/";
          ...
          $sql3='SELECT * from  `'.$nomtableDesignation.'` where classe = 0 order by idDesignation desc';
           ...
           if($res3=mysql_query($sql3)){
              ...
              while($tab3=mysql_fetch_array($res3)){
                  ...
                  $req = $bddV->prepare("INSERT INTO `".$nomtableDesignation."`(designation, designationJcaisse, categorie, idBoutique, idDesignation, uniteStock,nbreArticleUniteStock,prix, prixuniteStock, codeBarreDesignation, codeBarreuniteStock, idFusion)

              VALUES(:designation,:designationJcaisse,:categorie,:idBoutique, :idDesignation, :uniteStock, :nbreArticleUniteStock, :prix, :prixuniteStock, :codeBarreDesignation, :codeBarreuniteStock, :idFusion)") ;
              ...
              $req->execute(array(
                            ...
                            'nbreArticleUniteStock' => $tab3['nbreArticleUniteStock'],
    ...
    echo'
       ...
       $req2 = $bddV->prepare("SELECT upToDate, codeColorMenu1, codeColorMenu2, codeColorCategorie from boutique where idBoutique = :idB ");
    ...
    <center>
      <form method="post" class="form-inline">
        <div class="form-group">
          <label>Couleur du Menu 1 : </label>
          <input class="form-control" type="color" style="width:80%" id="favcolor1" name="favcolor1" value="'.$b['codeColorMenu1'].'">
        </div>&emsp;
        <div class="form-group">
          <label>Couleur du Menu 2 : </label>
          <input class="form-control" type="color" style="width:80%" id="favcolor2" name="favcolor2" value="'.$b['codeColorMenu2'].'">
        </div>&emsp;
        <div class="form-group">
          <label>Couleur de fond des catégories : </label>
          <input class="form-control" type="color" style="width:80%" id="favcolor" name="favcolor" value="'.$b['codeColorCategorie'].'">
        </div>&emsp;
        <input class="btn btn-info" type="submit" name="btncolor" value="Valider">
      </form>
    </center></br>
    ...
    <i class="glyphicon glyphicon-plus"></i>Ajouter un Produit</button></center>
      ...
      <form method="post" class="pull-right">
        ...
        <button type="submit" class="btn btn-primary btn-lg" name="upTodate"><i class="glyphicon glyphicon-refresh"></i> Mettre à jour';

        if ($b['upToDate'] == 1) {

          # code...

          echo'<span id="" style="margin-top: -20px;margin-right: -15px;border-radius: 50%;background: red;color: white;" class="badge bg-warning">V</span>';
    ...
    echo'<div class="modal fade bd-example-modal-xl" tabindex="-1" id="AjoutStockModal" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">';
                                    ...
                                    <div class="modal-dialog" role="document">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                              ...
                                              <form name="formulaireVersement" method="post" action="vitrine/vitrine.php" >
      ...
      $sql3='SELECT * from  `'.$nomtableDesignation.'` where classe = 0 order by idDesignation desc';
          ...
          if($res3=mysql_query($sql3)){
              ...
              // var_dump($idDTab);
               while($tab3=mysql_fetch_array($res3)){
                // var_dump($tab3['idDesignation']);

                //  if (in_array($tab3['idDesignation'], $idDTab)) {
                  // if (find_p_with_position($refVitrine, $tab3['idDesignation']) === FALSE) {
 ...
 <?php
                ...
                $sql00 = $bddV->prepare("select COUNT(*) as total_row from `".$nomtableDesignation."`  ");
                $sql00->execute()  or die(print_r($sql00->errorInfo()));
                $total_rows=$sql00->fetch();
                //////////////////////

                $sql01 = $bddV->prepare("select COUNT(*) as total_row from `".$nomtableDesignation."`  where image!=:im ");
                $sql01->execute(array('im' =>''))  or die(print_r($sql01->errorInfo()));
                $total_img=$sql01->fetch();

              ?>
              <h2><?php echo'<div class="row" align="center"><span id="" style="margin-top: -20px;margin-right: -15px;border-radius: 50%;background: red;color: white;"
                                        class="badge bg-warning">'.$total_img[0].' Images sur '.$total_rows[0].'</span></div>'; ?>  
              </h2>
    ...
    <div class="container" align="center">
            ...
            <ul class="nav nav-tabs">
              ...
              <li id="produitImgEvent"><a data-toggle="tab" href="#PRODUITIMAGE">LISTE DES PRODUITS AVEC IMAGE</a></li>
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane fade in active" id="STOCKPRODUITDETAIL">
                  ...
                  <label class="pull-left" for="nbEntree">Nombre d'entrées </label>
                  ...
                  <select class="pull-left" id="nbEntree">
                    ...
                    <!-- <input type="text" name="" id="nbEntree" placeholder="Nombre d'entrées"> -->

                    <input class="pull-right" type="text" name="" id="searchInput" placeholder="Rechercher...">

                    <!-- <div class="loading-div"><img src="images/loader.gif" alt="loading..." ></div> -->
                ...
                <div class="tab-pane fade" id="PRODUITIMAGE">

                  <div class="table-responsive">

                  <label class="pull-left" for="nbEntreeI">Nombre d'entrées </label>

                  <select class="pull-left" id="nbEntreeI">
                    ...
                    <input class="pull-right" type="text" name="" id="searchInputI" placeholder="Rechercher...">

                    <!-- <div class="loading-div"><img src="images/loader.gif" alt="loading..." ></div> -->

                    <div id="resultsI"><!-- content will be loaded here --></div>
                              ...
                              <div class="modal-body" style="">
                                ...
                                <form method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group" align="right">
                                        ...
                                        <input type="button" class="boutonbasic" onclick="deleteImg()" id="btnSupImg" name="btnSupImg" value="Supprimer >>"/>
                        ...
                        <div class="modal-body" style="padding:40px 50px;">
                            ...
                            <form role="form" class="" id="form" name="formulaire3" method="post" action="vitrine/vitrine">
                              ...
                              <div class="form-group">
                                ...
                                <label for="uniteDetails"> UNITE DETAILS </label>

                                <input type="text" class="form-control" name="uniteDetails_edit" id="uniteDetails_edit" />
